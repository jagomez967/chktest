IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGDEvolucionPreciosDiario_T22'))
   exec('CREATE PROCEDURE [dbo].[AGDEvolucionPreciosDiario_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[AGDEvolucionPreciosDiario_T22]
(
	@IdCliente			int
 ...
 -------------------------------------------------------------------- END (Filtros) ----------------------------------------------------------------    
  ...
  select 		
	...
	create table #datosFinal(idPRoducto int, producto varchar(max), fecha int, valorFecha varchar(max),
					         valor decimal(11,2), color varchar(max), dashStyle varchar(max), linkedTo int, orden int)

	insert #datosFinal(idPRoducto,producto,fecha,valorFecha,valor,color,dashStyle,linkedTo,orden)
