IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_CobAvanze_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_CobAvanze_T27] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[AGD_CobAvanze_T27] 	
(
	@IdCliente			int
...
*/		
	...
	insert #reportes (idcliente, idpuntodeventa, idreporte)
	select pdv.idcliente, pdv.idpuntodeventa, max(r.idreporte)
	from reporte r
	inner join puntodeventa pdv on pdv.IdPuntoDeVenta = r.IdPuntoDeVenta
	where	pdv.idcliente in (247,249)
			and convert(date,r.fechacreacion) between convert(date, @fechadesde) and convert(date, @fechahasta)		
			AND (ISNULL(@cPuntosDeVenta,0) = 0 OR EXISTS (SELECT 1 FROM #puntosdeventa WHERE idPuntoDeVenta = PDV.IdPuntoDeVenta))
			and (isnull(@cUsuarios,0) = 0 or exists(select 1 from #usuarios where idUsuario = r.IdUsuario)) and r.IdUsuario not in (select idUsuario from usuario where escheckpos = 1)
	group by pdv.idcliente, pdv.idpuntodeventa

	declare @relevados int
	select @relevados = COUNT(*) from
	(
		select p.Nombre, COUNT(*) as cant from #reportes r
	...
	if(@asignados>0)
		set @resultado =  @relevados*100.0/@asignados
	else
		set @resultado = 0

	--Resultados
	SELECT 'Cobertura' SerieName,convert(varchar,CONVERT(DECIMAL(10),@resultado)) + '%'  AS Total, 'AVANCE' Target

	SELECT 'Relevados' as Point,@relevados as value, '#0000FF' as Color, '' as Texto 
	union
	SELECT 'Asignados' as Point ,@asignados  as value, '#3EA9F5' as Color, '' as Texto 
