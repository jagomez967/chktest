IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_CobFaltantes_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_CobFaltantes_T27] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[AGD_CobFaltantes_T27] 	
(
	@IdCliente			int
...
*/		
	...
	insert #reportes (idempresa, idpuntodeventa, idreporte)
	select r.IdEmpresa, r.idpuntodeventa, max(r.idreporte) as idreporte
	from reporte r
	...
	where	c.IdCliente = 249
			...
			and isnull(r.AuditoriaNoAutorizada,0) = 0
	...
	inner join #reportes r on r.IdReporte = rp.IdReporte
	inner join cliente c on c.IdEmpresa = r.IdEmpresa
	...
	select @cantidadreportes = count(*) from #reportes

	declare @resultado int
	if(@cantidadproductos>0 and @cantidadreportes>0)
		set @resultado =  ceiling(@faltantes*100.0/(@cantidadproductos*@cantidadreportes))
	else
		set @resultado = 0


	--Resultados
	SELECT 'Faltantes' SerieName,convert(varchar,@resultado) + '%'  AS Total, 'Faltantes' Target

	SELECT 'Productos faltantes %' as Point,@resultado as value, '#F05A23' as Color, '' as Texto 
	union
	SELECT 'Productos no faltantes %' as Point ,100-@resultado  as value, '#79C843' as Color, '' as Texto 
