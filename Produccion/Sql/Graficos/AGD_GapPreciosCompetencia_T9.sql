IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_GapPreciosCompetencia_T9'))
   exec('CREATE PROCEDURE [dbo].[AGD_GapPreciosCompetencia_T9] AS BEGIN SET NOCOUNT ON; END')
Go
ALTER procedure [dbo].[AGD_GapPreciosCompetencia_T9]  
(  
 @IdCliente   int  
...
*/    
 ...
 -------------------------------------------------------------------- END (Filtros) ----------------------------------------------------------------    
  ...
  create table #competencia(idProducto int,idProductoCompetencia int,precio decimal (11,2),idTipo int)

  insert #competencia (idProducto,idProductoCompetencia, precio,idTipo)
  select 		
		pc.idProducto,
		rp.IdProducto,
		 AVG(
	...
	group by pc.idProducto,rp.idProducto,pdv.idTipo
 ...
 (  
  ...
  (0,0,1,'precioCompetencia','Precio Competencia',5),  
  (0,0,1,'productoCompetencia','SKU Competidor',5),  
  (0,0,1,'gap','Gap',5),  
  (0,0,1,'jmp','Alerta',5)  
    
  create table #datosFinal(id int identity(1,1),tipo varchar(100),producto varchar(200),
				precio varchar(100), precioCompetencia varchar(100),productoCompetencia varchar(200), gap varchar(max), jmp varchar(max))

   insert #datosFinal (tipo,producto,precio,precioCompetencia,productoCompetencia,gap,jmp)
  select t.nombre as tipo,
			p.nombre as producto,
			'$' +convert(varchar(100),tp.precio) as precio,
			'$' +convert(varchar(100),c.precio) as precioCompetencia,
			p2.nombre as productoCompetencia,
			ltrim(rtrim(str(((tp.precio/c.precio)-1)*100,18,2)))+'%' as gap, 
			CASE 
				 when ((tp.precio/c.precio) -1 ) * 100 > 1 then  
				 '<div style="text-align:center;"><img src="images/circuloRojo.png" align="middle" width="16" height="16"/></div>'    
				 when (tp.precio = c.precio)  then  
				 '<div style="text-align:center;"><img src="images/circuloVerde.png" align="middle" width="16" height="16"/></div>'  
				 when ((tp.precio/c.precio) -1 ) * 100 between 0 and 1  then  
				 '<div style="text-align:center;"><img src="images/circuloVerde.png" align="middle" width="16" height="16"/></div>'  
				 when ((tp.precio/c.precio) -1 ) * 100 between -1 and 0  then  
	...
	inner join #competencia c on c.idProducto = tp.idProducto	and c.idTipo = tp.idTipo
	inner join tipo t on t.idTipo = tp.idTipo
	inner join producto p on p.idProducto = tp.idProducto 
	inner join Producto p2 on p2.IdProducto = c.idProductoCompetencia
  ...
  if(@NumeroDePagina>0)
		select tipo,producto,precio,precioCompetencia,productoCompetencia,gap,jmp from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select tipo,producto,precio,precioCompetencia,productoCompetencia,gap,jmp from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select tipo,producto,precio,precioCompetencia,productoCompetencia,gap,jmp from #datosFinal
