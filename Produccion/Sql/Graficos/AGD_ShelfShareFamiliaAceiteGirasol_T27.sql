IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_ShelfShareFamiliaAceiteGirasol_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_ShelfShareFamiliaAceiteGirasol_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[AGD_ShelfShareFamiliaAceiteGirasol_T27]
(
	@IdCliente			INT,
...
BEGIN
 ...
 and not exists (select 1 from #Localidades where idLocalidad = l.idLocalidad)  
	...
	SELECT DISTINCT 'Shelf Share Aceites Girasol' SerieName, 'AGD' Target, 
	CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),(CONVERT(NUMERIC(18,2),(SELECT SUM(Propio) FROM #RawProd WHERE idPRoducto IN (21507,21511)) * 100)) 
	/ 
	NULLIF((SELECT SUM(Propio) FROM #RawProd WHERE idFamilia = 1507),0)
	)) + ' %'  Total, 1 ShowText FROM  #RawProd RP WHERE idProducto IN (21507,21511,21531,21536)
	...
	SELECT 
		...
		/
		NULLIF(CONVERT(NUMERIC(18,2),(SELECT SUM(PROPIO) FROM #RawProd WHERE idFamilia = 1507)),0)
		)) + ' %)' Texto
	FROM  
		#RawProd RP 
		INNER JOIN Producto P ON RP.idProducto = P.idProducto
