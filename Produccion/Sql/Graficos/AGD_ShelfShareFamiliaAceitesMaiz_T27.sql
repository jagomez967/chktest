IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_ShelfShareFamiliaAceitesMaiz_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_ShelfShareFamiliaAceitesMaiz_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[AGD_ShelfShareFamiliaAceitesMaiz_T27]
(
	@IdCliente			INT,
...
BEGIN
 ...
 and not exists (select 1 from #Localidades where idLocalidad = l.idLocalidad)  
	...
	SELECT DISTINCT 'Shelf Share Aceites Maiz' SerieName, 'AGD' Target, 
	CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),(CONVERT(NUMERIC(18,2),(SELECT SUM(Propio) FROM #RawProd WHERE idPRoducto IN (21514)) * 100)) 
	/ 
	NULLIF((SELECT SUM(Propio) FROM #RawProd WHERE idFamilia = 1519),0)
	)) + ' %'  Total, 1 ShowText FROM  #RawProd RP WHERE idProducto IN (21514,21529,21533,21539)
	...
	SELECT 
		...
		/
		NULLIF(CONVERT(NUMERIC(18,2),(SELECT SUM(PROPIO) FROM #RawProd WHERE idFamilia = 1519)),0)
		)) + ' %)' Texto
	FROM  
		#RawProd RP 
		INNER JOIN Producto P ON RP.idProducto = P.idProducto
