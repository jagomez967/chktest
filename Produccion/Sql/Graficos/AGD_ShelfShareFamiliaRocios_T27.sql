IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_ShelfShareFamiliaRocios_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_ShelfShareFamiliaRocios_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[AGD_ShelfShareFamiliaRocios_T27]
(
	@IdCliente			INT,
...
BEGIN
 ...
 and not exists (select 1 from #Localidades where idLocalidad = l.idLocalidad)  
	...
	SELECT DISTINCT 'Shelf Share Rocios' SerieName, 'AGD' Target, 
	CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),(CONVERT(NUMERIC(18,2),(SELECT SUM(Propio) FROM #RawProd WHERE idPRoducto IN (21510,21516)) * 100)) 
		/ NULLIF((SELECT SUM(Propio) FROM #RawProd WHERE idFamilia = 1506),0)
		)) + ' %'  Total, 1 ShowText FROM  #RawProd RP WHERE idProducto IN (21510,21513,21515,21516,21545,21546,21547)
	...
	SELECT 
		...
		/
		NULLIF(CONVERT(NUMERIC(18,2),(SELECT SUM(PROPIO) FROM #RawProd WHERE idFamilia = 1506)),0)
		)) + ' %)' Texto
	FROM  
		#RawProd RP 
		INNER JOIN Producto P ON RP.idProducto = P.idProducto
