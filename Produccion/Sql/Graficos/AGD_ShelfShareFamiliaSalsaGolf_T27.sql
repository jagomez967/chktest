IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.AGD_ShelfShareFamiliaSalsaGolf_T27'))
   exec('CREATE PROCEDURE [dbo].[AGD_ShelfShareFamiliaSalsaGolf_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[AGD_ShelfShareFamiliaSalsaGolf_T27]
(
	@IdCliente			INT,
...
BEGIN
 ...
 if(@strFHasta='00010101' or @strFHasta is null)  
  ...
  select @fechaHasta =  fecha from #fechaCreacionReporte where id = 3  
	...
	SELECT DISTINCT 'Shelf Share Salsa Golf' SerieName, 'AGD' Target,
			CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),(CONVERT(NUMERIC(18,2),(SELECT SUM(Propio) FROM #RawProd WHERE idPRoducto IN (21527,21528)) * 100)) 
			/ 
			NULLIF((SELECT SUM(Propio) FROM #RawProd WHERE idFamilia = 1517),0)
			)) + ' %'  Total, 1 ShowText FROM  #RawProd RP WHERE idProducto IN (21527,21528,21544,21551)
	...
	SELECT 
		...
		/
		NULLIF(CONVERT(NUMERIC(18,2),(SELECT SUM(PROPIO) FROM #RawProd WHERE idFamilia = 1517)),0)
		)) + ' %)' Texto
	FROM  
		#RawProd RP 
		INNER JOIN Producto P ON RP.idProducto = P.idProducto
