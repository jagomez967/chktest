IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_CuotaVentasNotebook_T9'))
   exec('CREATE PROCEDURE [dbo].[ASUS_CuotaVentasNotebook_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_CuotaVentasNotebook_T9]
(
	@IdCliente			int
...
*/		
    ...
    where isnull(rp.Precio,0) > 0
        and etl.Cuota > 0
		and etl.Familia like '%NOTEBOOK%'
		and f.Nombre like '%NOTEBOOK%'
