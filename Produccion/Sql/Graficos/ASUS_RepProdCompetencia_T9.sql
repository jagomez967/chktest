IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_RepProdCompetencia_T9'))
   exec('CREATE PROCEDURE [dbo].[ASUS_RepProdCompetencia_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_RepProdCompetencia_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (Fecha, idPuntoDeVenta, PuntoDeVenta, Usuario, Zona, Provincia, Cadena, idMarca, Marca, idProducto, Producto, Ventas)
	select	convert(varchar, rep.FechaCreacion, 120),
			...
			isnull(rp.Precio,0)
	from ReporteProductoCompetencia rp
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Fecha Reporte',5),('PuntoDeVenta','PuntoDeVenta',5),('Usuario','RTM',5),('Zona','Zona',5),('Provincia','Provincia',5),('Cadena','Cadena',5),('Marca','Marca',5),('Producto','Producto',5),('Ventas','Ventas',5)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Date',5),('PuntoDeVenta','PDV',5),('Usuario','RTM',5),('Zona','Zone',5),('Provincia','Province',5),('Cadena','Retail',5),('Marca','Brand',5),('Producto','Product',5),('Ventas','Sales',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select Fecha, PuntoDeVenta, Usuario, Zona, Provincia, Cadena, Marca, Producto, Ventas from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select Fecha, PuntoDeVenta, Usuario, Zona, Provincia, Cadena, Marca, Producto, Ventas from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select Fecha, PuntoDeVenta, Usuario, Zona, Provincia, Cadena, Marca, Producto, Ventas from #datosFinal
