...
*/		
	...
	*/
	...
	--Datos
	if(@NumeroDePagina>0)
		select pdv, usuario, fecha, estado from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select pdv, usuario, fecha, estado from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
