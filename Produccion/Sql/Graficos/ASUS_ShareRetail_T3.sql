IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_ShareRetail_T3'))
   exec('CREATE PROCEDURE [dbo].[ASUS_ShareRetail_T3] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_ShareRetail_T3] 	
(
	@IdCliente			int
...
create table #Equipo
	...
	select pdv.idCadena, p.idMarca, sum(isnull(rp.Cantidad,0) + isnull(rp.Cantidad2,0)), t.mes from #reportesMesPdv t
	...
	insert #datos (idCadena, idMarca, qty, mes)
	select pdv.idCadena, p.idMarca, sum(isnull(rpc.Cantidad,0) + isnull(rpc.Cantidad2,0)), t.mes from #reportesMesPdv t
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	...
	where (isnull(rpc.Cantidad,0) > 0 or isnull(rpc.Cantidad2,0) > 0)
