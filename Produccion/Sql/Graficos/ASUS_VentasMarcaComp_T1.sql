IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_VentasMarcaComp_T1'))
   exec('CREATE PROCEDURE [dbo].[ASUS_VentasMarcaComp_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[ASUS_VentasMarcaComp_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #datosFinal (marca, ventas)
	select e.Nombre, sum(isnull(rp.Precio,0)) from #tempReporte t
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('marca','Marca'),('ventas','Ventas')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('marca','Brand'),('ventas','Sales')
