IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_VentasMarca_T1'))
   exec('CREATE PROCEDURE [dbo].[ASUS_VentasMarca_T1] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_VentasMarca_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	where isnull(rp.Precio,0) > 0 and p.Reporte = 1
