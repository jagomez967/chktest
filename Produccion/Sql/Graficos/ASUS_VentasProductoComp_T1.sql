IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_VentasProductoComp_T1'))
   exec('CREATE PROCEDURE [dbo].[ASUS_VentasProductoComp_T1] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_VentasProductoComp_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #datosFinal (producto, ventas)
	select p.Nombre, sum(isnull(rp.Precio,0)) from #tempReporte t
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('producto','Producto'),('ventas','Ventas')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('producto','Product Name'),('ventas','Sales')
