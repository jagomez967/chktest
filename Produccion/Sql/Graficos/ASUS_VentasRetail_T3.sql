IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ASUS_VentasRetail_T3'))
   exec('CREATE PROCEDURE [dbo].[ASUS_VentasRetail_T3] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ASUS_VentasRetail_T3] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idCadena, idMarca, qty, mes)
	select pdv.idCadena, p.idMarca, sum(isnull(rp.Precio,0)), t.mes from #reportesMesPdv t
	...
	group by pdv.idCadena, p.idMarca, t.mes

	insert #datos (idCadena, idMarca, qty, mes)
	select pdv.idCadena, p.idMarca, sum(isnull(rpc.Precio,0)), t.mes from #reportesMesPdv t
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	inner join ReporteProductoCompetencia rpc on rpc.idReporte = t.idReporte
	...
	where isnull(rpc.Precio,0) > 0
			and (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = mc.IdMarca))
			and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = pc.IdProducto))
			and (isnull(@cCompetenciaPrimaria,0)=0 or m.idmarca in (select MarcaCompetencia.idmarcaCompetencia from MarcaCompetencia where MarcaCompetencia.idMarca in (select #CompetenciaPrimaria.idMarcaComp from #CompetenciaPrimaria) and MarcaCompetencia.esCompetenciaPrimaria = 1))
	group by pdv.idCadena, p.idMarca, t.mes


	select	ltrim(rtrim(c.Nombre)),
			ltrim(rtrim(m.Nombre)),
			isnull(d.qty,0),
			d.idCadena,
			left(convert(varchar,d.mes,112),6)
	from #datos d
