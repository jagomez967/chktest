IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Anwo_LoginLogut_T9'))
   exec('CREATE PROCEDURE [dbo].[Anwo_LoginLogut_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Anwo_LoginLogut_T9]
(
	@IdCliente			int
...
begin
	...
	insert #datos (usuario, operacion, fecha)
	...
	where	exists(select 1 from #clientes where idCliente = cu.idCliente) 
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('usuario','Usuario',5),('operacion','Operación',5),('fecha','Fecha',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('usuario','User',5),('operacion','Operación',5),('fecha','Date',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select usuario, operacion, fecha from #datos where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select usuario, operacion, fecha from #datos where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select usuario, operacion, fecha from #datos
end