IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.BancoChile_ConteoElementosCriticosGGEE_T9'))
   exec('CREATE PROCEDURE [dbo].[BancoChile_ConteoElementosCriticosGGEE_T9] AS BEGIN SET NOCOUNT ON; END')
GO	
alter procedure [dbo].[BancoChile_ConteoElementosCriticosGGEE_T9]
(
	@IdCliente			int
...
*/		
	...
	where (isnull(rp.Cantidad,0)>0 or isnull(rp.Cantidad2,0)>0 or isnull(Precio,0)>0)
		...
		and rp.idProducto in (8414,8415,8416)
	order by pdv.Nombre
