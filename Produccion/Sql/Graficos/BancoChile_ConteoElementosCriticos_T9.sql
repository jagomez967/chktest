IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.BancoChile_ConteoElementosCriticos_T9'))
   exec('CREATE PROCEDURE [dbo].[BancoChile_ConteoElementosCriticos_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[BancoChile_ConteoElementosCriticos_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (idPuntoDeVenta, PuntoDeVenta, Usuario, Zona, Producto, Frentes1, Frentes2, Fecha)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('PuntoDeVenta','PuntoDeVenta',5),('Usuario','RTM',5),('Zona','Zona',5),('Producto','Producto',5), ('Fecha','Fecha',5),('Frentes1','Cantidad buena',5),('Frentes2','Cantidad mala',5)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('PuntoDeVenta','PDV',5),('Usuario','RTM',5),('Zona','Zone',5),('Producto','Product',5), ('Fecha','Date',5),('Frentes1','Good Qty',5),('Frentes2','Bad Qty',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select PuntoDeVenta, Usuario, Zona, Producto, Fecha, Frentes1, Frentes2 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select PuntoDeVenta, Usuario, Zona, Producto, Fecha, Frentes1, Frentes2 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select PuntoDeVenta, Usuario, Zona, Producto, Fecha, Frentes1, Frentes2 from #datosFinal
