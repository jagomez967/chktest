IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.BancoChile_SucursalesSinAuditar_T9'))
   exec('CREATE PROCEDURE [dbo].[BancoChile_SucursalesSinAuditar_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[BancoChile_SucursalesSinAuditar_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosfinal (idpdv,nombrePdv,direccion,zona)
	select distinct p.idPuntoDeVenta, 
		   p.nombre,
		   p.direccion,
		   z.nombre
	from puntoDeVenta p
	inner join usuario_puntoDeVenta up
		on up.idPuntoDeVenta = p.idPuntoDeVenta
	inner join zona z 
		on z.idZona = p.idZona 
	where p.idPuntoDeVenta not in
	(select idPuntoDeVenta from #tempReporte)
	...
	and (isnull(@cJefeGestion,0) = 0 or exists(select 1 from #JefeGestion where idJefeGestion in (select isnull(id,0) from extData_bancochile_filtroBI where idPuntoDeVenta = p.idPuntoDeVenta)))
	and (isnull(@cJefeZonal,0) = 0 or exists(select 1 from #JefeZonal where idJefeZonal in (select isnull(idZonal,0) from extData_bancochile_filtroBI where idPUntoDeVenta = p.idPuntoDeVenta)))
	and (isnull(@cUsuarios,0) = 0 or exists(select 1 from #usuarios where idUsuario = up.IdUsuario))	   		
	declare @maxpag int
	...
	select idPdv,nombrePdv,direccion,zona
	from #datosFinal
