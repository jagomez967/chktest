...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.BancoChile_PerformancePorItem_T1'))
   exec('CREATE PROCEDURE [dbo].[BancoChile_PerformancePorItem_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[BancoChile_PerformancePorItem_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Filtros) ----------------------------------------------------------------
	...
	( idModulo int,
	  nombreModulo varchar(100) collate database_default,
	  qty decimal(9,2)
	)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('NombreMarca','Item'),('qty','Performance')
