IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Bayton_SharePop_T2'))
   exec('CREATE PROCEDURE [dbo].[Bayton_SharePop_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Bayton_SharePop_T2]
(
	@IdCliente			int
...
*/		
	...
	select rp.idPop, r.mes, sum(isnull(rp.Cantidad,0))
	from #tempReporte r
	...
	select LTRIM(rtrim(p.Nombre)),SUM(isnull(d.qty,0)), te.idPop
	from #PopMeses te
	left join #datos d on d.idPop=te.idPop and d.mes=te.mes
	inner join Pop p on p.idPop = te.idPop
	group by rtrim(p.nombre), te.idPop
	order by te.idPop
end