IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Buhl_DistCadenas_T9'))
   exec('CREATE PROCEDURE [dbo].[Buhl_DistCadenas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Buhl_DistCadenas_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #RelevadosCadena (qty, idCadena)
	select count(distinct r.idReporte), pdv.idCadena from #reportesMesPdv r
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = r.idPuntoDeVenta
	group by pdv.idCadena


	insert #datos (idEmpresa, Empresa, idCadena, qty)
	select m.idEmpresa, e.Nombre, pdv.idCadena, count(distinct r.idPuntoDeVenta) from #reportesMesPdv r
	...
	group by m.idEmpresa, e.Nombre, pdv.idCadena


	insert #datos (idEmpresa, Empresa, idCadena, qty)
	select m.idEmpresa, m.Nombre, pdv.idCadena, count(distinct r.idPuntoDeVenta) from #reportesMesPdv r
	inner join ReporteProductoCompetencia rp on rp.idReporte = r.idReporte
	inner join Producto p on p.idProducto = rp.idProducto
	inner join Marca m on m.idMarca = p.idMarca
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = r.idPuntoDeVenta
	where isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0) > 0
	group by m.idEmpresa, m.Nombre, pdv.idCadena
	...
	insert #datosRes (d.idEmpresa, Empresa, d.idCadena, qty)
	select 'col'+cast(d.idEmpresa as varchar(100)), d.Empresa, d.idCadena, avg(d.qty*100.0/rz.qty) from #datos d
	inner join #RelevadosCadena rz on rz.idCadena = d.idCadena
	group by d.idEmpresa, d.Empresa, d.idCadena

	insert #totalEmpresa (idEmpresa, total)
	select idEmpresa, sum(qty)/(select count(distinct idCadena) from #datos) from #datosRes
	group by idEmpresa
	...
	set @ColDef='idCadena varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idCadena],'+@PivotColumnHeaders+')
	SELECT [idCadena],'+@PivotColumnHeaders+'
	  FROM (
		Select idEmpresa, idCadena, qty from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(qty)
		FOR idEmpresa IN (
	...
	insert #DatosPivotFinal ([idCadena],'+@PivotColumnHeaders+')
	select [idCadena],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Cadena'+char(39)+','+char(39)+'Cadena'+char(39)+', 80, -1)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select c.Nombre as Cadena, rz.qty as TotalPdv, '+@PivotColumnHeaders+'
			...
			union all
			select total collate database_default as Cadena, (select sum(qty) from #RelevadosCadena) as TotalPdv, '+@PivotColumnHeaders+' from #totalRow
