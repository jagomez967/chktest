IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Buhl_PrecioPromedioComp_T9'))
   exec('CREATE PROCEDURE [dbo].[Buhl_PrecioPromedioComp_T9] AS BEGIN SET NOCOUNT ON; END')
GO

alter procedure [dbo].[Buhl_PrecioPromedioComp_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (categoria, marca, producto, promedio)
	select	ltrim(rtrim(f.Nombre)),
			ltrim(rtrim(m.Nombre)),
			ltrim(rtrim(p.Nombre)),
			avg(isnull(rp.Cantidad,0))
	...
	inner join Familia f on f.idMarca = m.idMarca
	group by f.Nombre, m.Nombre, p.Nombre
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('categoria','Categoria',5),('marca','Marca',5),('producto','Producto',5),('promedio','Precio Promedio',5)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('categoria','Category',5),('marca','Brand',5),('producto','Product',5),('promedio','Avg Price',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select categoria, marca, producto, promedio from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select categoria, marca, producto, promedio from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select categoria, marca, producto, promedio from #datosFinal
...
--	Buhl_PrecioPromedioComp_T9 100