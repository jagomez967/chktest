IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.CM_PuntajeEncuesta_T9'))
   exec('CREATE PROCEDURE [dbo].[CM_PuntajeEncuesta_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].CM_PuntajeEncuesta_T9
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden)
	select	r.idReporte,
			r.idPuntoDeVenta,
			r.Usuario,
	...
	insert #PuntajePdvs (idReporte, pdv, puntaje, acumulado)
	select idReporte, pdv, sum(convert(int, valor)), CEILING(sum(convert(numeric(18,2), valor)*100)/30) from #datos
	group by idReporte, pdv
	...
	select	r.idReporte,
			r.IdPuntoDeVenta,
			r.usuario,
			case when isnull(mdr.Valor1,0) = 1 then LabelCampo1
			when isnull(mdr.Valor2,0) = 1 then LabelCampo2 end,
			mi.IdItem,
			mdr.idMarca,
			mi.Nombre,
			mi.Orden as preg
	...
	where mdr.idItem in (8516,8517,8518)
	...
	insert #datosRes (idreporte, iditem, idmarca, preg, pdv, usuario, valor)
	select idreporte, idItem, idmarca, preg, pdv, usuario, valor from #datos order by orden
	...
	set @ColDef='idreporte varchar(max), pdv varchar(max), usuario varchar(max), idmarca varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idreporte],[pdv],[usuario],[idmarca],'+@PivotColumnHeaders+')
	SELECT [idreporte],[pdv],[usuario],[idmarca],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, idreporte, pdv, usuario, idmarca, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([idreporte],[pdv],[usuario],[idmarca],'+@PivotColumnHeaders+')
	select [idreporte],[pdv],[usuario],[idmarca],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idReporte'+char(39)+','+char(39)+'idReporte'+char(39)+', 80, 0)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Nombre'+char(39)+','+char(39)+'PDV'+char(39)+', 80, 1)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Puntaje'+char(39)+','+char(39)+'Puntaje'+char(39)+', 80, 9998)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Acumulado'+char(39)+','+char(39)+'Acumulado'+char(39)+', 80, 9999)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select d.idReporte as idReporte, pdv.Nombre,'+@PivotColumnHeaders+', pp.Puntaje, cast(pp.Acumulado as varchar(10))+'' %'' as Acumulado
			from #DatosPivotFinal d
			inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = d.pdv
			inner join #PuntajePdvs pp on pp.pdv = d.pdv and pp.idReporte = d.idReporte
