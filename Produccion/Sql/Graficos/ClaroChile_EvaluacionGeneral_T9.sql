--ClaroChile_EvaluacionGeneral_T9 11
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ClaroChile_EvaluacionGeneral_T9'))
   exec('CREATE PROCEDURE [dbo].[ClaroChile_EvaluacionGeneral_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[ClaroChile_EvaluacionGeneral_T9]
(
	@IdCliente			int
...
*/		
	...
	group by r.IdEmpresa, r.IdPuntoDeVenta--, FechaCreacion
	...
	SELECT temp.idPuntoDeVenta as IdPDV 
		  ...
		  , round((sum((t.Valor * t.PonderacionItem / 100.0)* (t.PonderacionModulo /100.0) * (t.PonderacionMarca / 100.0))/count(distinct t.idReporte))*100.0,2)
			--avg(cast(i.Nombre as numeric(18,2))) as qty
	...
	inner join md_item i on i.idItem = t.idItem
	where t.valor = 1
	...
	declare @Evaluacion decimal(9,2)

	select @Evaluacion = avg(Cumplimiento)
	from #datosFinal
	...
	if(@TamañoPagina=0)
		...
		select @maxpag=ceiling(1.0/@TamañoPagina) from #datosFinal
	...
	if(@lenguaje = 'es')
	begin
		insert #columnasConfiguracion (name, title, width) values ('Evaluacion','Evaluacion General', 5)
	end
	if(@lenguaje='en')
	begin
		insert #columnasConfiguracion (name, title, width) values ('Evaluacion','Evaluacion General', 5)
	...
	select '<h1>'+'<div style="text-align:center;font-weight:bold">'+cast(@Evaluacion as varchar)+'</div></h1>' as Evaluacion
