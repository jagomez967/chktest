...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ClaroChile_EvaluacionPorItem_T1'))
   exec('CREATE PROCEDURE [dbo].[ClaroChile_EvaluacionPorItem_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[ClaroChile_EvaluacionPorItem_T1] 	
(
	@IdCliente			int
...
*/		
	...
	--SELECT * fROM #tablaPonderaciones order by idReporte,idModulo,idItem
	/*-----------------------FIN LLENADO TABLA TEMPORAL---------------------------------*/
	create table #datosFinal
	( idModulo int,
	  nombreModulo varchar(500) collate database_default,
	  qty decimal(9,2)
	)

	insert #datosFinal(idModulo,nombreModulo,qty)
	select t.idModulo,
		   M.Nombre,
		  round((sum(t.Valor * t.PonderacionItem / 100.0)/count(distinct t.idReporte))*100.0,2)
	 from #tablaPonderaciones t
	 inner join md_modulo M on M.idModulo = t.idModulo
	 group by t.idModulo,M.nombre
	 order by t.idModulo
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('NombreMarca','Item'),('qty','Evaluacion')
	...
	select NombreModulo,qty from #datosFinal 
