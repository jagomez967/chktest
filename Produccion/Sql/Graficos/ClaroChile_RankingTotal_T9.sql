IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ClaroChile_RankingTotal_T9'))
   exec('CREATE PROCEDURE [dbo].[ClaroChile_RankingTotal_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[ClaroChile_RankingTotal_T9]
(
	@IdCliente			int
...
*/		
	...
	--Datos
		select row_number() OVER(ORDER BY cumplimiento DESC) AS Posicion,NombrePDV,Direccion,
