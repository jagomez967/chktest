IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Claro_Relevo_T9'))
   exec('CREATE PROCEDURE [dbo].[Claro_Relevo_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Claro_Relevo_T9
(
	@IdCliente			int
...
*/		
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre as Modulo, mdm.Nombre as Item,'+@PivotColumnHeaders+'
