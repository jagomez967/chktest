...
alter procedure [dbo].[Cob_DistExhQuiebre_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	select mes, isnull(count(distinct r.idPuntoDeventa),0) from #tempReporte t
	...
	insert #aud (mes, qty)
	select r.mes, isnull(r.qty,0)-isnull(n.qty,0) from #Relevados r
	...
	(
		...
		where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where #marcas.idMarca = producto.idMarca))
				...
				and reporteproducto.idreporte=r.idreporte and ReporteProducto.stock=0
	)
	and (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = prod.idMarca))
	and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = prod.idProducto))
	group by t.mes
	...
	(
		...
		where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where #marcas.idMarca = producto.idMarca))
				and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where #productos.idProducto = producto.idProducto))
				and reporteproducto.idreporte=r.idreporte and ReporteProducto.notrabaja=0
	)
	and (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = prod.idMarca))
	and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = prod.idProducto))
	group by t.mes
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('cobertura','Cobertura'),('auditados','Auditados'),('distribucion','Distribucion'),('exhibicion','Exhibicion'),('quiebre','Quiebre'),('notrabaja','No Trabajan')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('mes','month'),('cobertura','Cobertura'),('auditados','Audited'),('distribucion','Distribution'),('exhibicion','Exhibition'),('quiebre','Quiebre'),('notrabaja','No Trabajan')
