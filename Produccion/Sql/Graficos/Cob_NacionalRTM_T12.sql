IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_NacionalRTM_T12'))
   exec('CREATE PROCEDURE [dbo].[Cob_NacionalRTM_T12] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Cob_NacionalRTM_T12]
(
	@IdCliente			int
...
*/		
	...
	select idusuario, count(IdPuntoDeVenta) from #tempReporte t
	group by idusuario


	insert #datos (idUsuario, valor)
	select r.idUsuario, cast(r.qty*100.00/a.Visitas as decimal(18,2)) from #Relevados r
	inner join ETL_Andromaco_ObjetivoRTM a on a.idRTM = r.idUsuario
