IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_ObjetivoAndromaco_T1'))
   exec('CREATE PROCEDURE [dbo].[Cob_ObjetivoAndromaco_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_ObjetivoAndromaco_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('usuario','Usuario'),('visitas','Visitas'),('relevados','Relevados')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('usuario','User'),('asignados','Visits'),('relevados','Visited')

	select name, title from #columnasConfiguracion

	select u.Apellido+', '+u.Nombre collate database_default as usuario
	,etl.Visitas as visitas
	,isnull(r.qty,0) as relevados
	from #Relevados r
	inner join Usuario u on u.IdUsuario=r.idUsuario
	inner join ETL_Andromaco_ObjetivoRTM etl on etl.idRTM = r.idUsuario
