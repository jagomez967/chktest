IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_ObjetivoAndromaco_T9_datos_anidados'))
   exec('CREATE PROCEDURE [dbo].[Cob_ObjetivoAndromaco_T9_datos_anidados] AS BEGIN SET NOCOUNT ON; END')
GO --Cob_Usuarios_T9_Datos_Anidados 9
alter procedure [dbo].[Cob_ObjetivoAndromaco_T9_datos_anidados]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (nested1, nested3,nested2)
	select ltrim(rtrim(pdv.nombre)), cast(count(r.idpuntodeventa)as varchar),
	ltrim(rtrim(pdv.direccion collate database_Default))
		from #Asignados a
	...
	group by pdv.nombre,pdv.direccion
	order by pdv.nombre
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('nested1','Punto de Venta',50)
		,('nested2','Direccion',30)
		,('nested3','Visitas',30)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('nested1','Point of Sale',50)
		,('nested2','Address',30)
		,('nested3','Visited',30)

	select name, title, width from #columnasConfiguracion

	select nested1, nested2,nested3 from #datosFinal-- order by nested2 desc
end
go
