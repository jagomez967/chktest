IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_Objetivo_T1'))
   exec('CREATE PROCEDURE [dbo].[Cob_Objetivo_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_Objetivo_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	select mes, count(distinct r.idPuntoDeventa) from #tempReporte t
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('asignados','Asignados'),('relevados','Relevados'),('auditados','Auditados')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('asignados','Assigned'),('relevados','Visited'),('auditados','Audited')

	select name, title from #columnasConfiguracion

	select right(CONVERT(VARCHAR(11),convert(datetime, A.mes),6),6) as mes, A.qty as asignados, isnull(V.qty,0) as relevados, isnull(V.qty,0)-isnull(N.qty,0) as auditados from #Asignados A left join #Relevados V on V.mes=A.mes left join #NOAuditados N on N.mes=A.mes
