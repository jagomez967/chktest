IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvAudNoAut_T1'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvAudNoAut_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PdvAudNoAut_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('relevados','Relevados'),('noautorizados','No Autorizados')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('relevados','Visited'),('noautorizados','Not Authorized')

	select name, title from #columnasConfiguracion

	select right(CONVERT(VARCHAR(11),convert(datetime, A.mes),6),6) as mes, isnull(V.qty,0) as relevados, isnull(A.qty,0) as noautorizados from #NoAut A left join #Relevados V on V.mes=A.mes
