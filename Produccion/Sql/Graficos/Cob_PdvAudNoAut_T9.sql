IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvAudNoAut_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvAudNoAut_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Cob_PdvAudNoAut_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #tempReporte (idCliente, idUsuario, IdPuntoDeVenta, fecha, mes, idReporte)
	select	c.IdCliente
			,r.IdUsuario
			,r.IdPuntoDeVenta
			,r.fechaCreacion
	...
	group by c.IdCliente ,r.IdUsuario ,r.IdPuntoDeVenta,r.fechaCreacion ,convert(varchar, dateadd(day, -(day(FechaCreacion) - 1), FechaCreacion),112)
	...
	insert #datosFinal (fecha, idreporte, nombre, idpdv, npdv, dirpdv, localidad, zona)
	select	distinct
			CONVERT(VARCHAR(10),t.fecha,103),
			t.idReporte,
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Fecha',20),('idReporte','idReporte',20),('Nombre','Usuario',20),('idpdv','idPuntoDeVenta',20),('npdv','NombrePdv',50),('dirpdv','Direccion',50),('localidad','Localidad',50),('zona','Zona',50)--,('cadena','Cadena',50)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Date',20),('idReporte','ReportId',20),('Nombre','User',20),('idpdv','POS Id',20),('npdv','POS Name',50),('dirpdv','Address',50),('localidad','District',50),('zona','Zone',50)--,('cadena','Retail',50)

	IF (@lenguaje = 'es' AND @idCliente = 174) BEGIN
		UPDATE #columnasConfiguracion SET title = 'Médico' WHERE name = 'npdv'
	END
	...
	--Datos
	if(@NumeroDePagina>0)
		select fecha,idReporte,Nombre,idpdv, npdv, dirpdv, localidad, zona from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select fecha,idReporte,Nombre,idpdv, npdv, dirpdv, localidad, zona from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select fecha,idReporte,Nombre,idpdv, npdv, dirpdv, localidad, zona from #datosFinal
end
go
