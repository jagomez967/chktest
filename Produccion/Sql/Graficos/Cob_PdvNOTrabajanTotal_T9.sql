alter procedure [dbo].[Cob_PdvNOTrabajanTotal_T9]
(
	@IdCliente			int
...
*/		
	...
	create table #PreDatos(idPuntodeventa int, idProducto int, noTrabaja int)

	insert #PreDatos(idPuntodeventa,idProducto,noTrabaja)
	select	t.IdPuntoDeVenta, rp.idProducto, rp.noTrabaja	
	from #tempReporte t
	inner join reporte r on r.idreporte=t.idreporte
	inner join ReporteProducto rp on rp.idReporte = r.idReporte
	inner join producto prod on prod.idproducto = rp.idproducto
	where (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = prod.idMarca))
	...
	and (isnull(@cCompetenciaPrimaria,0) = 0 or exists (select 1 from #CompetenciaPrimaria where idMarca = prod.idMarca)) 
	
	--Elimino los puntos de venta que tenga el NO trabaja en 0, para dejar aquellos que lo tengan checkeado
	delete from #PreDatos 
	where idPuntodeventa in(select idPuntodeventa from #PreDatos where noTrabaja = 0)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('idpdv','idPuntoDeVenta',20),('npdv','NombrePdv',50),('dirpdv','Direccion',50),('localidad','Localidad',50),('zona','Zona',50),('cadena','Cadena',50)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('idpdv','idPdv',20),('npdv','PDV Name',50),('dirpdv','Address',50),('localidad','District',50),('zona','Zone',50),('cadena','Retail',50)
