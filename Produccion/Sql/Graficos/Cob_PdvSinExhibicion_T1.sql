IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvSinExhibicion_T1'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvSinExhibicion_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PdvSinExhibicion_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #datos (mes, qty)
	select count(*), mes from
	(
		select r.IdPuntoDeVenta, mes, sum(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0)) as exhibicion
		from #tempReporte r
	...
	select distinct mes, qty from #datos
