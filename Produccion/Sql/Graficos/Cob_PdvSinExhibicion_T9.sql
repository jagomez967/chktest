IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvSinExhibicion_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvSinExhibicion_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PdvSinExhibicion_T9]
(
	@IdCliente			int
...
*/		
	...
	and (isnull(@cCompetenciaPrimaria,0) = 0 or exists (select 1 from #CompetenciaPrimaria where #CompetenciaPrimaria.idMarcaComp = p.idMarca)) 
	...
	insert #datosFinal (idpdv, npdv, dirpdv, localidad, zona, contacto, vendedor)
	select distinct t.idPuntoDeVenta, pdv.Nombre, pdv.Direccion, l.Nombre, z.Nombre, pdv.Contacto, v.Nombre 
	from #tempReporte t
	...
	inner join Marca m on m.idMarca = p.idMarca
	left join PuntoDeVenta_Vendedor pdvv on pdvv.idPuntoDeVenta = t.idPuntoDeVenta
	left join Vendedor v on v.idVendedor = pdvv.idVendedor
	left join Equipo e on e.idEquipo = v.idEquipo
	where (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where #productos.idProducto = rp.IdProducto))
	group by t.idPuntoDeVenta, pdv.Nombre, pdv.Direccion, l.Nombre, z.Nombre, pdv.Contacto, v.Nombre
	having isnull(sum(rp.Cantidad),0) = 0

	insert #datosFinal (idpdv, npdv, dirpdv, localidad, zona, contacto, vendedor)
	select distinct t.idPuntoDeVenta, pdv.Nombre, pdv.Direccion, l.Nombre, z.Nombre, pdv.Contacto, v.Nombre 
	from #tempReporte t
	inner join Reporte rp on rp.idReporte = t.idReporte
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	inner join Localidad l on l.idLocalidad = pdv.idLocalidad
	inner join Zona z on z.idZona = pdv.idZona
	left join PuntoDeVenta_Vendedor pdvv on pdvv.idPuntoDeVenta = t.idPuntoDeVenta
	left join Vendedor v on v.idVendedor = pdvv.idVendedor
	left join Equipo e on e.idEquipo = v.idEquipo
	where isnull(rp.AuditoriaNoAutorizada,0) = 1
	group by t.idPuntoDeVenta, pdv.Nombre, pdv.Direccion, l.Nombre, z.Nombre, pdv.Contacto, v.Nombre
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('idpdv','idPuntoDeVenta',20),('npdv','NombrePdv',50),('dirpdv','Direccion',50),('localidad','Localidad',50),('zona','Zona',50),('contacto','Contacto',50),('vendedor','Vendedor',50)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('idpdv','idPdv',20),('npdv','PDV Name',50),('dirpdv','Address',50),('localidad','District',50),('zona','Zone',50),('contacto','Contact',50),('vendedor','Salesman',50)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select idpdv, npdv, dirpdv, localidad, zona, contacto, vendedor from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select idpdv, npdv, dirpdv, localidad, zona, contacto, vendedor from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select idpdv, npdv, dirpdv, localidad, zona, contacto, vendedor from #datosFinal
end
GO

--COB_PDVSINEXHIBICION_T9 44