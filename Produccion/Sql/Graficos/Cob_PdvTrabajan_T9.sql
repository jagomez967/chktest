IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvTrabajan_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvTrabajan_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PdvTrabajan_T9]
(
	@IdCliente			int
...
*/		
	...
	where (isnull(rp.Stock,0) = 0 and isnull(rp.NoTrabaja,0) = 0) or (isnull(rp.Stock,0) = 1 and isnull(rp.NoTrabaja,0) = 0)
	...
	and (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = (select idMarca from producto where idProducto = rp.idProducto)))
