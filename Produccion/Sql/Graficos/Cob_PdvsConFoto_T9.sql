IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PdvsConFoto_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_PdvsConFoto_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PdvsConFoto_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datosfinal(fecha,idpdv, pdv, direccion, localidad, zona, rtm, foto)
	select
			 t.fecha
	        ,t.idPuntoDeVenta
			...
			,case when exists (select 1 from PuntoDeVentaFoto pdvf where pdvf.idReporte = t.idReporte) then count(t.idReporte)
			else 0 end
	from #tempReporte t
	...
	where	convert(date,FechaCreacion) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
			...
			and (isnull(@cTipoRtm,0) = 0 or exists(select 1 from UsuarioPerfil upe where upe.IdUsuario=u.IdUsuario and upe.IdPerfil in (select IdTipoRTM from #tipoRtm)))
	...
	if(@lenguaje='es')	
		insert #columnasConfiguracion (name, title, width) values ('fecha','fecha',5),('idpdv','idPuntoDeVenta',5),('pdv','PDV',5),('direccion','Dirección',5),('localidad','Localidad',5),('zona','Zona',5),('rtm','RTM',5),('foto','Foto',5)

	if(@lenguaje='en')	
		insert #columnasConfiguracion (name, title, width) values ('fecha','fecha',5),('idpdv','idPuntoDeVenta',5),('pdv','POS',5),('direccion','Address',5),('localidad','Location',5),('zona','Zone',5),('rtm','RTM',5),('foto','Picture',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select fecha,idpdv, pdv, direccion, localidad, zona, rtm, foto from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select fecha,idpdv, pdv, direccion, localidad, zona, rtm, foto from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select fecha,idpdv, pdv, direccion, localidad, zona, rtm, foto from #datosFinal
end
go

 
[Cob_PdvsConFoto_T9] 190