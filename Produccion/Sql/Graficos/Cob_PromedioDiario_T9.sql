IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_PromedioDiario_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_PromedioDiario_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_PromedioDiario_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #tempDatos (usuario, puntodeventa, fechacreacion, mes, idreporte, idzona)
	select	r.idusuario
			,r.IdPuntoDeVenta
	...
	insert #cantDias
	select count(distinct(fechacreacion)), usuario, mes from #tempDatos
	...
	insert #datosRes
	select isnull(count(idreporte)/c.cantdias,0), t.usuario, t.mes from #tempDatos t
	inner join #cantDias c on c.usuario = t.usuario and c.mes = t.mes
	group by c.cantdias, t.usuario, t.mes
	order by t.usuario, t.mes
	...
	select @cantUsuarios = count(distinct usuario) from #datosRes
	...
	select usuario, sum(isnull(promedio,0))/@cantMeses from #datosRes
	group by usuario
	...
	insert #totalesMeses
	select mes, sum(isnull(promedio,0))/@cantUsuarios from #datosRes
	group by mes
	union all
	select 'totalRow',isnull(sum(total),0)/@cantUsuarios from #totalesUsuario


	declare @minRojo int=0, @maxRojo int=16
	declare @minAmarillo int=17, @maxAmarillo int=19
	declare @minVerde int=20
	...
	alter column promedio varchar(100)
	

	if @NumeroDePagina>=0
	begin

	update #datosRes set promedio = case when promedio between @minRojo and @maxRojo then '<img src="images/circuloRojo.png" width="16" height="16"/>'
	when promedio between @minAmarillo and @maxAmarillo then '<img src="images/circuloAmarillo.jpg" width="16" height="16"/>'
	when promedio >= @minVerde then '<img src="images/circuloVerde.png" width="16" height="16"/>' end +' '+ cast(promedio as varchar(100))
	...
	--select cast(promedio as varchar(100))+' / '+cast(d2.cantDias as varchar(100))+' dias' from #datosRes d1, #cantDias d2
	--where d1.usuario = d2.usuario and d1.mes = d2.mes

	--select * from #cantDias
	--return
	...
	insert #DatosPivot([usuario],'+@PivotColumnHeaders+')
	SELECT [usuario],'+@PivotColumnHeaders+'
	  FROM (
		Select usuario, mes, promedio from #DatosRes
		) AS PivotData
	  PIVOT (
		max(promedio)
		FOR mes IN (
	...
	insert #DatosPivotFinal ([usuario],'+@PivotColumnHeaders+')
	select [usuario],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	update #DatosPivotFinal set total = t.total from (select usuario, total from #totalesUsuario) t where t.usuario = #DatosPivotFinal.usuario
	...
	select distinct cast(i.mes as varchar) as name, datename(month,i.fechacreacion) as title, 40 as width, 5 as orden
	from #tempDatos i
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select ltrim(rtrim(u.Apellido)) as usuario,'+@PivotColumnHeaders+', tu.total
			from #DatosPivotFinal d
			inner join Usuario u on u.idusuario = d.usuario
			inner join #totalesUsuario tu on tu.usuario = d.usuario
