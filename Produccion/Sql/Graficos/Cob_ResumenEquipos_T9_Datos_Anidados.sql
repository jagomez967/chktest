IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_ResumenEquipos_T9_Datos_Anidados'))
   exec('CREATE PROCEDURE [dbo].[Cob_ResumenEquipos_T9_Datos_Anidados] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_ResumenEquipos_T9_Datos_Anidados]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEquipo, idCadena, qty)
	select e.idEquipo, isnull(pdv.idCadena,0), count(distinct a.idpuntodeventa) from #asignados a
	...
	group by e.idEquipo, pdv.idCadena
	...
	insert #datosDist (idEquipo, idCadena, qty1, qty2)
	select e.idEquipo, pdv.idCadena, count(distinct t.idPuntoDeVenta), count(distinct t.idPuntoDeVenta)*100/d.qty from #tempReporte t
	...
	inner join #datos d on d.idCadena = pdv.idCadena and d.idEquipo = e.idEquipo
	where isnull(rp.Stock,0) = 0
	group by e.idEquipo, pdv.idCadena, d.qty
	...
	insert #datosExh (idEquipo, idCadena, qty1, qty2)
	select e.idEquipo, pdv.idCadena, count(distinct t.idPuntoDeventa), count(distinct t.idPuntoDeVenta)*100/d.qty from #tempReporte t
	inner join ReporteProducto rp on rp.idReporte = t.idReporte
	inner join Equipo_RTM e on e.idUsuario = t.idUsuario
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	...
	insert #datosFac (idEquipo, idCadena, qty1)
	select e.idEquipo, pdv.idCadena, sum(isnull(Cantidad,0))+sum(isnull(cantidad2,0)) from #tempReporte t
	inner join ReporteProducto rp on rp.idReporte = t.idReporte
	inner join Equipo_RTM e on e.idUsuario = t.idUsuario
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	inner join #datos d on d.idCadena = pdv.idCadena and d.idEquipo = e.idEquipo
	...
	insert #datosQuiebre (idEquipo, idCadena, qty1)
	select idEquipo, idCadena, sum(qty) from (
	select e.idEquipo, pdv.idCadena, 1 as qty from #tempReporte t
	inner join ReporteProducto rp on rp.idReporte = t.idReporte
	inner join Equipo_RTM e on e.idUsuario = t.idUsuario
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = t.idPuntoDeVenta
	inner join #datos d on d.idCadena = pdv.idCadena and d.idEquipo = e.idEquipo
	group by e.idEquipo, pdv.IdCadena, t.idreporte
	having count(distinct idproducto) = sum(stock)) as EE
	group by idEquipo, idCadena
	...
	insert #datosFinal (idEquipo, Cadena, q,q2,q3,q4,q5,q6,q7,q8,q9)
	select	d.idEquipo,
			c.Nombre,
			d.qty,
			d1.qty1 as Dist,
			...
			from #datos d
	left join #datosDist d1 on d1.idEquipo = d.idEquipo and d1.idCadena = d.idCadena
	left join #datosExh d2 on d2.idEquipo = d.idEquipo and d2.idCadena = d.idCadena
	left join #datosFac d3 on d3.idEquipo = d.idEquipo and d3.idCadena = d.idCadena
	left join #datosQuiebre d4 on d4.idEquipo = d.idEquipo and d4.idCadena = d.idCadena
	left join Cadena c on c.idCadena = d.idCadena
	union all
	select	0,
			'Total',
			sum(d.qty),
			sum(d1.qty1) as Dist,
			sum(d1.qty2)/count(distinct d.idCadena) as '%Dist',
			sum(d2.qty1) as Exh,
			sum(d2.qty2)/count(distinct d.idCadena) as '% Exh',
			sum(d3.qty1) as Fac,
			sum(d3.qty1/d.qty)/count(distinct d.idCadena) as '% Fac',
	...
	--Datos
	if(@NumeroDePagina>0)
		select idEquipo, Cadena, q,q2,q3,q4,q5,q6,q7,q8,q9 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select idEquipo, Cadena, q,q2,q3,q4,q5,q6,q7,q8,q9 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select idEquipo, Cadena, q,q2,q3,q4,q5,q6,q7,q8,q9 from #datosFinal
end
