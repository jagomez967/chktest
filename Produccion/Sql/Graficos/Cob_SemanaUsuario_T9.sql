IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_SemanaUsuario_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_SemanaUsuario_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Cob_SemanaUsuario_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #Relevados (idUsuario, idPuntoDeVenta, Fecha, semana, qty)
	select	r.IdUsuario
			,r.idPuntoDeVenta
	...
	group by r.IdUsuario, r.idPuntoDeVenta,'W'+convert(varchar,datepart(wk,FechaCreacion)),'col'+convert(varchar,datepart(wk,FechaCreacion))
	...
	insert #datosRes (qty, idUsuario, idPuntoDeVenta, semana)
	select qty, idUsuario, idPuntoDeVenta, semana from #Relevados
	...
	set @ColDef='idusuario int, idPuntoDeVenta int'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idusuario], [idPuntoDeVenta],'+@PivotColumnHeaders+')
	SELECT [idusuario], [idPuntoDeVenta],'+@PivotColumnHeaders+'
	  FROM (
		Select idusuario, idPuntoDeVenta, semana, qty from #DatosRes
		) AS PivotData
	  PIVOT (
		max(qty)
		FOR semana IN (
	...
	insert #DatosPivotFinal ([idusuario], [idPuntoDeVenta],'+@PivotColumnHeaders+')
	select [idusuario], [idPuntoDeVenta],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idPuntoDeVenta'+char(39)+','+char(39)+'PuntoDeVenta'+char(39)+', 150, 2)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select ltrim(rtrim(u.Apellido)) as Usuario, ltrim(rtrim(pdv.Nombre)) as PuntoDeVenta,'+@PivotColumnHeaders+'
