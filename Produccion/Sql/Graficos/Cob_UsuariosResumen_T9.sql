IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_UsuariosResumen_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_UsuariosResumen_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_UsuariosResumen_T9]  
(  
 @IdCliente   int  
...
*/    
 ...
 if(@lenguaje = 'en')  
  ...
  values-- (0,1,'idUsuario','IdUsuario',0),   
    ...
    (0,1,'faltantes','Missing',10)  
  
 select esclave, mostrar, name, title, width from #columnasConfiguracion  
  
 --Datos  
 if(@NumeroDePagina>0)  
  select idUsuario, usuarionombre collate database_default as usuarionombre, asignados, relevados, (Asignados-relevados) as faltantes from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina) order by usuarionombre  
   
 if(@NumeroDePagina=0)  
  select idUsuario, usuarionombre collate database_default as usuarionombre, asignados, relevados, (Asignados-relevados) as faltantes  from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina) order by usuarionombre  
    
 if(@NumeroDePagina<0)  
  select idUsuario, usuarionombre collate database_default as usuarionombre, asignados, relevados,  (Asignados-relevados) as faltantes  from #datosFinal order by usuarionombre  
end  
go