IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_Usuarios_Porcentaje_T1'))
   exec('CREATE PROCEDURE [dbo].[Cob_Usuarios_Porcentaje_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cob_Usuarios_Porcentaje_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('usuario','Usuario'),('asignados','Asignados'),('relevados','Relevados'),('p_relevados','% Relevados')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('usuario','User'),('asignados','Assigned'),('relevados','Visited'),('p_relevados','% Visited')

	select name, title from #columnasConfiguracion

	select u.Apellido+', '+u.Nombre collate database_default as usuario
	,COUNT(distinct a.idpuntodeventa) as asignados
	,isnull(r.qty,0) as relevados,
	case COUNT(distinct a.idpuntodeventa)
	when 0 THEN 0
	ELSE
	(isnull(r.qty,0)*100.00 / COUNT(distinct a.idpuntodeventa)) * count(distinct a.idPuntoDeVenta) /100.0
	END as p_relevados
	from #asignados a
