IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_Usuarios_T9_Datos_Anidados'))
   exec('CREATE PROCEDURE [dbo].[Cob_Usuarios_T9_Datos_Anidados] AS BEGIN SET NOCOUNT ON; END')
GO --Cob_Usuarios_T9_Datos_Anidados 66
alter procedure [dbo].[Cob_Usuarios_T9_Datos_Anidados]
(
	@IdCliente			int
...
*/		
	...
	insert #Asignados (idPuntoDeVenta)
	Select distinct IdPuntoDeVenta from PuntoDeVenta_Cliente_Usuario where idPuntodeventa_Cliente_usuario in
	(
	...
	insert #Relevados (idPuntoDeVenta)
	...
	select ltrim(rtrim(pdv.nombre)), case when r.idpuntodeventa is null then '0' else 
	CONVERT (VARCHAR,(SELECT COUNT(DISTINCT idReporte) FROM Reporte WHERE idPuntoDeVenta = r.idPuntoDeVenta AND idusuario = @idusuario and convert(date,fechacreacion) between convert(Date, @fechadesde) and convert(date, @fechahasta)))
	end
	from #Asignados a
	inner join PuntoDeVenta pdv on pdv.idpuntodeventa = a.idpuntodeventa
	left join #Relevados r on r.idpuntodeventa = a.idpuntodeventa
	order by case when r.idpuntodeventa is null then '0' else 
	CONVERT (VARCHAR,(SELECT COUNT(DISTINCT idReporte) FROM Reporte WHERE idPuntoDeVenta = r.idPuntoDeVenta AND idusuario = @idusuario and convert(date,fechacreacion) between convert(Date, @fechadesde) and convert(date, @fechahasta)))
	end
