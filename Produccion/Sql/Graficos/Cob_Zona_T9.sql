IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cob_Zona_T9'))
   exec('CREATE PROCEDURE [dbo].[Cob_Zona_T9] AS BEGIN SET NOCOUNT ON; END')
GO 
alter PROCEDURE [dbo].[Cob_Zona_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #tempReporte (idCliente, idUsuario, IdPuntoDeVenta)
	select c.IdCliente, r.IdUsuario, r.IdPuntoDeVenta
	from reporte r
	...
	insert #datosFinal (Zona, qty)
	select z.Nombre, count(distinct t.IdPuntoDeVenta)
	from #tempReporte t
	...
	insert #datosFinal (Zona, qty)
	select 'Total PDV', sum(qty) from #datosFinal
	...
	--Datos
	if(@NumeroDePagina>0)
		select Zona, qty from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select Zona, qty from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select Zona, qty from #datosFinal
end
go