...
*/		
	...
	PIVOT
		(
			avg(cantMin) for dia in ([Lunes], [Martes], [Miércoles], [Jueves], [Viernes], [Sábado], [Domingo])
		) as piv
	...
	--Datos
	if(@NumeroDePagina>0)
		select pdv, usuario, lu, ma, mie, jue, vie, sa, dom from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select pdv, usuario, lu, ma, mie, jue, vie, sa, dom from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
