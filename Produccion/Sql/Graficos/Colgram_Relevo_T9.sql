IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Colgram_Relevo_T9'))
   exec('CREATE PROCEDURE [dbo].[Colgram_Relevo_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Colgram_Relevo_T9
(
	@IdCliente			int
...
*/		
	...
	select r.IdPuntoDeVenta, r.idUsuario, max(r.IdReporte), left(convert(varchar,r.fechacreacion,112),6)
	from Reporte r
	...
	insert #datos (idpdv, idreporte, valor1, valor2, iditem, preg, orden)
	select r.idPuntoDeVenta, r.idreporte, mi.LabelCampo1, mdr.valor4, mi.IdItem, mi.Nombre as preg, mi.orden from MD_ReporteModuloItem mdr
	...
	where isnull(mdr.valor1,0) > 0 and mi.Activo = 1
	...
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	where isnull(mdr.Valor2,0) > 0 and mi.Activo = 1
	order by r.idreporte
	...
	insert #datosRes (iditem, preg, idpdv, idreporte, valor)
	select orden, preg, idpdv, idreporte, valor1+' - '+valor2 collate database_default from #datos
	...
	set @ColDef='idpdv int, idreporte int'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idpdv],[idreporte],'+@PivotColumnHeaders+')
	SELECT [idpdv],[idreporte],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, idpdv, idreporte, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([idpdv],[idreporte],'+@PivotColumnHeaders+')
	select [idpdv], [idreporte],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'nested1'+char(39)+','+char(39)+'PuntoDeVenta'+char(39)+', 20, -1)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'nested2'+char(39)+','+char(39)+'Fecha'+char(39)+', 5, -3)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'nested3'+char(39)+','+char(39)+'Usuario'+char(39)+', 5, 0)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'nested4'+char(39)+','+char(39)+'Cadena'+char(39)+', 5, -2)

	insert #columnasDef (name, title, width, orden)
	select distinct cast(i.iditem as varchar) as name, i.preg as title, 20 as width, i.iditem as orden
	from #DatosRes i

	select name, title, width from #columnasDef order by orden, name

	if('+cast(@NumeroDePagina as varchar)+'>0)
			select convert(varchar(10),r.FechaCreacion,111) as nested2, ltrim(rtrim(c.Nombre)) as nested4, ltrim(rtrim(pdv.Nombre)) as nested1, u.Apellido+'', ''+u.Nombre collate database_default as nested3,'+@PivotColumnHeaders+'
