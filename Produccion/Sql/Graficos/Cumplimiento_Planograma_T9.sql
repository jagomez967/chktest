IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Cumplimiento_Planograma_T9'))
   exec('CREATE PROCEDURE [dbo].[Cumplimiento_Planograma_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Cumplimiento_Planograma_T9] 	
(
	@IdCliente			int
...
*/		
	...
	select	 r.fechacreacion
			...
			,count(p.IdProducto) 
			,abs(((sum(rp.NoTrabaja) * 100.0) / count(p.IdProducto)) - 100)
	from reporte r
	...
	order by abs(((sum(rp.NoTrabaja) * 100.0) / count(p.IdProducto)) - 100)
	...
	--Datos
	if(@NumeroDePagina>0)
		select fecha, pdv, direccion, zona, usuario, planograma, objetivo,'<p style="color:' + CASE WHEN cumplimiento between 0 and 80 then 'red' else 'green' end + ' ">' + convert(varchar,cumplimiento)+ ' %</p>' as cumplimiento 
...
exec Cumplimiento_Planograma_T9 @IdCliente=212,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
