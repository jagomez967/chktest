IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Davitel_Relevo_T9'))
   exec('CREATE PROCEDURE [dbo].[Davitel_Relevo_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Davitel_Relevo_T9
(
	@IdCliente			int
...
*/		
	...
	select r.IdPuntoDeVenta, r.idUsuario, r.IdReporte, convert(smalldatetime, r.fechaCreacion)
	from Reporte r
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo1+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg, r.fecha from MD_ReporteModuloItem mdr
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo2+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg, r.fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, NULL as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg, r.fecha 
	...
	insert #datosRes (iditem, idmarca, preg, pdv, usuario, valor, fecha)
	select orden, idmarca, preg, pdv, usuario, valor, 
	convert(varchar,fecha,3)+' '+convert(varchar,fecha,108) from #datos order by orden
	...
	set @ColDef='pdv varchar(max), usuario varchar(max), idmarca varchar(max), fecha varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([pdv],[usuario],[idmarca],[fecha],'+@PivotColumnHeaders+')
	SELECT [pdv],[usuario],[idmarca], [fecha],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, pdv, usuario, idmarca, fecha, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([pdv],[usuario],[idmarca], [fecha],'+@PivotColumnHeaders+')
	select [pdv],[usuario],[idmarca], [fecha],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Nombre'+char(39)+','+char(39)+'PDV'+char(39)+', 80, -6)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Usuario'+char(39)+','+char(39)+'Usuario'+char(39)+', 80, -5)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Marca'+char(39)+','+char(39)+'Marca'+char(39)+', 80, -4)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Fecha'+char(39)+','+char(39)+'Fecha'+char(39)+', 80, -3)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Descripcion'+char(39)+','+char(39)+'Descripcion'+char(39)+', 80, -2)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Observaciones'+char(39)+','+char(39)+'Observaciones'+char(39)+', 80, -1)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'CodigoEvento'+char(39)+','+char(39)+'CodigoEvento'+char(39)+', 80, 0)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre as Marca, d.Fecha as Fecha, cc.Descripcion as Descripcion, c.Observaciones as Observaciones, c.CodigoEvento as CodigoEvento,'+@PivotColumnHeaders+'
			...
			inner join Calendario c on c.idPuntoDeVenta = d.pdv and c.idUsuario = d.usuario and convert(date,c.FechaInicio) = d.fecha
			inner join CalendarioConceptos cc on cc.Id = c.ConceptoId
