IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.DirecTv_AudNoAutorizadas_T9'))
   exec('CREATE PROCEDURE [dbo].[DirecTv_AudNoAutorizadas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure dbo.DirecTv_AudNoAutorizadas_T9
(
	@IdCliente			int
...
*/		
	...
	select r.idReporte, 'col'+cast(i.IdItem as varchar)
	,case when mrep.Valor1=1 then case when isnull(mrep.valor4,'')='' then 'SI' else 'SI - ' end else case when isnull(mrep.valor4,'')='' then 'NO' else 'NO - ' end  end + mrep.Valor4
	,i.IdItem
	...
	select @maxpag
		...
		begin
			...
			set @PivotTableSQL = @PivotTableSQL + ' insert #columnasDef (name, title, width, orden) values ('+char(39)+'idReporte'+char(39)+','+char(39)+'Reporte'+char(39)+', 50, 1)
					...
					insert #columnasDef (name, title, width, orden) values ('+char(39)+'rtm'+char(39)+','+char(39)+'Usuario'+char(39)+', 50, 5)
					insert #columnasDef (name, title, width, orden) values ('+char(39)+'sap'+char(39)+','+char(39)+'SAP'+char(39)+', 50, 4)'
		end
		...
		begin
			...
			set @PivotTableSQL = @PivotTableSQL + ' insert #columnasDef (name, title, width, orden) values ('+char(39)+'idReporte'+char(39)+','+char(39)+'Report'+char(39)+', 50, 1)
					...
					insert #columnasDef (name, title, width, orden) values ('+char(39)+'rtm'+char(39)+','+char(39)+'User'+char(39)+', 50, 5)
					insert #columnasDef (name, title, width, orden) values ('+char(39)+'sap'+char(39)+','+char(39)+'SAP'+char(39)+', 50, 4)'
		end

		set @PivotTableSQL = @PivotTableSQL + '
		insert #columnasDef (name, title, width, orden)
		select '+char(39)+'col'+char(39)+'+cast(i.iditem as varchar) as name, i.nombre as title, 100 as width, 6 as orden
		from MD_Item i
		where IdModulo=104 and Activo=1
		...
		if('+cast(@NumeroDePagina as varchar)+'>0)
			select r.idReporte, r.FechaCreacion as fechaCreacion,ltrim(rtrim(pdv.nombre)) as pdv, pdv.CodigoSAP, u.Apellido+'+char(39)+', '+char(39)+'+u.Nombre collate database_default as rtm,'+@PivotColumnHeaders+'
	...
	--exec DirecTv_AudNoAutorizadas_T9 11, 'M,00010101,00010101',null,null,null,null,null,null,null,-1,null,'modDatos'
end