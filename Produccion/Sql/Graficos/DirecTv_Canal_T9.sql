IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.DirecTv_Canal_T9'))
   exec('CREATE PROCEDURE [dbo].[DirecTv_Canal_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure dbo.DirecTv_Canal_T9
(
	@IdCliente			int
...
*/		
	...
	select r.idReporte, 'col'+cast(i.IdItem as varchar), case when mrep.Valor1=1 then 'SI' else 'NO' end, i.IdItem
	...
	DECLARE @ColumnsWithIsNull VARCHAR(MAX)
	SELECT @ColumnsWithIsNull = 
	  COALESCE(
		@ColumnsWithIsNull + ',isnull(' + cast(iditem as varchar(500)) + ','+char(39)+'NO'+char(39)+ ')','isnull(' + cast(iditem as varchar(500)) + ','+char(39)+'NO'+CHAR(39)+')'
	  )
	FROM (select distinct iditem from #Datos) x order by iditem
	...
	select @maxpag
		...
		from MD_Item i
		where IdModulo=99 and Activo=1
	...
	--exec DirecTv_Canal_T9 11, 'M,00010101,00010101',null,null,null,null,null,null,null,1,null,'modDatos'
end