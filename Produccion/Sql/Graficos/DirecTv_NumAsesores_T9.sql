IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.DirecTv_NumAsesores_T9'))
   exec('CREATE PROCEDURE [dbo].[DirecTv_NumAsesores_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure dbo.DirecTv_NumAsesores_T9
(
	@IdCliente			int
...
*/		
	...
	select @maxpag
		...
		from MD_Item i
		where IdModulo=93 and Activo=1
