IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Dtv_Relevo_T9'))
   exec('CREATE PROCEDURE [dbo].[Dtv_Relevo_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Dtv_Relevo_T9
(
	@IdCliente			int
...
*/		
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo1+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg from MD_ReporteModuloItem mdr
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo2+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (pdv, usuario, valor, iditem, idmarca, preg, orden)
	select r.IdPuntoDeVenta, r.usuario, NULL as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datosRes (iditem, idmarca, preg, pdv, usuario, valor)
	select orden, idmarca, preg, pdv, usuario, valor from #datos order by orden
	...
	set @ColDef='pdv varchar(max), usuario varchar(max), idmarca varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([pdv],[usuario],[idmarca],'+@PivotColumnHeaders+')
	SELECT [pdv],[usuario],[idmarca],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, pdv, usuario, idmarca, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([pdv],[usuario],[idmarca],'+@PivotColumnHeaders+')
	select [pdv],[usuario],[idmarca],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idPuntoDeVenta'+char(39)+','+char(39)+'idPDV'+char(39)+', 80, -3)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.idPuntoDeVenta,pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre as Marca,'+@PivotColumnHeaders+'
		...
		if('+cast(@NumeroDePagina as varchar)+'<0)
			select pdv.idPuntoDeVenta,pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre,'+@PivotColumnHeaders+'
			from #DatosPivotFinal d
			inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = d.pdv
			inner join Usuario u on u.idUsuario = d.usuario
			inner join Marca m on m.idMarca = d.idMarca
	...
	--exec Dtv_Relevo_T9 8,'M,20160401,20160730'
end