IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EPSONShaISC_T11'))
   exec('CREATE PROCEDURE [dbo].[EPSONShaISC_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EPSONShaISC_T11] 	
(
	@IdCliente			int
...
*/		
	...
	declare @marcaGrafico int = 2992 -- ISC

	insert #tempProducto (idMarca,idProducto,cantidad)
	select	p.idMarca,rp.idProducto, rp.cantidad
	from reporte r
	...
	where	c.idCliente = @idcliente
			...
			and (isnull(@cEquipo,0) = 0 or exists(select 1 from puntodeventa_Vendedor pve2 where pve2.idPuntodeventa = pdv.idPuntodeventa  and pve2.idVendedor
				in(select idVendedor from vendedor where idEquipo in(select idEquipo from #Equipo))))
			and p.idProducto in(select idProductoCompetencia from productoCompetencia 
								where idProducto in (select idProducto from producto where idMarca = @marcaGrafico))
	
	select m.idMarca, m.nombre , sum(isnull(t.cantidad,0)),null as scolor,1 showVal
	from #tempProducto t
	inner join marca m on m.idMarca = t.idMarca 
	group by m.idMarca,m.nombre 
	
	select m.idMarca, m.nombre,p.idProducto,p.nombre, sum(isnull(t.cantidad,0)),1 showVal
	from #tempProducto t
	inner join marca m on m.idMarca = t.idMarca 
	inner join producto p on p.idProducto = t.idProducto
	group by m.idMarca,m.nombre,p.idProducto,p.nombre 
