IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_Cob_Mensual_T5'))
   exec('CREATE PROCEDURE [dbo].[Edding_Cob_Mensual_T5] AS BEGIN SET NOCOUNT ON; END')
Go--[edding_Cob_Mensual_T5] 127
alter procedure [dbo].[Edding_Cob_Mensual_T5] 	
(
	@IdCliente			int
...
*/		
	...
	begin
		...
		insert #tempPCU_final(idusuario,direccion,id)
		select tmp.idUsuario,pdv.direccion,tmp.id from #tempPCU tmp
	...
	select mes, count(distinct direccion) from #tempReporte_final group by mes
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('relevados','Tendencia')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('relevados','Trend')

	select name, title from #columnasConfiguracion

	select isnull(V.qty,0) as relevados from #Asignados A left join #Relevados V on V.mes=A.mes
end
go