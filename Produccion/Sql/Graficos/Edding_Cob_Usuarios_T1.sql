IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_Cob_Usuarios_T1'))
   exec('CREATE PROCEDURE [dbo].[Edding_Cob_Usuarios_T1] AS BEGIN SET NOCOUNT ON; END')
GO--Edding_Cob_Usuarios_T1  127
alter procedure [dbo].[Edding_Cob_Usuarios_T1] 	
(
	@IdCliente			int
...
*/		
	...
	select idUsuario, count(distinct direccion) from #tempReporte_final group by idUsuario
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('usuario','Usuario'),('asignados','Asignados'),('relevados','Relevados')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('usuario','User'),('asignados','Assigned'),('relevados','Visited')

	select name, title from #columnasConfiguracion

	select u.Apellido+', '+u.Nombre collate database_default as usuario
	,COUNT(distinct a.direccion) as asignados
	,isnull(r.qty,0) as relevados
	from #asignados a
	left join #Relevados r on r.idUsuario=a.idUsuario
	inner join Usuario u on u.IdUsuario=a.idUsuario
	group by  u.Apellido+', '+u.Nombre collate database_default, r.qty
	order by  u.Apellido+', '+u.Nombre collate database_default
