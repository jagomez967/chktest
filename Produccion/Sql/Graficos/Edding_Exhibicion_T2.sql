IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_Exhibicion_T2'))
   exec('CREATE PROCEDURE [dbo].[Edding_Exhibicion_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Edding_Exhibicion_T2]
(
	@IdCliente			int
...
*/		
	...
	select m.idMarca, tmp.mes, COUNT(isnull(rp.Valor1,0))
	from Reporte r
	inner join MD_ReporteModuloItem rp on rp.IdReporte = r.IdReporte
	inner join MD_Item mi on mi.idItem = rp.idItem
	...
	where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = rp.IdMarca))
			and isnull(rp.Valor1,0) > 0
			and mi.Nombre like '%Exhibe%'
	group by m.idMarca, tmp.mes	
	...
	inner join Marca m on m.idMarca=te.idMarca

	select LTRIM(rtrim(m.nombre)),round(SUM(isnull(d.qty,0))*100/@total,1)
	from #MarcasMeses te
	left join #datos d on d.idMarca=te.idMarca and d.mes=te.mes
	inner join Marca m on m.idMarca=te.idMarca
	group by rtrim(m.nombre), te.idMarca
	order by te.idMarca
end