IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_InventarioProdMes_T9'))
   exec('CREATE PROCEDURE [dbo].[Edding_InventarioProdMes_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Edding_InventarioProdMes_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datosRes (idPuntoDeVenta, idPop, idUsuario, qty, mesString, mes)
	...
	group by t.idPuntoDeVenta, rp.idPop, t.idUsuario, t.mesString, t.mes
	...
	set @ColDef='idPuntoDeVenta int, idUsuario int, idPop int'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idPuntoDeVenta], [idUsuario], [idPop],'+@PivotColumnHeaders+')
	SELECT [idPuntoDeVenta], [idUsuario], [idPop],'+@PivotColumnHeaders+'
	  FROM (
		Select idPuntoDeVenta, idUsuario, idPop, mes, qty from #DatosRes
		) AS PivotData
	  PIVOT (
		max(qty)
		FOR mes IN (
	...
	insert #DatosPivotFinal ([idPuntoDeVenta], [idUsuario], [idPop],'+@PivotColumnHeaders+')
	select [idPuntoDeVenta], [idUsuario], [idPop],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idUsuario'+char(39)+','+char(39)+'Usuario'+char(39)+', 150, 2)
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idPop'+char(39)+','+char(39)+'Producto'+char(39)+', 150, 3)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select ltrim(rtrim(pdv.Nombre)) as idPuntoDeVenta, u.Apellido+'', ''+u.Nombre collate database_default as idUsuario, ltrim(rtrim(p.Nombre)) as idPop,'+@PivotColumnHeaders+'
