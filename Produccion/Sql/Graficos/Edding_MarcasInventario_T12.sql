IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_MarcasInventario_T12'))
   exec('CREATE PROCEDURE [dbo].[Edding_MarcasInventario_T12] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Edding_MarcasInventario_T12]
(
	@IdCliente			int
...
*/		
	...
	insert #datos(idEmpresa, idMarca, valor)
	select r.idEmpresa, rp.idMarca, count(isnull(rp.idPop,0))
	...
	group by r.idEmpresa, rp.idMarca


	insert #datosPeriodoAnterior(idEmpresa, idMarca, valor)
	select r.idEmpresa, rp.idMarca, count(isnull(rp.idPop,0))
	...
	create table #MarcaPeriodoAnterior
	(
		idEmpresa int,
		idMarca int,
		valor decimal(10,2),
	...
	create table #PopPorMarca
	(
		idMarca int,
		cant int
	)
	insert #PopPorMarca (idMarca, cant)
	select p.idMarca, count (distinct p.idPop) from Pop_Marca p
	...
	insert #MarcaActual (idEmpresa, idMarca, valor, varianza)
	select d.idEmpresa, d.idmarca, d.valor*100.0/(select cant from #PopPorMarca where #PopPorMarca.idMarca=d.idMarca), 0
	from #datos d
	inner join cliente c on c.IdEmpresa=d.idempresa
	where c.IdCliente=@IdCliente

	insert #MarcaPeriodoAnterior (idEmpresa, idMarca, valor, varianza)
	select d.idEmpresa, d.idmarca, d.valor*100.0/(select cant from #PopPorMarca where #PopPorMarca.idMarca=d.idMarca), 0
	from #datosPeriodoAnterior d
	inner join cliente c on c.IdEmpresa=d.idempresa
	where c.IdCliente=@IdCliente

	update #MarcaActual set valor = valor / (select count(distinct idPuntoDeVenta) from #reportesMesPdv)
	update #MarcaPeriodoAnterior set valor = valor / (select count(distinct idPuntoDeVenta) from #reportesMesPdvPeriodoAnterior)

	update A
	set A.varianza = A.valor - B.valor
	from #MarcaActual A, #MarcaPeriodoAnterior B
	where A.idEmpresa = B.idEmpresa and A.idMarca = B.idMarca


	select d.id as id, '7EA568' as color, NULL as logo, 0 as nivel, d.valor as valor, isnull(d.varianza,0) as varianza, 0 as parentId,ltrim(rtrim(m.Nombre)) as nombre,m.idMarca as idMarca
	from #MarcaActual d
