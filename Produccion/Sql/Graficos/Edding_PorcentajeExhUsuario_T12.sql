IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_PorcentajeExhUsuario_T12'))
   exec('CREATE PROCEDURE [dbo].[Edding_PorcentajeExhUsuario_T12] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Edding_PorcentajeExhUsuario_T12]
(
	@IdCliente			int
...
*/		
	...
	insert #reportesMesPdv (idUsuario, idPuntoDeVenta, mes, idReporte)
	select r.idUsuario, r.idpuntodeventa, left(convert(varchar,r.fechacreacion,112),6), max(r.idreporte)
	from Reporte r
	...
	group by r.idUsuario, r.idpuntodeventa, left(convert(varchar,r.fechacreacion,112),6)

	insert #reportesMesPdvPeriodoAnterior (idUsuario, idPuntoDeVenta, mes, idReporte)
	select r.idUsuario, r.idpuntodeventa, left(convert(varchar,r.fechacreacion,112),6), max(r.idreporte)
	from Reporte r
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta=r.IdPuntoDeVenta
	inner join Cliente c on c.IdEmpresa=r.IdEmpresa
	...
	select idUsuario, count(distinct idPuntoDeVenta) from #reportesMesPdv group by idUsuario


	create table #RelevadosAnterior
	(
		idUsuario int,
		qty int
	)

	insert #RelevadosAnterior (idUsuario, qty)
	select idUsuario, count(distinct idPuntoDeVenta) from #reportesMesPdvPeriodoAnterior group by idUsuario

	declare @CantProductos int
	select @CantProductos = count(distinct idProducto) from Producto p
	...
	insert #datos(idUsuario, valor)
	select r.idUsuario, sum(isnull(rp.Stock,0))*100.0/@CantProductos
	...
	insert #datosPeriodoAnterior(idUsuario, valor)
	select r.idUsuario, sum(isnull(rp.Stock,0))
	...
	update A
	set A.valor = A.valor/B.qty
	from #datosPeriodoAnterior A, #RelevadosAnterior B
	where A.idUsuario = B.idUsuario
