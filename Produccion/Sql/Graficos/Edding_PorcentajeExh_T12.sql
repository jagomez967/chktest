IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_PorcentajeExh_T12'))
   exec('CREATE PROCEDURE [dbo].[Edding_PorcentajeExh_T12] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Edding_PorcentajeExh_T12]
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, p.idMarca, sum(isnull(rp.Stock,0))
	...
	group by r.idEmpresa, p.idMarca
	...
	select d.idEmpresa, d.idmarca, d.valor*100.0/(select cant from #ProductosPorMarca where #ProductosPorMarca.idMarca=d.idMarca), 0
	from #datos d
