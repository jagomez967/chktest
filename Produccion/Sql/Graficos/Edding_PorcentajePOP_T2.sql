IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_PorcentajePOP_T2'))
   exec('CREATE PROCEDURE [dbo].[Edding_PorcentajePOP_T2] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].Edding_PorcentajePOP_T2
(
	@IdCliente			int
...
*/		
	...
	insert #datosRes (pop, qty)
	select mp.Nombre, count(rmp.idMarcaPropiedad) from #reportesMesPdv t
