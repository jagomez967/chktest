IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_PreciosCompetenciaCO_T9'))
   exec('CREATE PROCEDURE [dbo].[Edding_PreciosCompetenciaCO_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Edding_PreciosCompetenciaCO_T9
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (fecha, pdv, direccion, usuario, producto, valor, cantidad, cantidad2)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Fecha',5),('pdv','PuntoDeVenta',5),('direccion','Direccion',5),('usuario','Usuario',5),('producto','Producto',5),('valor','Precio Mayorista',5),('cantidad','Precio Minorista',5),('cantidad2','Precio Distribuidor',5)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Date',5),('pdv','PointOfSale',5),('direccion','Address',5),('usuario','User',5),('producto','Product',5),('valor','Precio Mayorista',5),('cantidad','Precio Minorista',5),('cantidad2','Precio Distribuidor',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select fecha, pdv, direccion, usuario, producto, valor, cantidad, cantidad2 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select fecha, pdv, direccion, usuario, producto, valor, cantidad, cantidad2 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select fecha, pdv, direccion, usuario, producto, valor, cantidad, cantidad2 from #datosFinal
