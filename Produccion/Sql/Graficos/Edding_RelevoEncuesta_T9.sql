IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_RelevoEncuesta_T9'))
   exec('CREATE PROCEDURE [dbo].[Edding_RelevoEncuesta_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Edding_RelevoEncuesta_T9
(
	@IdCliente			int
...
*/		
	...
	insert #datos (pdv, usuario, valor, iditem, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo1+' - '+mdr.Valor4 as valor, mi.IdItem, mi.Nombre, mi.Orden as preg, r.Fecha from MD_ReporteModuloItem mdr
	...
	insert #datos (pdv, usuario, valor, iditem, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, mi.LabelCampo2+' - '+mdr.Valor4 as valor, mi.IdItem, mi.Nombre, mi.Orden as preg, r.Fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (pdv, usuario, valor, iditem, preg, orden, fecha)
	select r.IdPuntoDeVenta, r.usuario, NULL as valor, mi.IdItem, mi.Nombre, mi.Orden as preg, r.Fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datosRes (iditem, preg, pdv, usuario, valor, fecha)
	select orden, preg, pdv, usuario, valor, fecha from #datos order by orden
	...
	set @ColDef='pdv varchar(max), usuario varchar(max), fecha datetime'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([pdv],[usuario], [fecha],'+@PivotColumnHeaders+')
	SELECT [pdv],[usuario], [fecha],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, pdv, usuario, fecha, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([pdv],[usuario], [fecha],'+@PivotColumnHeaders+')
	select [pdv],[usuario], [fecha],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, d.fecha as Fecha,'+@PivotColumnHeaders+'
