IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Edding_TipoIntervencion_T9'))
   exec('CREATE PROCEDURE [dbo].[Edding_TipoIntervencion_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Edding_TipoIntervencion_T9
(
	@IdCliente			int
...
*/		
	...
	where isnull(mdr.valor1,0) > 0 and mm.idModulo = 501
	...
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	where isnull(mdr.Valor2,0) > 0 and mm.idModulo = 501
