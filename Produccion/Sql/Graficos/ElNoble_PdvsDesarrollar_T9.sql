IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ElNoble_PdvsDesarrollar_T9'))
   exec('CREATE PROCEDURE [dbo].[ElNoble_PdvsDesarrollar_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[ElNoble_PdvsDesarrollar_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #Resultados (pdv, performance)
	select cast(d.pdv as varchar)+' - '+pdv.Nombre, sum(performance) from #datosFinal d
	inner join PuntoDeVenta pdv on pdv.idpuntodeventa = d.pdv
	group by cast(d.pdv as varchar)+' - '+pdv.Nombre
	having sum(performance) between 50 and 75
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('pdv','PuntoDeVenta',50),('performance','PERFORMANCE',30)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('pdv','Point Of Sale',50),('performance','PERFORMANCE',30)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select pdv, performance from #Resultados where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select pdv, performance from #Resultados where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select pdv, performance from #Resultados
end