IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ElNoble_PerfGeneral_T12'))
   exec('CREATE PROCEDURE [dbo].[ElNoble_PerfGeneral_T12] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[ElNoble_PerfGeneral_T12]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinalAnterior (pdv, idusuario, idmarca, performance, mes)
	select d.idPuntoDeVenta, d.idusuario, d.idmarca,
			case when d.idmarca = 375 then (sum(valor)*@pondIE)*@pondIMG
			when d.idmarca = 376 then (sum(valor)*@pondII)*@pondIMG
			when d.idmarca = 377 then sum(valor)*@pondOYL
	...
	insert #Resultados (idmarca, valor)
	select idmarca, sum(performance) from #datosFinal
	group by idmarca


	create table #ResultadosAnterior
	(
		idmarca int,
		valor decimal(18,2)
	)

	insert #ResultadosAnterior
	select idmarca, sum(performance) from #datosFinalAnterior
	group by idmarca
	...
	insert #varianza (valor, varianza)
	select sum(isnull(valor,0)), 0 from #Resultados

	insert #varianzaAnterior (valor, varianza)
	select sum(isnull(valor,0)), 0 from #ResultadosAnterior

	update A
	set A.varianza = A.valor - B.valor
	from #varianza A, #varianzaAnterior B
