...
*/		
	...
	--Datos
	if(@NumeroDePagina>0)
		select idpdv, pdv, direccion, region, auditor, fecha from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select idpdv, pdv, direccion, region, auditor, fecha from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
