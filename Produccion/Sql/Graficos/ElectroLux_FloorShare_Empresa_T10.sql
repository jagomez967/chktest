IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ElectroLux_FloorShare_Empresa_T10'))
   exec('CREATE PROCEDURE [dbo].[ElectroLux_FloorShare_Empresa_T10] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].ElectroLux_FloorShare_Empresa_T10
(
	@IdCliente			INT
...
*/		
	...
	SELECT 
		DENSE_RANK() OVER (ORDER BY Empresa) Id,
		Empresa,
		RIGHT('00'+ CONVERT(VARCHAR,MCreacion),2)+'-'+ CONVERT(VARCHAR,Ycreacion) mesStr,
		RIGHT('00'+ CONVERT(VARCHAR,MCreacion),2)+'-'+ CONVERT(VARCHAR,Ycreacion) mes,
	...
	FROM 
		#RawQty RQ
	GROUP BY Empresa,MCreacion,Ycreacion
...
--exec ElectroLux_FloorShare_Empresa_T10 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'