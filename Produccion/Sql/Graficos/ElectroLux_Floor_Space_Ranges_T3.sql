IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.ElectroLux_Floor_Space_Ranges_T3'))
   exec('CREATE PROCEDURE [dbo].[ElectroLux_Floor_Space_Ranges_T3] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[ElectroLux_Floor_Space_Ranges_T3]
(
	@IdCliente			int
	...
	(
		...
		GROUP BY
			...
			WHERE 
				...
				R.idEmpresa = 1165 and m.idmarca = 3092 and pop.idpop in (2616,2617)
...
exec ElectroLux_Floor_Space_Ranges_T3 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
