IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Electrolux_FloorSpace_Freezer_T3'))
   exec('CREATE PROCEDURE [dbo].[Electrolux_FloorSpace_Freezer_T3] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Electrolux_FloorSpace_Freezer_T3]
(
	@IdCliente			int
...
*/		
	...
	SELECT mesStr,GrupoPop,qty,Color,Texto,isnull(Orden,999999) as orden FROM #GpopVacio
	UNION
	SELECT 
