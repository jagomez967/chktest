IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Encuesta_BcoChile2015_T9'))
   exec('CREATE PROCEDURE [dbo].[Encuesta_BcoChile2015_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Encuesta_BcoChile2015_T9
(
	@IdCliente			int
...
*/		
	...
	where isnull(mdr.valor1,0) > 0
		and mm.idModulo in (1673,1674,1675,1676)
	...
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
		where mm.idModulo in (1673,1674,1675,1676)
