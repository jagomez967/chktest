IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Encuesta_Completa_T9'))
   exec('CREATE PROCEDURE [dbo].[Encuesta_Completa_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Encuesta_Completa_T9
(
	@IdCliente			int
...
*/		
	...
	select r.IdPuntoDeVenta, r.idUsuario, r.IdReporte,r.fechacreacion
	from Reporte r
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo1+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha from MD_ReporteModuloItem mdr
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo2+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo3+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, NULL as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha 
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'direccion'+char(39)+','+char(39)+'Direccion'+char(39)+', 80, -3)
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select d.idReporte as idReporte, pdv.Nombre, pdv.direccion as direccion,l.nombre Localidad,u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre as Marca,'+@PivotColumnHeaders+',Convert(varchar(10),CONVERT(date,d.fecha,106),103) as fecha
...
--
--exec Encuesta_Completa_T9 @IdCliente=201,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
