IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonComparacionPreciosCISS_T21'))
   exec('CREATE PROCEDURE [dbo].[EpsonComparacionPreciosCISS_T21] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EpsonComparacionPreciosCISS_T21] 	
(
	@IdCliente			int
...
*/		
	...
	insert #data(valor,producto,marca,colorMarca,idProducto,idCadena)			
	select rp.precio,p.nombre,ltrim(rtrim(str(m.idSubMarca))) + ' - ' + m.nombre,m.MarcaColor,p.idProducto,pdv.idCadena From reporte r
	inner join reporteProducto rp on rp.idReporte = r.idReporte
	inner join producto p on p.idProducto = rp.idProducto
	inner join SubMarca_Producto smp on smp.idProducto = p.IdProducto
	inner join SubMarca m on m.idMarca = p.idMarca and m.idSubMarca = smp.idSubMarca
	...
	where	convert(date,r.FechaCreacion) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
			...
			and p.idfamilia in(select idFamilia from familia where nombre = 'CISS')
	and rp.precio <> 0	
--	group by p.nombre,ltrim(rtrim(str(m.idMarcaEpson))) + ' - ' + m.nombre,m.MarcaColor,p.idProducto,pdv.idCadena
	...
	select cast(max(valor) as decimal(18,2)) as valor,producto,marca,colorMarca from #data group by producto,marca,colorMarca
	order by marca
	---------------------------------------------------------------------------------------------------------------------------------
end
GO
