IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonComparacionPreciosIC_T21'))
   exec('CREATE PROCEDURE [dbo].[EpsonComparacionPreciosIC_T21] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EpsonComparacionPreciosIC_T21] 	
(
	@IdCliente			int
...
*/		
	...
	insert #data(valor,idProducto,producto,marca,colorMarca,idCadena)			
	select rp.precio,p.idProducto,p.nombre,ltrim(rtrim(str(m.idSubMarca))) + ' - ' + m.nombre,m.MarcaColor,pdv.idCadena From reporte r
	inner join reporteProducto rp on rp.idReporte = r.idReporte
	inner join producto p on p.idProducto = rp.idProducto
	inner join submarca_producto smp on smp.idProducto =rp.idProducto
	inner join SubMarca m on p.idMarca = m.idMarca and smp.idSubMarca = m.idSubMarca
	...
	where	convert(date,r.FechaCreacion) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
			...
			and p.idfamilia in(select idFamilia from familia where nombre = 'IC')
	and rp.precio <> 0	
	...
	select max(valor) as valor,producto,marca,colorMarca from #data group by producto,marca,colorMarca
	order by marca
