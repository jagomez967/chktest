IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonCompetimeter_T24'))
   EXEC('CREATE PROCEDURE [dbo].[EpsonCompetimeter_T24] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[EpsonCompetimeter_T24]
(
	@IdCliente			int
...
begin
	...
	BEGIN  
		...
		--FIN MODA	
		if exists (select 1 from #Familia where idFamilia in( select idFamilia from familia where nombre = 'CISS HW')) --CISS
		BEGIN
			select @idProductoPropio = x.idProducto
			from
				(select idProducto,count(*) as cantidad_repeticiones,rank() over(order by count(*) desc) as rank
				from #data
				where idProducto in(select idProducto
									from producto where nombre in
									('EPSON L395','EPSON L4150')
									and idMarca in(
									select idMarca from marca where idEmpresa in(
									select idEmpresa from cliente where idCliente = @idCliente_actual))
									)  --LEVANTO EL PRODUCTO CON MAYOR CANTIDAD DE VALORES
				group by idProducto)x
			where x.rank = 1		
			if @@ROWCOUNT = 0
			BEGIN
						select @idProductoPropio = idProducto
									from producto where nombre in
									('EPSON L395')--,'EPSON L4150')
									and idMarca in(
									select idMarca from marca where idEmpresa in(
									select idEmpresa from cliente where idCliente = @idCliente_actual))
						select @idProductoPropio = isnull(@idProductoPropio,idProducto)
									from producto where nombre in
									('EPSON L4150')
									and idMarca in(
									select idMarca from marca where idEmpresa in(
			...
			select @idProductoCompetencia = idProducto
									from producto where nombre in
									('HP 415')
									and idMarca in(
									select idMarca from marca where idEmpresa in(
		...
		select @idProductoCompetencia = isnull(@idProductoCompetencia,idProducto) 
									from producto where nombre in
									('CANON G3100')
									and idMarca in(
									select idMarca from marca where idEmpresa in(
		...
		END
		ELSE	
		if exists (select 1 from #Familia where idFamilia in(select idFamilia from familia where nombre = 'IC')) --IC
		BEGIN				
			select @idProductoPropio = idProducto
									from producto where nombre in
									('EPSON XP241')
	...
	if (@idCLiente = 196)
	BEGIN
		if (isnull(@idProductoPropio,0) = 0) set @idProductoPropio = 14529
		if (isnull(@idProductoCompetencia,0) = 0) set @idProductoCompetencia = 14343
	END
		 


		select @propio = UPPER(nombre)
		from producto where idProducto = @idProductoPropio
		
		select @competencia = UPPER(nombre)
		from producto where idProducto = @idProductoCompetencia   


		select @precioPropio = max(precio) 
		from #preciosPorProducto 
		where idProducto = @idProductoPropio

		select @precioCompetencia = max(precio)
		from #preciosPorProducto 
		where idProducto = @idProductoCompetencia 	
		
--		BEGIN TRY
			select @valor = (((@precioPropio - @precioCompetencia   )/@precioPropio * 1.0)*100.0)
--		END TRY
--		BEGIN CATCH
--			select @valor = 0
--		END CATCH
