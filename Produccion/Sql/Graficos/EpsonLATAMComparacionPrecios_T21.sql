alter procedure [dbo].[EpsonLATAMComparacionPrecios_T21]
(
	@IdCliente			int
...
*/		
	...
	select rp.precio/cur.value,p.idProducto,p.nombre,ltrim(rtrim(str(m.idSubMarca/100))) + ' - ' + m.nombre,m.MarcaColor,pdv.idCadena,m.nombre
	From reporte r
	...
	select cast(avg(p.precio) as decimal(18,2)) as valor, 
	upper(d.producto) as producto ,d.marca,
	d.colorMarca,d.marcaEpson as drilldown
	from #data d
	inner join #preciosPorProducto p
	on p.idProducto = d.idProducto 
	group by d.producto ,d.marca,d.colorMarca,d.marcaEpson
	order by producto 

	select distinct ltrim(rtrim(str(m.idSubMarca/100))) + ' - ' + m.nombre as marca,
	m.imagenMarca 
	from SubMarca m
	where exists 
...
exec EpsonLATAMComparacionPrecios_T21 @IdCliente=194,@Filtros=@p2,@NumeroDePagina=0,@Lenguaje='es',
