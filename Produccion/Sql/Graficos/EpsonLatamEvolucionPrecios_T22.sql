IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonLatamEvolucionPrecios_T22'))
   exec('CREATE PROCEDURE [dbo].[EpsonLatamEvolucionPrecios_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[EpsonLatamEvolucionPrecios_T22]
(
	@IdCliente			int
...
*/		
	...
	from rangoPrecios where idCliente = (select min(idCliente) from #clientes)
	...
	and idCliente =(select min(idCliente) from #clientes)
	
	insert #configuracion(xTitulo,yTitulo)
	values('Producto','Precio $')
	
	
	insert #data(idPRoducto,producto,dashStyle,color,fecha,valorFecha,valor,linkedTo,marca,idCadena,idCliente)			
	...
	insert #preciosPorProducto(idProducto,precio,cantidad,fecha,idCliente)
	select idProducto,valor,count(valor),fecha,idCliente
	from  #data
	group by idProducto,valor,fecha,idCliente
	
	delete p 
	from #preciosPorProducto p
	where exists(select 1 from #preciosPorProducto where idproducto = p.idProducto and cantidad > p.cantidad and  p.fecha = fecha and p.idCliente = idCliente)

	delete p 
	from #preciosPorProducto p
	where exists(select 1 from #preciosPorProducto where idproducto = p.idProducto and precio > p.precio and  p.fecha = fecha and p.idCliente = idCliente)

	create table #ProductosUnificados
	( id int identity(1,1),
	  producto varchar(max) ) 

	insert #ProductosUnificados(producto)
	select producto 
	from #data
	group by producto 

		select id,producto,fecha,valorFecha,avg(precio),color,dashStyle,linkedTo,Marca,orden From (
		select pu.id,upper(d.producto) as producto,d.fecha,d.valorFecha,
		p.precio,
		d.color,
		d.dashStyle,null as linkedTo,d.Marca,prod.orden from #data d
		...
		inner join #ProductosUnificados pu on pu.producto = d.producto collate database_default 
		group by pu.id,d.producto,d.fecha,d.valorFecha,d.color
		,d.dashStyle,d.Marca,prod.orden,p.precio) x
		group by id,producto,fecha,valorFecha,color,dashStyle,linkedTo,Marca,orden
		order by fecha,producto,id,orden
...
insert into @p2 values(N'fltFechaReporte',N'M,20180621,20180621')
exec EpsonLATAMEvolucionPrecios_T22 @IdCliente=194, @filtros = @p2
