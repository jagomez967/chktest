IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonRegionCP_T21'))
   exec('CREATE PROCEDURE [dbo].[EpsonRegionCP_T21] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[EpsonRegionCP_T21]
(
	@IdCliente			int
...
--	FIN MODA	
	...
	select avg(p.precio) as valor, 
	d.producto ,d.marca,d.colorMarca,d.marcaEpson as drilldown
	from #data d
	inner join #preciosPorProducto p
	on p.idProducto = d.idProducto 
	...
	select distinct cast(p.precio as int) as valor, 
...
exec [EpsonRegionCP_T21] @IdCliente=194,@Filtros=@p2,@NumeroDePagina=0,@Lenguaje='es',
