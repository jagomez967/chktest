IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonSelfShareDetalle_T9'))
   exec('CREATE PROCEDURE [dbo].[EpsonSelfShareDetalle_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EpsonSelfShareDetalle_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idProducto, mes, qty)
	select p.idProducto, tmp.mes, SUM(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0))
	from Reporte r
	...
	insert #datosFinal(idMarca,idProducto,cantidad)
	select 0,idProducto, qty 
	from #Datos
	order by idProducto
...
select 1
	...
	if(@lenguaje='es')
		...
		values ('marca','Marca',0,0,0,1),('producto','Producto',0,0,1,0),('cantidad','cantidad',0,0,1,0)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width,esClave,Mostrar,esAgrupador) 
		values ('marca','Brand',0,0,0,1),('producto','Product',0,0,1,0),('cantidad','qty',0,0,1,0)
			...
			select  distinct
			 cast(DENSE_RANK() OVER   
					(order by e.idSubMarca asc) as varchar(10))
					 +' - '+
					 e.nombre as marca, p.nombre as producto, d.cantidad as cantidad
		from #datosFinal d 
