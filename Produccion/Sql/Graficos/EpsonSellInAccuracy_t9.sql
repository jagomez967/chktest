IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonSellInAccuracy_t9'))
   exec('CREATE PROCEDURE [dbo].[EpsonSellInAccuracy_t9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EpsonSellInAccuracy_t9]
(
	@IdCliente			int
...
*/		
	...
	select 'Argentina',
		   ...
		   (ru.ventas * 100.0) / ru.objetivo,
		   (ru.Objetivo * 100.0) /	ru.objetivo
	...
	--		 ('cumplimientoPrecio','% Objetivo',10)
		...
		if(@lenguaje = 'en')
			...
			values 
			 ...
			 ('cumplimientoVolumen','Sales Forecast Accuracy',10)
...
--			 ('cumplimientoPrecio','% Sales in Mix Target',10)
	...
	BEGIN
		...
		set cumplimientoVolumen = (ventasVolumen*100.0)/ObjetivoVolumen * 1.0
			,cumplimientoPrecio = (ObjetivoVolumen * 100) /ObjetivoVolumen * 1.0
		...
		select '<b>TOTAL:</b>',null,sum(ventasVolumen),sum(objetivoVolumen),null,
				sum(objetivoVentas),sum(ventasVolumen)*100.0/sum(objetivoVolumen),avg(cumplimientoPrecio)
