IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonSellInVolume_T12'))
   exec('CREATE PROCEDURE [dbo].[EpsonSellInVolume_T12] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[EpsonSellInVolume_T12]
(
	@IdCliente			int
...
*/		
	...
	select ru.idProducto, ru.ventas,ru.objetivo,ru.idcadena,844 --EPSON ARG
	...
	--and convert(date,ru.StartDate) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
