IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonSellOutAccuracy_t9'))
   exec('CREATE PROCEDURE [dbo].[EpsonSellOutAccuracy_t9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[EpsonSellOutAccuracy_t9]
(
	@IdCliente			int
...
*/		
	...
	select 'Argentina',
		   ...
		   (dif.ventas * 100.0) / dif.Objetivo * 1.0,
...
--			 ('cumplimientoPrecio','% Sales in Mix Target',10)
	 ...
	 insert #datosFinal_Agrupados(pais,producto,ventasVolumen,
		...
		select '<b>TOTAL:</b>',null,sum(ventasVolumen),sum(objetivoVolumen),null,
				sum(objetivoVentas),sum(ventasVolumen)*100.0/sum(ObjetivoVolumen),sum(cumplimientoPrecio)
