IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EpsonTradeCapacitacion_T30'))
   exec('CREATE PROCEDURE [dbo].[EpsonTradeCapacitacion_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[EpsonTradeCapacitacion_T30]
(
	@IdCliente			INT,
...
BEGIN
	...
	end as [Realizo capacitación?],
	rc.Contacto as [Mail Contacto],
	rc.Telefono as [Vendedores Capacitados],
	rc.Fecha as fecha
	from reporte r 
...
insert into @p3 values(N'fltFechaReporte',N'D,20180901,20181030')
exec [EpsonTradeCapacitacion_T30] @IdCliente=222,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es',@IdUsuarioConsulta=19,@TamañoPagina=8
