...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_Calculo_Ponderacion'))
   exec('CREATE PROCEDURE [dbo].[Epson_Calculo_Ponderacion] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Epson_Calculo_Ponderacion] 	
as
begin
