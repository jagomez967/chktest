IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_ChannelMixConsPSI_T27'))
   exec('CREATE PROCEDURE [dbo].[Epson_ChannelMixConsPSI_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_ChannelMixConsPSI_T27]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT
		'CHANNEL MIX (CONS) Sell-in' SerieName, 
		CASE WHEN (SELECT COUNT(1) FROM #DATA WHERE Point = 'RET') = 0
		THEN
			CONVERT(VARCHAR,((SELECT value FROM #DATA WHERE Point = 'DIST') * 100) / (SELECT SUM(value) FROM #DATA))
		ELSE
			CONVERT(VARCHAR,((SELECT value FROM #DATA WHERE Point = 'RET') * 100) / (SELECT SUM(value) FROM #DATA))
		END
		+ '%' Total,
		CASE WHEN (SELECT COUNT(1) FROM #DATA WHERE Point = 'RET') = 0 THEN 'Dist' ELSE 'Retail' END Target,
		'8px' LegendFontSize
