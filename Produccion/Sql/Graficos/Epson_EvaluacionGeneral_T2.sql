IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_EvaluacionGeneral_T2'))
   exec('CREATE PROCEDURE [dbo].[Epson_EvaluacionGeneral_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Epson_EvaluacionGeneral_T2]
(
	@IdCliente			int
...
*/		
	...
	select @logrado = isnull(round(avg(qty),2),0)  
	from #datosFinal 
