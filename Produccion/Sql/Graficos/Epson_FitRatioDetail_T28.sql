IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_FitRatioDetail_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_FitRatioDetail_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_FitRatioDetail_T28]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 'FIT RATIO' SerieName, '' Perc, 1 Stack, 1 ShowText 
	...
	SELECT
		...
		CASE TC.Nombre
			WHEN 'Distribucion' THEN '#F05A23' 
			WHEN 'Retail' THEN '#79C843'
		END Color,
		0 Line,
		CONVERT(NUMERIC(18,2),ISNULL(SUM(ISNULL(FR.FitRatio,0)),0)) Text
	INTO 
		#DATA
	FROM 
		FitRatio FR
	...
	SELECT Fecha,DATENAME(MONTH,Fecha) + ' ' + CONVERT(VARCHAR,DATEPART(YEAR,Fecha)) Point, SerieName, value, Color, Line, Text, '' ShowPerc FROM #DATA
	UNION
	SELECT Fecha,DATENAME(MONTH,Fecha) + ' ' + CONVERT(VARCHAR,DATEPART(YEAR,Fecha)) Point, 'Total' SerieName, SUM(value) value, '#FF0000' Color, 1 Line, CONVERT(VARCHAR,SUM(value)) Text, CONVERT(VARCHAR,SUM(value)) ShowPerc FROM #DATA  GROUP BY Fecha,DATENAME(MONTH,Fecha) + ' ' + CONVERT(VARCHAR,DATEPART(YEAR,Fecha))
