IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_FitRatioPSI_T29'))
   exec('CREATE PROCEDURE [dbo].[Epson_FitRatioPSI_T29] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_FitRatioPSI_T29]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT
		'FIT RATIO' SerieName,
		DATENAME(MONTH,@FechaHasta) + ' ' + CONVERT(VARCHAR,DATEPART(YEAR,@FechaHasta)) AxisLabel,
		CONVERT(NUMERIC(18,2),ISNULL(SUM(ISNULL(FR.FitRatio,0)),0)) value,
		'#FF0000' Color,
		0 minY,
		2.5 maxY,
		1.7 Target,
		'CISS Target: ' TargetLabel
	FROM 
		FitRatio FR
	...
	WHERE 
		CONVERT(DATE,FR.Fecha) >= DATEADD(MONTH,-1,@FechaHasta)
