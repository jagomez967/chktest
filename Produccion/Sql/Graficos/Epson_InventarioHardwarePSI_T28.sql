IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_InventarioHardwarePSI_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_InventarioHardwarePSI_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_InventarioHardwarePSI_T28]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	/*
	DECLARE	@FY1Begin	DATE
	DECLARE @FY1End		DATE
	DECLARE @FY2Begin	DATE
	DECLARE @FY2End		DATE

	SELECT @FY1Begin = CONVERT(DATE,CONVERT(VARCHAR(4), DATEPART(YEAR, GETDATE()) - 1) + '04' + '01')
	SELECT @FY1End = CONVERT(DATE,CONVERT(VARCHAR(4), DATEPART(YEAR, GETDATE())) + '03' + '31')
	
	SELECT @FY2Begin = CONVERT(DATE,CONVERT(VARCHAR(4), DATEPART(YEAR, GETDATE())) + '04' + '01')
	SELECT @FY2End = CONVERT(DATE,CONVERT(VARCHAR(4), DATEPART(YEAR, GETDATE()) + 1) + '03' + '31')
	*/

	
	SELECT 
		'FY' + CONVERT(VARCHAR(2),RIGHT(DATEPART(YEAR,DATEADD(YEAR,-1,@FechaDesde)),2)) Point,
		F.Nombre SerieName,
		SUM(ISNULL(FC.ChannelInv,0)) value,
	...
	UNION
	SELECT 
		'FY' + CONVERT(VARCHAR(2),RIGHT(DATEPART(YEAR,@FechaDesde),2)) Point,
		F.Nombre SerieName,
		SUM(ISNULL(FC.ChannelInv,0)) value,
		CASE F.Nombre WHEN 'CISS HW' THEN '#3EA9F5' 
		WHEN 'IC HW' THEN '#0000FF' 
	...
	SELECT 
		'INVENTARIO (HW)' SerieName, 
		 '' Perc,
		 1 Stack,
		 0 ShowText, 
		 1 PersistTooltip

	SELECT *,'' Text, '' ShowPerc FROM #DATA
