IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_MarketShareYTD_T27'))
   exec('CREATE PROCEDURE [dbo].[Epson_MarketShareYTD_T27] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_MarketShareYTD_T27]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 'MARKET SHARE YTD' SerieName, '' Total, 1 FullPie, 1 ShowText,@Fecha SubTitle
	...
	SELECT *, CONVERT(VARCHAR,CONVERT(INT,value)) +' (' + CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),value * 100 / (SELECT SUM(value) FROM #DATA))) + ' %)' Texto FROM #DATA 
