IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_SellInICYTD_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_SellInICYTD_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].Epson_SellInICYTD_T28
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 
		'' Point,
		'YTD ' + CASE WHEN DATEPART(MONTH,GETDATE()) >= 4 THEN CONVERT(VARCHAR,YEAR(DATEADD(YEAR,-1,GETDATE()))) ELSE CONVERT(VARCHAR,YEAR(DATEADD(YEAR,-2,GETDATE()))) END SerieName,
		ISNULL(SUM(ISNULL(FC.SalesIn,0)),0) value,
		'#F15A24' Color,
		0 Line
	INTO
		#DATA
	...
	WHERE 
		CONVERT(DATE,Fecha) >= CASE WHEN DATEPART(MONTH,GETDATE()) >= 4 THEN CONVERT(VARCHAR,DATEPART(YEAR,GETDATE()) - 1) + '0401' ELSE CONVERT(VARCHAR,DATEPART(YEAR,GETDATE()) -2) + '0401' END
		AND CONVERT(DATE,Fecha) <= CASE WHEN DATEPART(MONTH,GETDATE()) >= 4 THEN GETDATE() ELSE DATEADD(YEAR,-1,GETDATE()) END
	...
	SELECT 
		'' Point,
		'YTD ' + CASE WHEN DATEPART(MONTH,GETDATE()) >= 4 THEN CONVERT(VARCHAR,YEAR(GETDATE())) ELSE CONVERT(VARCHAR,YEAR(DATEADD(YEAR,-1,GETDATE()))) END SerieName,
		ISNULL(SUM(ISNULL(FC.SalesIn,0)),0) value,
		'#7AC943' Color,
		0 Line
	FROM 
	...
	SELECT *,'' Text, value ShowPerc FROM #DATA ORDER BY Point DESC
