IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_SellInIC_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_SellInIC_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_SellInIC_T28]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 
		'' Point,
		'Plan' SerieName,
		SUM(ISNULL(FC.PlanOriginalSellIn,0)) value,
		'#0071BC' Color,
		0 Line
	INTO
		#DATA
