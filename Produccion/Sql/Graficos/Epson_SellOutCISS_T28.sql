IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_SellOutCISS_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_SellOutCISS_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_SellOutCISS_T28]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 
		'Sell-Out | Results (CISS)' SerieName, 
		 '' Perc,
		 0 Stack,
		 0 ShowText, 
		 0 PersistTooltip,
