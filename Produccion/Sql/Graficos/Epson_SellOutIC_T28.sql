IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_SellOutIC_T28'))
   exec('CREATE PROCEDURE [dbo].[Epson_SellOutIC_T28] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_SellOutIC_T28]
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT 
		'' Point,
		'Plan' SerieName,
		SUM(ISNULL(FC.PlanOriginalSellOut,0)) value,
		'#0071BC' Color,
		0 Line
	INTO
		#DATA
	...
	SELECT 
		'' Point,
		'Actuals' SerieName,
		SUM(ISNULL(FC.SalesOut,0)) value,
		'#F7931E' Color,
		0 Line
	FROM 
		Forecasting FC
