IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Epson_ventas_producto_T9'))
   exec('CREATE PROCEDURE [dbo].[Epson_ventas_producto_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Epson_ventas_producto_T9]
(
	@IdCliente			INT,
...
BEGIN
	...
	create table #ventasProductos (producto varchar(100),cantidad int,cadena varchar(100))

	
	insert #ventasProductos(producto,cantidad,cadena)
	select p.nombre ,
		   sum(case when @idCliente = 222 then isnull(rp.Cantidad2,0) else isnull(rp.Cantidad,0) end),
		   cad.nombre 
	from reporte r 
	...
	group by p.nombre , cad.nombre

	declare @DinamicCol varchar(5000)
	declare @SqlPivot nvarchar(max)

	select @DinamicCol = COALESCE(@DinamicCol + ',' , '') + '[' + x.cadena + ']'
	from (select distinct cadena from #ventasProductos)  x

	set @SqlPivot = 'SELECT Producto,' + @DinamicCol  + ' from' +
	'(SELECT producto,cadena,cantidad FROM #ventasProductos)p '+
	'PIVOT'+
	'(SUM(cantidad) FOR cadena IN ('+@DinamicCol +')) as pvt'
