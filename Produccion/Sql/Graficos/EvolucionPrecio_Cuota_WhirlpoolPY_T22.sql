IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.EvolucionPrecio_Cuota_WhirlpoolPY_T22'))
   exec('CREATE PROCEDURE [dbo].[EvolucionPrecio_Cuota_WhirlpoolPY_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[EvolucionPrecio_Cuota_WhirlpoolPY_T22]
(
	@IdCliente			int
...
WHERE 
 ...
 --select top 10 * from #data where idPRoducto =19045
 ---select * from #data 
...
exec EvolucionPrecio_Cuota_WhirlpoolPY_T22 @IdCliente=190,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
