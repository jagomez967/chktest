IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FSCampanasyPDV_T30'))
   exec('CREATE PROCEDURE [dbo].[FSCampanasyPDV_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[FSCampanasyPDV_T30]
(
	@IdCliente			INT,
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			into #temp1 
			from #tempreporte 
			where idReporte NOT IN (SELECT idReporte FROM ReportePop WHERE idMarca = 2472) 
			...
			WHERE pm.idMarca = 2472 and pop.Activo = 1
...
exec FSCampanasyPDV_T30 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
*/
