IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FSCocinasyPDV_T30'))
   exec('CREATE PROCEDURE [dbo].[FSCocinasyPDV_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[FSCocinasyPDV_T30]
(
	@IdCliente			INT,
...
--	group by  r.idPuntodeventa,convert(varchar,r.FechaCreacion,103),p.Direccion,z.Nombre,ca.Nombre,u.Usuario,p.Nombre
		...
		select
			...
			where m.IdMarca = 2469
 			group by m.idempresa, m.IdMarca, rp.idpop, t.mes,rp.idreporte,rp.cantidad,m.Nombre,p.Nombre
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			into #temp1 
			from #tempreporte 
			where idReporte NOT IN (SELECT idReporte FROM ReportePop WHERE idMarca = 2469) 
			...
			WHERE pm.idMarca = 2469 and pop.Activo = 1
...
insert into @p2 values(N'fltFechaReporte',N'M,20190214,20190214')
--insert into @p2 values(N'fltpuntosdeventa',N'176866')
--insert into @p2 values(N'fltusuarios',N'4148')
--insert into @p2 values(N'fltMarcas',N'648')
exec FSCocinasyPDV_T30 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
