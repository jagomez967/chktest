IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FSPurificadoresDeAguaYPDV_T30'))
   exec('CREATE PROCEDURE [dbo].[FSPurificadoresDeAguaYPDV_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[FSPurificadoresDeAguaYPDV_T30]
(
	@IdCliente			INT,
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			into #temp1 
			from #tempreporte 
			where idReporte NOT IN (SELECT idReporte FROM ReportePop WHERE idMarca = 2474) 
			...
			WHERE pm.idMarca = 2474 and pop.Activo = 1
...
insert into @p2 values(N'fltFechaReporte',N'M,20180101,20190228')
...
exec FSPurificadoresDeAguaYPDV_T30 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
