IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FSPurificadoresYPDV_T30'))
   exec('CREATE PROCEDURE [dbo].[FSPurificadoresYPDV_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[FSPurificadoresYPDV_T30]
(
	@IdCliente			INT,
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			into #temp1 
			from #tempreporte 
			where idReporte NOT IN (SELECT idReporte FROM ReportePop WHERE idMarca = 2470) 
			...
			WHERE pm.idMarca = 2470 and pop.Activo = 1
...
exec FSPurificadoresYPDV_T30 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
*/
