IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Fac_DetallePrecioPropio3_T9'))
   exec('CREATE PROCEDURE [dbo].[Fac_DetallePrecioPropio3_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Fac_DetallePrecioPropio3_T9]
(
	@IdCliente			int
...
*/		
	...
	where (isnull(rp.Cantidad,0)>0 or isnull(rp.Cantidad2,0)>0 or isnull(Precio3,0)>0)
...
exec Fac_DetallePrecioPropio3_T9 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec Fac_DetallePrecioPropio3_T9 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/