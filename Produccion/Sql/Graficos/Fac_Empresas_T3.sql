IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Fac_Empresas_T3'))
   exec('CREATE PROCEDURE [dbo].[Fac_Empresas_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Fac_Empresas_T3]
(
	@IdCliente			int
...
*/		
	...
	select LTRIM(rtrim(e.nombre)), right(CONVERT(VARCHAR(11),convert(datetime,r.mes+'01'),6),6), isnull(r.qty,0), r.idEmpresa, r.mes,1 Visiblelabel
	from #Resultados r
