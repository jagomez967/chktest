IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Fac_MarcaProd_T6'))
   exec('CREATE PROCEDURE [dbo].[Fac_MarcaProd_T6] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Fac_MarcaProd_T6] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, m.idMarca, rp.IdProducto, r.mes, SUM(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0))
	...
	select r.idmarca, LTRIM(rtrim(m.nombre)), right(CONVERT(VARCHAR(11),convert(datetime,r.mes+'01'),6),6), isnull(r.qty,0), r.mes,1 Visiblelabel
	from #Resultados r
	...
	select r.idMarca, LTRIM(rtrim(m.nombre)), r.idProducto, ltrim(rtrim(p.Nombre)), right(CONVERT(VARCHAR(11),convert(datetime,r.mes+'01'),6),6), isnull(r.qty,0), r.mes,1 Visiblelabel
	from #Resultados r
	inner join marca m on m.IdMarca=r.idMarca
