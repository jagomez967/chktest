...
*/		
	...
	--Datos
	if(@NumeroDePagina>0)
		select fecha, pdv, direccion, zona, usuario, q1, q2, q3 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina) order by fecha desc
	
	if(@NumeroDePagina=0)
		select fecha, pdv, direccion, zona, usuario, q1, q2, q3 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina) order by fecha desc
