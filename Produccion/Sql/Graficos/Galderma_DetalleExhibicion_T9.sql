IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Galderma_DetalleExhibicion_T9'))
   exec('CREATE PROCEDURE [dbo].[Galderma_DetalleExhibicion_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Galderma_DetalleExhibicion_T9
(
	@IdCliente			int
...
*/		
	...
	where isnull(mdr.valor1,0) > 0 and mi.idModulo = 1886
	...
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	where isnull(mdr.Valor2,0) > 0 and mi.idModulo = 1886
	...
	set @ColDef='pdv varchar(max), usuario varchar(max), modulo varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([pdv],[usuario], [modulo],'+@PivotColumnHeaders+')
	SELECT [pdv],[usuario], [modulo],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, pdv, usuario, modulo, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, z.Nombre as Zona, mdm.Nombre as Item,'+@PivotColumnHeaders+'
