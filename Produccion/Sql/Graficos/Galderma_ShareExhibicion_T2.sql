IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Galderma_ShareExhibicion_T2'))
   exec('CREATE PROCEDURE [dbo].[Galderma_ShareExhibicion_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Galderma_ShareExhibicion_T2]
(
	@IdCliente			int
...
*/		
	...
	select LTRIM(rtrim(mi.Nombre)),SUM(isnull(d.qty,0)), te.idItem
	from #ItemMeses te
	left join #datos d on d.idItem = te.idItem and d.mes = te.mes
	inner join MD_Item mi on mi.idItem = te.idItem
	group by rtrim(mi.nombre), te.idItem
	order by te.idItem
end