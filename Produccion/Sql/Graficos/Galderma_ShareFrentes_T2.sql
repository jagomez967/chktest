IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Galderma_ShareFrentes_T2'))
   exec('CREATE PROCEDURE [dbo].[Galderma_ShareFrentes_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Galderma_ShareFrentes_T2]
(
	@IdCliente			int
...
*/		
	...
	select rmi.idItem, t.mes, sum(isnull(convert(int,rmi.Valor4),0)) from #tempReporte t
	inner join MD_ReporteModuloItem rmi on rmi.idReporte = t.idReporte
	inner join MD_Item mi on mi.idItem = rmi.idItem
	where isnull(convert(int,rmi.Valor4),0) > 0
			and mi.idModulo = 1887
