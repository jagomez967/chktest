IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.GardenPE_CumplimientoCuota_T9'))
   exec('CREATE PROCEDURE [dbo].[GardenPE_CumplimientoCuota_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[GardenPE_CumplimientoCuota_T9]
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Fecha',5),('Zona','Zona',5),('realCuota','Real',5),('Objetivo','Cuota',5),('cumplimiento','%',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Date',5),('Zona','Zone',5),('realCuota','Real',5),('Objetivo','Quota',5),('cumplimiento','%',10)
...
--GardenPE_CumplimientoCuota_T9 81
