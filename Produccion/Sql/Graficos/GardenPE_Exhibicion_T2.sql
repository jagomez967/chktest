IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.GardenPE_Exhibicion_T2'))
   exec('CREATE PROCEDURE [dbo].[GardenPE_Exhibicion_T2] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].GardenPE_Exhibicion_T2
(
	@IdCliente			int
...
*/		
	...
	select mi.Nombre, count(isnull(rmi.Valor1,0)) from #reportesMesPdv t
	inner join MD_ReporteModuloItem rmi on rmi.idReporte = t.idReporte
	inner join MD_Item mi on mi.idItem = rmi.idItem
	where isnull(rmi.Valor1,0) > 0 and mi.idModulo = 1551
	group by mi.Nombre

	select exhibicion, total from #datosRes
