IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.GardenPE_PromedioPedido_T9'))
   exec('CREATE PROCEDURE [dbo].[GardenPE_PromedioPedido_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[GardenPE_PromedioPedido_T9]
(
	@IdCliente			int
...
*/		
	...
	create table #DatosParcial_cant
	(
		idUsuario int,
		cantidad int,
		año int,
	...
	--Cantidad de relevos mensuales (VentaEfectiva)	
	insert #datosParcial_cant(idUsuario,cantidad,año,mes)
	...
	select c.idUsuario,
		  sum(rp.cantidad)/c.cantidad,
		  c.año,
		  c.mes
	from #tempreporte t
	inner join reportePOP rp on rp.idReporte = t.idReporte
	inner join #datosParcial_Cant c on c.idUsuario = t.idUsuario
	and c.año = t.año and c.mes = t.mes 
	group by c.idUsuario, c.año,c.mes,c.cantidad


	update dp
	set objetivo = gc.Pedido
	from #datosParcial dp
	inner join gardenPE_PromedioPedido gc
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Fecha',5),('Zona','Zona',5),('realCuota','Promedio Pedido',5),('Objetivo','Objetivo',5),('cumplimiento','%',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Date',5),('Zona','Zone',5),('realCuota','Promedio',5),('Objetivo','Target',5),('cumplimiento','%',10)
	...
	BEGIN
		if(@idCliente = 81)
			select datename(month,Fecha) + ' ' + cast(year(Fecha) as varchar) as Fecha,Zona,'S/ ' + convert(varchar,realCuota),'S/ ' + convert(varchar,Objetivo),cast(Cumplimiento as varchar)+' %' as cumplimiento from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
		else
	...
	BEGIN
		if(@idCliente = 81)
			select datename(month,Fecha) + ' ' + cast(year(Fecha) as varchar) as Fecha,Zona,'S/ ' + convert(varchar,realCuota),'S/ ' + convert(varchar,Objetivo),cast(Cumplimiento as varchar)+' %' as cumplimiento from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		else
	...
	BEGIN
		if(@idCliente = 81)
			select datename(month,Fecha) + ' ' + cast(year(Fecha) as varchar) as Fecha,Zona,'S/ ' + convert(varchar,realCuota),'S/ ' + convert(varchar,Objetivo),cast(Cumplimiento as varchar)+' %' as cumplimiento from #datosFinal
		else
