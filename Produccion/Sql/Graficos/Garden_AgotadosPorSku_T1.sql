IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_AgotadosPorSku_T1'))
   exec('CREATE PROCEDURE [dbo].[Garden_AgotadosPorSku_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_AgotadosPorSku_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	declare @cantRep numeric(18,5)
	select @cantRep = count(distinct idreporte) from #tempReporte

	create table #AgotadosPorSku
	(
		producto varchar(100),
		agotados numeric
	)
	
	insert #AgotadosPorSku
	select ltrim(rtrim(p.Nombre)), cast(sum(isnull(rp.stock,0))*100/@cantRep as decimal (10,2)) from #tempReporte r
	inner join ReporteProducto rp on rp.IdReporte = r.IdReporte
	inner join Producto p on p.IdProducto = rp.IdProducto
	where FechaCreacion between @fechaDesde and @fechaHasta
	group by rp.IdProducto, p.Nombre
	order by 2
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('producto','Producto'),('agotados','% SKU Agotados')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('producto','Products'),('agotados','% Out of Stock')

	select name, title from #columnasConfiguracion

	select ltrim(rtrim(producto)), isnull(agotados,0) from #AgotadosPorSku


	--exec Garden_AgotadosPorSku_T1 1,'M,20160701,20160731'
