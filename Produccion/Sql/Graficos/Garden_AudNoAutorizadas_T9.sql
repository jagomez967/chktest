IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_AudNoAutorizadas_T9'))
   exec('CREATE PROCEDURE [dbo].[Garden_AudNoAutorizadas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_AudNoAutorizadas_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #tempReporte (IdPuntoDeVenta, idUsuario, mes)
	...
	and r.AuditoriaNoAutorizada = 1
	

	insert #datos
	select pdv.Nombre, 1 from #tempReporte temp
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = temp.IdPuntoDeVenta


	--exec Garden_AudNoAutorizadas_T9 1,'M,20160601,20160630'
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('pdv','PuntoDeVenta',5),('aud','AuditoriaNoAutorizada',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('pdv','PointOfSale',5),('aud','UnauthorizedAudit',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select pdv, aud from #datos where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select pdv, aud from #datos where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select pdv, aud from #datos
