IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_Categorizacion_T2'))
   exec('CREATE PROCEDURE [dbo].[Garden_Categorizacion_T2] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].Garden_Categorizacion_T2
(
	@IdCliente			int
...
*/		
	...
	select 'A', sum(convert(int,rmi.Valor1)) from MD_ReporteModuloItem rmi
	inner join #reportesMesPdv r on r.IdReporte = rmi.IdReporte
	where rmi.IdItem = 2102
	union
	select 'B', sum(convert(int,rmi.Valor2)) from MD_ReporteModuloItem rmi
	inner join #reportesMesPdv r on r.IdReporte = rmi.IdReporte
	where rmi.IdItem = 2102
	union
	select 'C', sum(convert(int,rmi.Valor3)) from MD_ReporteModuloItem rmi
	inner join #reportesMesPdv r on r.IdReporte = rmi.IdReporte
	where rmi.IdItem = 2102
