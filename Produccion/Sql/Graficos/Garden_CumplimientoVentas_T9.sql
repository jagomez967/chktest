IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_CumplimientoVentas_T9'))
   exec('CREATE PROCEDURE [dbo].[Garden_CumplimientoVentas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_CumplimientoVentas_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (IdZonaVenta, Zona, Ventas, Cuota, Cumplimiento, Fecha)
	select	vg.IdZona,
	...
	inner join ETL_RealVentasGarden rvg on rvg.mes = vg.mes and rvg.idzona = vg.idzona
		and rvg.año = vg.año
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Fecha',5),('Zona','Zona',5),('Ventas','Ventas',5),('Cuota','Cuota',5),('Cumplimiento','%',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('Fecha','Date',5),('Zona','Zone',5),('Ventas','Sales',5),('Cuota','Quota',5),('Cumplimiento','%',10)
	...
	--exec Garden_CumplimientoVentas_T9 null,'M,20160601,20160709'
	end