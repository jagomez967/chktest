IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_Exhibicion_T2'))
   exec('CREATE PROCEDURE [dbo].[Garden_Exhibicion_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Garden_Exhibicion_T2
(
	@IdCliente			int
...
*/		
	...
	(
		exhibicion varchar(100),
		total int,
	)


	insert #datosRes (exhibicion, total)
	select mi.Nombre, sum(convert(int,rmi.valor1)) from MD_ReporteModuloItem rmi
	inner join #reportesMesPdv r on r.IdReporte = rmi.IdReporte
	inner join MD_Item mi on mi.IdItem = rmi.IdItem
	where mi.IdModulo = 455 and rmi.Valor1 > 0
	group by mi.Nombre

	select * from #datosRes

	--exec Garden_Exhibicion_T2 1,'M,20160701,20160731'
