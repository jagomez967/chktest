IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_RealVentas_T9'))
   exec('CREATE PROCEDURE [dbo].[Garden_RealVentas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_RealVentas_T9]
(
	@IdCliente			int
...
*/		
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			...
			inner join ETL_ZonasVentas zv on d.idZona = zv.IdZona
