IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_RepProdPropio_T9'))
   exec('CREATE PROCEDURE [dbo].[Garden_RepProdPropio_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_RepProdPropio_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (idPuntoDeVenta, PuntoDeVenta, Usuario, Zona, Tramo, Producto, Frentes1)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('PuntoDeVenta','PuntoDeVenta',5),('Usuario','RTM',5),('Zona','Zona',5),('Tramo','Tramo',5),('Producto','Producto',5),('Frentes1','Cantidad',5)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('PuntoDeVenta','PDV',5),('Usuario','RTM',5),('Zona','Zone',5),('Tramo','Tramo',5),('Producto','Product',5),('Frentes1','Qty',5)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select PuntoDeVenta, Usuario, Zona, Tramo, Producto, Frentes1 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select PuntoDeVenta, Usuario, Zona, Tramo, Producto, Frentes1 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select PuntoDeVenta, Usuario, Zona, Tramo, Producto, Frentes1 from #datosFinal
