IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_TransCopidrogas_T9'))
   exec('CREATE PROCEDURE [dbo].[Garden_TransCopidrogas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_TransCopidrogas_T9]
(
	@IdCliente			int
...
*/		
	...
	where convert(date,cast(vg.año as varchar) + right('00' + cast(vg.mes as varchar),2) + '01') 
	between convert(date,@fechaDesde) and convert(date,@fechaHasta)
	...
	insert #totalesZona
	select IdZona, sum(isnull(Cumplimiento,0))/@cantMeses from #datosRes
	group by IdZona
	...
	select distinct cast(i.Fecha as varchar) as name, datename(month,i.FechaT) + '' '' + cast(year(i.FechaT) as varchar) as title, 40 as width, 5 as orden
	from #DatosRes i
	...
	----Datos
	--if(@NumeroDePagina>0)
	--	select Zona,Real,Cuota,Cumplimiento from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	--if(@NumeroDePagina=0)
	--	select Zona,Real,Cuota,Cumplimiento from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
	...
	--exec Garden_TransCopidrogas_T9 null,'M,20160101,20160505'
