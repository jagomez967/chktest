IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Garden_Transferencia_T1'))
   exec('CREATE PROCEDURE [dbo].[Garden_Transferencia_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Garden_Transferencia_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #tempReporte (idCliente, idUsuario, IdPuntoDeVenta, idReporte, fechaCreacion, mes)
	select	c.IdCliente
			,r.IdUsuario
			,r.IdPuntoDeVenta
			,r.idReporte
	...
	insert #Resultados
	select ltrim(rtrim(p.Nombre)), sum(isnull(rp.cantidad,0)) from ReportePop rp
	inner join #tempReporte r on r.IdReporte = rp.IdReporte
	inner join Pop p on p.IdPop = rp.IdPop
	group by rp.IdPop, p.Nombre
	order by 2
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('pop','POP'),('cant','Qty')
	...
	select ltrim(rtrim(pop)), isnull(cant,0) from #Resultados

	
	--exec Garden_Transferencia_T1 1,'M,20160601,20160701'
