IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Img_FotosCanon_T15'))
   exec('CREATE PROCEDURE [dbo].[Img_FotosCanon_T15] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Img_FotosCanon_T15] 	
(
	@IdCliente			int
...
*/		
	...
	where	c.IdCliente in(select idCliente from #clientes)
			...
			and exists(select 1 from imagenesTags where idImagen =pdvf.idpuntodeventafoto
				 and idTag in(select idTag from tags where leyenda like '%CANON%'))
		...
		where idPdvFoto not in (select idPdvFoto from #datosRes dr 
							inner join imagenesTags it on it.idImagen = dr.IdPdvFoto
								where idTag in(select idTag from tags where leyenda like '%CANON%')
							)
	...
	--Datos
		...
		if(@NumeroDePagina>0)
			select IdPdvFoto, empresa, fechaCreacion from #datosRes  where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
		
		if(@NumeroDePagina=0)
			select IdPdvFoto, empresa, fechaCreacion from #datosRes  where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
		if(@NumeroDePagina<0)
			select IdPdvFoto, empresa, fechaCreacion from #datosRes	

		--select 'https://vignette.wikia.nocookie.net/logopedia/images/c/c7/Canon_logo.png'
		select 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcToasjKTQCcYCLxiVJyZMqxnWt9fWBudTwyXVjMcR7YsSv1zvJP'
		as Imagen
end
go
--img_fotosCANON_t15 194


declare @p2 dbo.FiltrosReporting
insert into @p2 values(N'fltFechaReporte',N'M,20180601,20180630')
insert into @p2 values(N'fltClientes',N'178')

exec img_fotosCANON_t15 @IdCliente=194,@Filtros=@p2,@NumeroDePagina=0,@Lenguaje='en',
@IdUsuarioConsulta=827,@TamañoPagina=8
