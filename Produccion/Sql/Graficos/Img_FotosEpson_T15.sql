IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Img_FotosEpson_T15'))
   exec('CREATE PROCEDURE [dbo].[Img_FotosEpson_T15] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Img_FotosEpson_T15] 	
(
	@IdCliente			int
...
*/		
	...
	where	c.IdCliente in(select idCliente from #clientes)
			...
			and exists(select 1 from imagenesTags where idImagen =pdvf.idpuntodeventafoto
				 and idTag in(select idTag from tags where leyenda like '%EPSON%'))
		...
		where idPdvFoto not in (select idPdvFoto from #datosRes dr 
							inner join imagenesTags it on it.idImagen = dr.IdPdvFoto
								where idTag in(select idTag from tags where leyenda like '%EPSON%')
							)
	...
	--Datos
		...
		select 'https://i0.wp.com/comsyscr.com/wp-content/uploads/2018/01/epson.png'
		as Imagen
end
go

img_fotosEPSON_t15 194
