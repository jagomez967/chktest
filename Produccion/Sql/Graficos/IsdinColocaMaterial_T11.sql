IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinColocaMaterial_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinColocaMaterial_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinColocaMaterial_T11]
(
	@IdCliente			int
...
--	select distinct nombre From md_modulo where nombre like '%material%'	
--	and idCliente in
--(	select idCliente From familiaClientes where familia like'%isdin%')
	select @cantidad = count(distinct idReporte)
	From #baseContactos
	where nombreModulo = 'Colocación de Material'
	
	declare @total int

	select @total = count(1) from
	(select r.idPuntodeventa,r.dia 
	from #tempReporte r
	where exists (select 1 from md_reporteModuloItem md 
					inner join md_item i on i.idItem = md.idItem
					inner join md_modulo m on m.idModulo = i.idModulo
					where md.idReporte = r.idReporte
					and m.nombre = 'Colocación de Material')
	group by r.idPuntodeventa,r.dia)x
	

	select 1 as modulo,'Si',@cantidad as valor
	union
	select 2 as modulo,'No',@total - @cantidad as valor


	update #baseContactos
	set respuesta = 1
	where isnumeric(respuesta) <> 1

	select 1 as modulo,'Si',ROW_NUMBER() over(Order by NombreItem desc),NombreItem, sum(cast(respuesta as int)) as valor
	From #baseContactos
	where nombreModulo = 'De que linea colocaste material?'
	group by NombreItem
end
