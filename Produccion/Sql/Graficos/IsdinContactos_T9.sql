IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinContactos_T9'))
   exec('CREATE PROCEDURE [dbo].[IsdinContactos_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinContactos_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal(dia,idUsuario,usuario,contactos,objetivo)
	select bc.fecha,bc.idUsuario,u.apellido + ', ' + u.nombre collate database_default,  sum(bc.Contactos) , bo.objetivo
	from #baseContactos bc
	inner join #baseObjetivo bo on bo.idusuario = bc.idUsuario and bo.fecha = bc.fecha
	inner join usuario u on u.idUsuario = bc.idUsuario
	where (isnull(@cUsuarios,0) = 0 or exists(select 1 from #usuarios where idUsuario = bo.IdUsuario)) and bo.IdUsuario not in (select idUsuario from usuario where escheckpos = 1)
	group by bc.fecha,bc.idUsuario,u.apellido + ', ' + u.nombre collate database_default,  bo.objetivo
	...
	if(@lenguaje = 'es')
		...
		values	
			...
			(0,1,'objetivo','Objetivo',30),
			(0,1,'Contactos','Contactos',30)
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (esclave,mostrar,name, title, width) 
		values
			(1,0,'idUsuario','Id',5),
			(0,1,'dia','Date',30),
			(0,1,'usuario','User',50),	
			(0,1,'objetivo','Target',30),
			(0,1,'Contactos','Contacts',30)

	select esclave,mostrar,name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select cast(d.idUsuario as varchar(max))+ ';'+ cast(d.dia as varchar) as idUsuario,
		d.dia,d.usuario,d.objetivo,d.contactos as Contactos
		from #datosFinal d 
		where d.id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
		order by d.dia, d.Usuario
		
	if(@NumeroDePagina=0)
		select cast(d.idUsuario as varchar(max))+ ';'+ cast(d.dia as varchar) as idUsuario,d.dia,d.usuario,d.objetivo,
		d.contactos as Contactos
		from #datosFinal d 
		where d.id
