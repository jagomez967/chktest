IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinDosisInicio_T11'))
   exec('CREATE PROCEDURE [dbo].[IsdinDosisInicio_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinDosisInicio_T11]
(
	@IdCliente			int
...
*/		
	...
	From #baseContactos
	where nombreModulo = 'Dosis de inicio'
	...
	From #baseContactos
	where nombreModulo = 'De cual producto entregaste dosis de inicio? '
	group by NombreItem
end
