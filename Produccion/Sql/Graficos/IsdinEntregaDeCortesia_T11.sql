IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinEntregaDeCortesia_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinEntregaDeCortesia_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinEntregaDeCortesia_T11]
(
	@IdCliente			int
...
*/		
	...
	From #baseContactos
	where nombreModulo = 'Entrega de Cortesía'
	
	
	--'De cual producto entregaste cortesía? '
	declare @total int
	...
	where exists (select 1 from md_reporteModuloItem md 
					...
					and m.nombre = 'Entrega de Cortesía')
	...
	select 1 as modulo,'Si',ROW_NUMBER() over(Order by NombreItem desc),NombreItem, count(cast(respuesta as int)) as valor
	From #baseContactos
	where nombreModulo = 'De cual producto entregaste cortesía? '	
	group by NombreItem
end
go