IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinEntregaMaterial_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinEntregaMaterial_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinEntregaMaterial_T11]
(
	@IdCliente			int
...
*/		
	...
	From #baseContactos
	where nombreModulo = 'Que Material entregaste?'
	group by NombreItem
end
