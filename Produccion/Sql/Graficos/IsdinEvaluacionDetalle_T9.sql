IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinEvaluacionDetalle_T9'))
   exec('CREATE PROCEDURE [dbo].[IsdinEvaluacionDetalle_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinEvaluacionDetalle_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal(modulo,pregunta,contactos,marca)
	select bc.nombreModulo,bc.nombreItem,sum(bc.contactos),m.nombre
	from #baseContactos bc
	LEFT join md_moduloMarca mm on mm.idModulo = bc.idModulo
	LEFT join marca m on m.idMarca = mm.idMarca
	group by  bc.nombreModulo,bc.nombreItem,m.nombre
	...
	if(@lenguaje = 'es')
		...
		values	
			...
			(0,1,'modulo','Modulo',50,0),	
			(0,1,'pregunta','Item',30,0),
			(0,1,'contactos','Contactos',30,0)
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (esclave,mostrar,name, title, width,esAgrupador) 
		values
			(0,0,'marca','Brand',5,1),

			(0,1,'modulo','Module',50,0),	
			(0,1,'pregunta','Item',30,0),
			(0,1,'contactos','Contacts',30,0)

	select esclave,mostrar,name, title, width,esAgrupador from #columnasConfiguracion


		select marca,modulo,pregunta,contactos
		from #datosFinal d 
