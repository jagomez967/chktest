IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinHiciste12_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinHiciste12_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinHiciste12_T11]
(
	@IdCliente			int
...
*/		
	...
	From #baseContactos
	where nombreModulo = 'Hiciste 1-2?'
	...
	From #baseContactos
	where nombreModulo = 'De que linea hiciste 1-2?'
	group by NombreItem
end
