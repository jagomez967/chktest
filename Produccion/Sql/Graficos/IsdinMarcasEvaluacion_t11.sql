IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinMarcasEvaluacion_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinMarcasEvaluacion_t11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinMarcasEvaluacion_t11]
(
	@IdCliente			int
...
*/		
	...
	select DENSE_RANK() OVER (ORDER BY m.nombre), m.nombre,sum(bc.contactos),1 showVal
	from #baseContactos bc
	inner join md_moduloMarca mm on mm.idModulo = bc.idModulo
	inner join marca m on m.idMarca = mm.idMarca
	group by m.nombre

	
	select DENSE_RANK() OVER (ORDER BY m.nombre), m.nombre,DENSE_RANK() OVER (ORDER BY bc.nombreModulo),bc.nombreModulo,sum(bc.contactos),1 showVal
	from #baseContactos bc
	inner join md_moduloMarca mm on mm.idModulo = bc.idModulo
	inner join marca m on m.idMarca = mm.idMarca
	group by m.nombre,bc.nombreModulo
