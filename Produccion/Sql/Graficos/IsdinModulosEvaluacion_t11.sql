IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinModulosEvaluacion_t11'))
   exec('CREATE PROCEDURE [dbo].[IsdinModulosEvaluacion_t11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinModulosEvaluacion_t11]
(
	@IdCliente			int
...
*/		
	...
	select idModulo,nombreModulo,sum(contactos)
	from #baseContactos
	group by idModulo,nombreModulo

	update #basecontactos set nombreItem = '-sin Item-' where nombreItem is null
	
	select idModulo,nombreModulo,DENSE_RANK() OVER (ORDER BY nombreItem), nombreItem,sum(contactos)
	from #baseContactos
	group by idModulo,nombreModulo,nombreItem
end
go
