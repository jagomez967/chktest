IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.IsdinObjetivoRTM_T12'))
   exec('CREATE PROCEDURE [dbo].[IsdinObjetivoRTM_T12] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[IsdinObjetivoRTM_T12]
(
	@IdCliente			int
...
*/		
	...
	create table #usuariosContactos(idUsuario int, contactos float, objetivo float,fecha date)

	insert #usuariosContactos(idUsuario,contactos,objetivo,fecha)
	select ISNULL(bc.idUsuario,bo.idUsuario),sum(isnull(bc.contactos,0)),bo.objetivo,ISNULL(bc.fecha,bo.fecha)
	from #baseObjetivo bo
	left join #baseContactos bc
	on bo.idusuario = bc.idusuario and bc.fecha = bo.fecha
	...
	group by bc.idUsuario,bo.idUsuario,bc.fecha,bo.objetivo,bo.fecha
	 
	insert #datos (idUsuario, valor)
	select idUsuario , (SUM(contactos)* 100 / SUM(Objetivo))
	...
	,0 as varianza
	,0 as parentId
	,u.Nombre collate database_default as nombre--,u.apellido+', '+u.Nombre collate database_default as nombre
	from #datos d
