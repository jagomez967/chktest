IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Isdin_LAM_ReporteNuevo_T20'))
   EXEC('CREATE PROCEDURE [dbo].[Isdin_LAM_ReporteNuevo_T20] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Isdin_LAM_ReporteNuevo_T20]
(
	@IdCliente			INT,
...
*/		
	...
	SELECT
		...
		CASE WHEN RMI.Valor4 LIKE '%[a-zA-z]%' OR RMI.Valor4 = '' THEN '0' ELSE RMI.Valor4 END Cantidad
	INTO
		#DatosFinal
	FROM
		Reporte R
	...
	WHERE 
		...
		AND 1 = CASE WHEN RMI.Valor4 LIKE '%[a-zA-z]%' OR RMI.Valor4 = '' THEN 0 ELSE 1 END 
	...
	IF ISNULL(@Lenguaje,'ES') = 'ES' BEGIN
		...
		('PuntoDeVenta','Punto de Venta',10,2),
		('Direccion','Direccion',10,3),
		('Cantidad','Cantidad',10,4)
	...
	IF ISNULL(@Lenguaje,'ES') = 'EN' BEGIN
		...
		('Fecha','Date',10,0),
		('Usuario','User',10,1),
		('PuntoDeVenta','Point of Sale',10,2),
		('Direccion','Adress',10,3),
		('Cantidad','Quantity',10,4)
	END
...
--exec [Isdin_LAM_ReporteNuevo_T20] 175 