IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_Exhibiciones_T1'))
   exec('CREATE PROCEDURE [dbo].[Lanix_Exhibiciones_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Lanix_Exhibiciones_T1] 	
(
	@IdCliente			int
...
*/		
	...
	select rmp.idMarcaPropiedad, count(t.idPuntoDeVenta) from #tempReporte t
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('item','Exhibicion'),('qty','Cantidad')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('item','Exhibition'),('qty','Qty')

	select name, title from #columnasConfiguracion

	select	ltrim(rtrim(mp.Nombre)) as item,
			isnull(r.qty,0) as qty
	from #Resultados r
	inner join MarcaPropiedad mp on mp.idMarcaPropiedad = r.item
	order by ltrim(rtrim(mp.Nombre))
