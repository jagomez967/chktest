IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_InventarioProdPdv_T6'))
   exec('CREATE PROCEDURE [dbo].[Lanix_InventarioProdPdv_T6] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Lanix_InventarioProdPdv_T6] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, r.idPuntoDeVenta, convert(varchar(3),r.fechaCreacion)+' '+cast(year(r.fechaCreacion) as varchar(4)), tmp.mes, SUM(isnull(rp.Cantidad2,0))
	from Reporte r
	...
	select 0, LTRIM(rtrim(r.mesStr)), r.idPuntoDeVenta, ltrim(rtrim(pdv.Nombre)), right(CONVERT(VARCHAR(11),convert(datetime,r.mes+'01'),6),6), isnull(r.qty,0), r.mes
	from #Resultados r
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = r.idPuntoDeVenta
	order by r.idPuntoDeVenta, r.mes, r.mesStr
end