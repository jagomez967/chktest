IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_PerformanceVendedor_T26'))
   exec('CREATE PROCEDURE [dbo].[Lanix_PerformanceVendedor_T26] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_PerformanceVendedor_T26]
(
	@IdCliente			int
...
---
---exec Lanix_PerformanceVendedor_T26 @IdCliente=147,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
