IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_RelevoMD_T9'))
   exec('CREATE PROCEDURE [dbo].[Lanix_RelevoMD_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Lanix_RelevoMD_T9
(
	@IdCliente			int
...
*/		
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre as Marca,'+@PivotColumnHeaders+'
		...
		if('+cast(@NumeroDePagina as varchar)+'<0)
			select pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario, m.Nombre,'+@PivotColumnHeaders+'
			from #DatosPivotFinal d
			inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = d.pdv
			inner join Usuario u on u.idUsuario = d.usuario
			inner join Marca m on m.idMarca = d.idMarca
