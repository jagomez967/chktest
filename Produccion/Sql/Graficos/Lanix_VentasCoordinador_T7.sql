IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasCoordinador_T7'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasCoordinador_T7] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Lanix_VentasCoordinador_T7] 	
(
	@IdCliente			INT
...
*/		
	...
	'Ventas' descr,--(SELECT ISNULL(Apellido,'') + ', ' + ISNULL(Nombre COLLATE DATABASE_DEFAULT,'') FROM Usuario WHERE idUsuario = 2710) descr,
	SUM(A) qty
	FROM
		(SELECT 
...
SELECT
	...
	FROM
		(SELECT 
			MONTH(R.fechaCreacion) MONTH,
			YEAR(R.fechaCreacion) YEAR,
			ISNULL(U.Apellido,'') + ', ' + ISNULL(U.Nombre COLLATE DATABASE_DEFAULT,'') AS Usr,
