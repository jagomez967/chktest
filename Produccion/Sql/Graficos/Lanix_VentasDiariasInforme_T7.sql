IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasDiariasInforme_T7'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasDiariasInforme_T7] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_VentasDiariasInforme_T7] 	
(
	@IdCliente			int
...
*/		
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by r.fecha,r.idPuntodeventa,r.NombrePDV
	union
	select	CONVERT(VARCHAR(10), x.fecha, 112), 
			Convert(varchar(10),CONVERT(date,x.fecha,106),103),
			r.idPuntodeventa, 
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by x.fecha,r.idPuntodeventa,r.NombrePDV
	
end
go

--exec Lanix_ventasdiarias_t7 147
--exec Lanix_VentasDiariasInforme_T7 147