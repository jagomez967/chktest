...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasDiariasProdInforme_T7'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasDiariasProdInforme_T7] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_VentasDiariasProdInforme_T7] 	
(
	@IdCliente			int
...
*/		
	...
	lx.[FECHA DE RECEPCION] as tipoDeEntrega,
	cast(lx.[FECHA DE RECEPCION]as date) as fecha,
	null as idEmpresa 
	...
	group by lx.[REFERENCIA], lx.[TIPO DE ENTREGA],lx.[FECHA DE RECEPCION],pdv.nombre,lx.[CAV],p.idProducto,p.idMarca
	...
	and (isnull(@cTipoEntrega,0) = 0 or exists(select 1 from #tipoEntrega where idTipoEntrega = te.id))
	group by lx.[REFERENCIA], lx.[TIPO DE ENTREGA],lx.[FECHA DE VENTA],pdv.nombre,lx.[CAV],p.idProducto,p.idMarca
	...
	select	CONVERT(VARCHAR(10), x.fecha, 112), 
			Convert(varchar(10),CONVERT(date,x.fecha,106),103), 
	...
	cross join (select fecha from #reporteBase group by fecha) x
	...
	group by x.fecha
	union
	select	CONVERT(VARCHAR(10), r.fecha, 112), 
			Convert(varchar(10),CONVERT(date,r.fecha,106),103), 
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by x.fecha,r.idProducto,r.nombreProducto
	union
	select	CONVERT(VARCHAR(10), r.fecha, 112), 
			Convert(varchar(10),CONVERT(date,r.fecha,106),103),
			--r.idPuntodeventa, 
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by r.fecha,r.idProducto,r.nombreProducto
	--order by r.fecha,r.nombreProducto
end
go

--exec Lanix_VentasDiariasprod_T7 147
--exec Lanix_VentasDiariasprodInforme_T7 147
---Lanix_ventasdiarias_t7 147