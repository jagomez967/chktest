IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasDiarias_T9'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasDiarias_T9] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_VentasDiarias_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (fecha, usuario, puntodeventa, producto, ventas)
	select	convert(varchar(10),t.Fecha),
	...
	group by t.Fecha, u.Apellido+', '+u.Nombre collate database_default, pdv.Nombre, p.Nombre
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Fecha',5),('usuario','Usuario',5),('puntodeventa','PuntoDeVenta',5),('producto','Producto',5),('ventas','Ventas',10)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Date',5),('usuario','User',5),('puntodeventa','PointOfSale',5),('producto','Product',5),('ventas','Sales',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select fecha, usuario, puntodeventa, producto, ventas from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select fecha, usuario, puntodeventa, producto, ventas from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select fecha, usuario, puntodeventa, producto, ventas from #datosFinal
end