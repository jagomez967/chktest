IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasMesProdInforme_T7'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasMesProdInforme_T7] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_VentasMesProdInforme_T7] 	
(
	@IdCliente			int
...
*/		
	...
	lx.[FECHA DE VENTA] as tipoDeEntrega,
...
--	and (lx.[TIPO DE ENTREGA] = 'DOMICILIO' or lx.[TIPO DE ENTREGA] = 'PRESENCIAL' or lx.[TIPO DE ENTREGA] = 'CAV')
	...
	union
	select	left(convert(varchar, x.fecha, 112),6),
			right(CONVERT(VARCHAR(11),convert(datetime,left(convert(varchar, x.fecha, 112),6)+'01'),6),6),
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by left(convert(varchar, x.fecha, 112),6)
	order by 1,3 desc
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by left(convert(varchar, r.fecha, 112),6),r.idProducto,r.nombreProducto
	union
	select	left(convert(varchar, x.fecha, 112),6),
			right(CONVERT(VARCHAR(11),convert(datetime,left(convert(varchar, x.fecha, 112),6)+'01'),6),6),
			--r.idPuntodeventa, 
	...
	where	(isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = pdv.IdZona))
		...
		and	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = r.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = r.IdProducto))
	group by left(convert(varchar, x.fecha, 112),6),r.idProducto,r.nombreProducto
...
--Lanix_VentasMesProdInforme_T7 147
