IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_VentasMesProd_T7'))
   exec('CREATE PROCEDURE [dbo].[Lanix_VentasMesProd_T7] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Lanix_VentasMesProd_T7] 	
(
	@IdCliente			int
...
*/		
	...
	select mes, right(CONVERT(VARCHAR(11),convert(datetime,mes+'01'),6),6), descr, isnull(qty, 0) from #resultados
	...
	insert #resultadosProdVentas (mes, idProducto, qty, descr)
	select m.mes, rp.idProducto, sum(isnull(rp.Cantidad,0)), @ventasTexto
	from #diasResultado m
	left join #tempReporteVentas a on a.mes = m.mes
	left join ReporteProducto rp on rp.idReporte = a.idReporte
	left join Producto p on p.idProducto = rp.idProducto
	where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = p.IdMarca))
			and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = p.IdProducto))
	group by m.mes, rp.idProducto

	insert #resultadosProdInventario (mes, idProducto, qty, descr)
	select m.mes, rp.idProducto, sum(isnull(rp.Cantidad2,0)), @inventarioTexto
	from #diasResultado m
	left join #tempReporteInventario a on a.mes = m.mes
	left join ReporteProducto rp on rp.idReporte = a.idReporte
	left join Producto p on p.idProducto = rp.idProducto
	...
	insert #datosFinal(mes, idProducto, Producto, descr, qty)
	select a.mes, a.idProducto, ltrim(rtrim(p.Nombre)), a.descr, a.qty
	...
	select mes, right(CONVERT(VARCHAR(11),convert(datetime,mes+'01'),6),6), idProducto, Producto, descr, qty from #datosFinal
