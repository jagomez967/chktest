IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_ConteoTonosMes_T3'))
   exec('CREATE PROCEDURE [dbo].[Loreal_ConteoTonosMes_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_ConteoTonosMes_T3] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #datos (idMarca, Marca, qty, mes)
	select m.idMarca, m.Nombre, count(isnull(rp.Cantidad,0)), mes from #tempReporte t
	...
	group by m.idMarca, m.Nombre, mes
	order by m.Nombre
	...
	select	right(CONVERT(VARCHAR(11),convert(datetime,r.mes),6),6) as mesStr, ltrim(rtrim(m.Nombre)) as NomMarca, isnull(r.qty,0) as qty, r.idMarca, r.mes as mes
	from #Resultados r
