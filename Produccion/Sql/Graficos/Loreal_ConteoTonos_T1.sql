IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_ConteoTonos_T1'))
   exec('CREATE PROCEDURE [dbo].[Loreal_ConteoTonos_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_ConteoTonos_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('marca','Marca'),('cant','Conteo Tonos Total')
	...
	select m.Nombre as marca, count(isnull(rp.Cantidad,0)) as cant from #tempReporte t
