IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_EspacioAnaquel_PDV_T9'))
   exec('CREATE PROCEDURE [dbo].[Loreal_EspacioAnaquel_PDV_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_EspacioAnaquel_PDV_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosMarcaPropia(idMarca, idPdv,cantidad,AnchoProducto,EspacioCategoriaCM)
	select etl.idMarca, temp.idPuntoDeVenta, rp.Cantidad, etl.PromedioAnchoProducto,etl.EspacioCategoriaCM from #tempReporte temp
	...
	insert #datosMarcaCompetencia(idMarca, idPdv,cantidad,AnchoProducto,EspacioCategoriaCM)
	select comp.idMarcaCompetencia, temp.idPuntoDeVenta,rp.cantidad,ext.AnchoPromedioProducto,mp.EspacioCategoriaCM from #tempReporte temp
	inner join reportePop rp on rp.idReporte = temp.idReporte
	...
	insert #datosRes(idMarca, idPdv,qty)	
	select d.idMarca, d.idPdv, avg((d.cantidad*d.AnchoProducto)*100.0/d.EspacioCategoriaCM) as qty from #datosMarcaPropia d
	group by d.idMarca, d.idPdv
	UNION
	select c.idMarca, c.idPdv, avg((c.cantidad*c.AnchoProducto)*100.0/c.EspacioCategoriaCM) as qty from #datosMarcaCompetencia c
	group by c.idMarca, c.idPdv
	...
	insert #DatosPivot([idPdv],'+@PivotColumnHeaders+')
	SELECT [idPdv],'+@PivotColumnHeaders+'
	  FROM (
		Select idMarca, idPdv, qty from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(qty)
		FOR idMarca IN (
	...
	insert #DatosPivotFinal ([idPdv],'+@PivotColumnHeaders+')
	select [idPdv],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'Nombre'+char(39)+','+char(39)+'PDV'+char(39)+', 80, 0)
	

	insert #columnasDef (name, title, width, orden)
	select distinct cast(i.idMarca as varchar) as name, m.Nombre as title, 40 as width, i.idMarca
	from #DatosRes i
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select pdv.Nombre,'+@PivotColumnHeaders+'
