IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_ExhibicionAdicional_T3'))
   exec('CREATE PROCEDURE [dbo].[Loreal_ExhibicionAdicional_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_ExhibicionAdicional_T3] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	insert #datos (idmarca, marcapropiedad, qty, mes)
	select mp.idMarca, mp.Nombre, count(distinct r.idPuntoDeVenta), r.mes
	from #tempReporte r
	...
	insert #Resultados (idmarca, marcapropiedad, mes, qty)
	select d.idmarca, d.marcapropiedad, te.mes, SUM(isnull(d.qty,0))
	from #MarcasMeses te
	left join #datos d on d.idmarca=te.idmarca and d.mes=te.mes-- and d.marcapropiedad = te.marcapropiedad
	group by d.idmarca, te.mes, d.marcapropiedad
	...
	select	ltrim(rtrim(m.Nombre)) as nombreMarca, ltrim(rtrim(r.marcapropiedad)) as marcaPropiedad, isnull(r.qty,0) as qty, 0, r.mes as mes
	from #Resultados r
