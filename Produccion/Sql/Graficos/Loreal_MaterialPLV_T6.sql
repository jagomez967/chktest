IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_MaterialPLV_T6'))
   exec('CREATE PROCEDURE [dbo].[Loreal_MaterialPLV_T6] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_MaterialPLV_T6] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, m.idMarca, rp.IdProducto, tmp.mes, SUM(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0))
	from Reporte r
