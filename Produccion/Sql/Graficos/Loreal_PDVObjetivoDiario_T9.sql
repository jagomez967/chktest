IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_PDVObjetivoDiario_T9'))
   exec('CREATE PROCEDURE [dbo].[Loreal_PDVObjetivoDiario_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_PDVObjetivoDiario_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (idPuntoDeVenta, nombrePdv, fecha, usuario, objetivo, cumplido)
	select	etl.idPuntoDeVenta,
			etl.Nombre,
			Convert(varchar(10),CONVERT(date,etl.Fecha,106),103),
			u.Apellido+', '+u.Nombre collate database_default,
			1,
			case when exists (select 1 from #tempReporte t where t.idUsuario = etl.idUsuario and t.idPuntoDeVenta = etl.idPuntoDeVenta and convert(date, t.Fecha) = convert(date, etl.Fecha))
			then 1 else 0 end
	from ExtData_LorealPanama_ObjetivosPDVDiarios etl
	inner join Usuario u on etl.idUsuario = u.idUsuario
	where convert(date, etl.Fecha) between convert(date, @fechaDesde) and convert(date, @fechaHasta)
	order by etl.idPuntoDeVenta
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('idPuntoDeVenta','idPuntoDeVenta',10),('nombrePdv','Nombre Pdv',50),('fecha','Fecha',20),('usuario','RTM',50),('objetivo','Objetivo',10),('cumplido','Cumplido',10)
	...
	--Datos
	if(@NumeroDePagina>0)
		select idPuntoDeVenta, nombrePdv, fecha, usuario, objetivo, cumplido from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina) order by idPuntoDeVenta, fecha asc
	
	if(@NumeroDePagina=0)
		select idPuntoDeVenta, nombrePdv, fecha, usuario, objetivo, cumplido from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina) order by idPuntoDeVenta, fecha asc
		
	if(@NumeroDePagina<0)
		select idPuntoDeVenta, nombrePdv, fecha, usuario, objetivo, cumplido from #datosFinal order by idPuntoDeVenta, fecha asc
end
go