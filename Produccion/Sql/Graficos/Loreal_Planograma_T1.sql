IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_Planograma_T1'))
   exec('CREATE PROCEDURE [dbo].[Loreal_Planograma_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_Planograma_T1] 	
(
	@IdCliente			int
...
*/		
	...
	insert #Relevados (qty)
	select count(distinct idPuntoDeVenta) from #tempReporte
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('Planograma','Planograma'),('Cumplen','PDV Cumplen'),('No Cumplen','PDV No Cumplen')
	...
	select mp.Nombre as Planograma, count(distinct r.idPuntoDeVenta) as Cumplen, r2.qty-count(distinct r.idPuntoDeVenta) as NoCumplen from #Relevados r2, #tempReporte r
