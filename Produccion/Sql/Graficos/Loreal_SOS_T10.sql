IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Loreal_SOS_T10'))
   exec('CREATE PROCEDURE [dbo].[Loreal_SOS_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Loreal_SOS_T10]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idPop, idPuntoDeVenta, mes, qty)
	select rp.idPop, r.idPuntoDeVenta, r.mes, ((sum(isnull(rp.Cantidad,0))*lap.PromAncho)/lap.EspacioCm)*100
	...
	inner join Loreal_AnaquelPropio_2018 lap on lap.idPop = rp.idPop and lap.idPuntoDeVenta = r.idPuntoDeVenta
	where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = rp.IdMarca))
			...
			and p.Nombre like '%Anaquel%'
	group by rp.idPop, r.idPuntoDeVenta, r.mes, lap.PromAncho, lap.EspacioCm
	...
	select r.idPop, SUBSTRING(p.Nombre, CHARINDEX(' ', p.Nombre) + 1, LEN(p.Nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1))
	from #Resultados r
	inner join Pop p on p.idPop = r.idPop
	group by r.idPop, SUBSTRING(p.Nombre, CHARINDEX(' ', p.Nombre) + 1, LEN(p.Nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6)
