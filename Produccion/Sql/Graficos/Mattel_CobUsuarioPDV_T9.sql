IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_CobUsuarioPDV_T9'))
   exec('CREATE PROCEDURE [dbo].[Mattel_CobUsuarioPDV_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Mattel_CobUsuarioPDV_T9
(
	@IdCliente			int
...
*/		
	...
	insert #Relevados (IdPdv, idUsuario, idReporte)
	select distinct IdPuntoDeVenta, idUsuario, idReporte from #tempReporte group by idpuntodeventa, idUsuario, idReporte
	...
	insert #resultados (idUsuario,nombre,idPdv,pdv, Semana, q)
	select u.idUsuario, u.Apellido+', '+u.Nombre collate database_default
	,r.IdPdv
	,pdv.Nombre
	,cast(DATEPART(year,rep.FechaCreacion) as varchar(8)) + '/' + cast(DATEPART(wk,rep.Fechacreacion)-1 as varchar(8))
	,1
	from #Relevados r
	inner join Usuario u on u.IdUsuario=r.idUsuario
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = r.IdPdv
	inner join Reporte rep on rep.IdReporte = r.idReporte
	group by  u.idUsuario,u.Apellido+', '+u.Nombre collate database_default, r.IdPdv,pdv.Nombre, rep.FechaCreacion
	order by  u.idUsuario,u.Apellido+', '+u.Nombre collate database_default, r.IdPdv,pdv.Nombre, rep.FechaCreacion


	insert #datosFinal (Pdv,Usuario,semana,q)
	select r.pdv, r.nombre, r.Semana, 1 from #resultados r
	...
	--exec Mattel_CobUsuarioPDV_T9 4,'M,20160301,20160331'
