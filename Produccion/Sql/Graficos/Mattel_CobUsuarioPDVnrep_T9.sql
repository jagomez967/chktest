IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_CobUsuarioPDVnrep_T9'))
   exec('CREATE PROCEDURE [dbo].[Mattel_CobUsuarioPDVnrep_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Mattel_CobUsuarioPDVnrep_T9
(
	@IdCliente			int
...
*/		
	...
	/*busco relevados*/
	insert #tempReporte (idCliente, idReporte, idUsuario, IdPuntoDeVenta, mes, semana)
	select	c.IdCliente
			,r.IdReporte
			,r.IdUsuario
			,r.IdPuntoDeVenta
			,FechaCreacion
			,cast(DATEPART(year,FechaCreacion) as varchar(8)) + '/' + right('0' + cast(DATEPART(wk, FechaCreacion)-1 as varchar(8)),2)
	from reporte r
	...
	insert #Relevados (IdPdv, idUsuario, idReporte, Semana)
	select distinct IdPuntoDeVenta, idUsuario, idReporte, semana
	from #tempReporte t group by t.idpuntodeventa, t.idUsuario, t.idReporte, t.semana

	insert #NoRelevados (idPdv, idUsuario, Semana)
	select distinct t.IdPuntoDeVenta, t.idUsuario, tr.semana from #tempPCU t, #tempReporte tr
	where (not exists (select rel.IdPdv, rel.idUsuario, rel.Semana from #Relevados rel where (rel.IdPdv = t.IdPuntoDeVenta and rel.idUsuario = t.idUsuario and rel.Semana = tr.semana)))
	...
	insert #datosFinal (Pdv,Usuario,semana,q)
	select ltrim(rtrim(pdv.nombre)), u.Apellido+', '+ u.Nombre  COLLATE database_default, nr.Semana, 1 from #NoRelevados nr
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = nr.idPdv
	inner join Usuario u on u.IdUsuario = nr.idUsuario
	order by usuario, nr.idpdv, nr.Semana
	...
	insert #columnasConfiguracion (name, title, width) values ('pdv','PuntoDeVenta',5),('usuario','Coordinador',5),('semana','Semana',5),('q','Q',10)
	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select pdv, usuario, semana, q from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select pdv, usuario, semana, q from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select pdv, usuario, semana, q from #datosFinal

	--exec Mattel_CobUsuarioPDVnrep_T9 4,'M,20160401,20160430'
