IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_ExhAdicional_T2'))
   exec('CREATE PROCEDURE [dbo].[Mattel_ExhAdicional_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Mattel_ExhAdicional_T2 	
(
	@IdCliente			int
...
*/		
	...
	insert #datosRes (empresa, total)
	select ltrim(rtrim(e.Nombre)), sum(d.valor) from #datos d
	...
	--exec Mattel_HotSpots_T2 4,'M,20160301,20160331'
