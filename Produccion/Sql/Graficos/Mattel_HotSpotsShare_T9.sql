IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_HotSpotsShare_T9'))
   exec('CREATE PROCEDURE [dbo].[Mattel_HotSpotsShare_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Mattel_HotSpotsShare_T9
(
	@IdCliente			int
...
*/		
	...
	insert #datosRes (empresa, total, valor)
	select ltrim(rtrim(e.Nombre)), sum(d.valor), cast(sum(d.valor)*100.0/@total as numeric(10,2)) from #datos d
	...
	insert #columnasConfiguracion (name, title, width) values ('empresa','Fabricante',5),('total', 'Q',5), ('valor', '%',10)
	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select empresa, total, valor from #datosRes where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select empresa, total, valor from #datosRes where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select empresa, total, valor from #datosRes


	--exec Mattel_HotSpotsShare_T9 4,'M,20160301,20160331'10
end