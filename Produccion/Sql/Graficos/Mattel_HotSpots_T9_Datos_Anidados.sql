IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_HotSpots_T9_Datos_Anidados'))
   exec('CREATE PROCEDURE [dbo].[Mattel_HotSpots_T9_Datos_Anidados] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].Mattel_HotSpots_T9_Datos_Anidados
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (esclave, mostrar, name, title, width) values (0,1,'nested1','Producto',30),(0,1,'nested2', 'Q',20), (0,1,'nested3', '%',20)

	if(@lenguaje='en')
		insert #columnasConfiguracion (esclave, mostrar, name, title, width) values (0,1,'nested1','Product',30),(0,1,'nested2', 'Q',20), (0,1,'nested3', '%',20)

	select esclave, mostrar, name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select nested1, nested2, nested3 from #datosRes where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select nested1, nested2, nested3 from #datosRes where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
