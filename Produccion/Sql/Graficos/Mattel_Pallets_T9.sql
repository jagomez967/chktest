...
*/		
	...
	--Datos
	if(@NumeroDePagina>0)
		select usuario, cadena, pdv, valor, pallets from #datosRes where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select usuario, cadena, pdv, valor, pallets from #datosRes where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
