IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_QuiebreProductos_T1'))
   exec('CREATE PROCEDURE [dbo].[Mattel_QuiebreProductos_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Mattel_QuiebreProductos_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('producto','Product'),('cantidad','Qty')

	select name, title from #columnasConfiguracion

	
	select ltrim(rtrim(p.Nombre)), count(rp.Stock)
	from Reporte r
