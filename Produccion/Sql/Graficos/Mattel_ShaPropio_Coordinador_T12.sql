IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_ShaPropio_Coordinador_T12'))
   exec('CREATE PROCEDURE [dbo].[Mattel_ShaPropio_Coordinador_T12] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[Mattel_ShaPropio_Coordinador_T12]
(
	@IdCliente			int
...
*/		
	...
	select r.idempresa, r.idpuntodeventa, left(convert(varchar,r.fechacreacion,112),6), r.idreporte, r.IdUsuario
	from Reporte r
	...
	insert #datosPeriodoAnterior(idEmpresa, idUsuario, valor)
	select r.idempresa, r.idUsuario, sum(isnull(rp.precio,0))
	...
	inner join cliente c on c.IdEmpresa=r.IdEmpresa
	where	isnull(rp.precio,0)>0
	group by  m.idempresa, r.idUsuario
	...
	create table #sharePorCoordinadorAnterior
	(
		idEmpresa int,
		idCoordinador int,
		valor decimal(10,2),
	...
	insert #sharePorCoordinador (idEmpresa, idCoordinador, valor, varianza)
	select d.idEmpresa, d.idUsuario, d.valor*100.0/(select sum(#datos.valor) from #datos where #datos.idUsuario=d.idUsuario), 0
	from #datos d
	inner join cliente c on c.IdEmpresa=d.idempresa
	where c.IdCliente=@IdCliente

	insert #sharePorCoordinadorAnterior (idEmpresa, idCoordinador, valor, varianza)
	select d.idEmpresa, d.idUsuario, d.valor*100.0/(select sum(#datosPeriodoAnterior.valor) from #datosPeriodoAnterior where #datosPeriodoAnterior.idUsuario=d.idUsuario), 0
	...
	from #sharePorCoordinador A, #sharePorCoordinadorAnterior B
	where A.idEmpresa = B.idEmpresa and A.idCoordinador = B.idCoordinador

	select d.id as id, '7EA568' as color, u.Imagen as logo, 0 as nivel, d.valor as valor, isnull(d.varianza,0) as varianza, 0 as parentId,
	u.Apellido+', '+u.Nombre collate database_default as nombre
	from #sharePorCoordinador d
	inner join Empresa e on e.IdEmpresa = d.idempresa
	inner join Usuario u on u.IdUsuario = d.idCoordinador
	order by d.id
end
