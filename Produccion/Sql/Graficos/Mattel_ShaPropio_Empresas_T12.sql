IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Mattel_ShaPropio_Empresas_T12'))
   exec('CREATE PROCEDURE [dbo].[Mattel_ShaPropio_Empresas_T12] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[Mattel_ShaPropio_Empresas_T12]
(
	@IdCliente			int
...
*/		
	...
	insert #datos(idEmpresa, valor)
	select r.idempresa, sum(isnull(rp.precio,0))
	...
	insert #datos (idEmpresa, valor)
	select m.idempresa, sum(isnull(rp.precio,0))
	from #reportesMesPdv r
	...
	insert #datosPeriodoAnterior(idEmpresa, valor)
	select r.idempresa, sum(isnull(rp.precio,0))
	...
	select d.id as id, '7EA568' as color, e.Logo as logo, 0 as nivel, d.valor as valor, isnull(d.varianza,0) as varianza, 0 as parentId,ltrim(rtrim(e.Nombre)) as nombre,d.idempresa as empresa
	from #datos d
	inner join Empresa e on e.IdEmpresa = d.idempresa
	order by d.nivel, d.parentId, d.id

	--exec Mattel_ShaPropio_Empresas_T12 4, 'M,20160301,20160331'
end
