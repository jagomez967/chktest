IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.MedicoAusente_T1'))
   exec('CREATE PROCEDURE [dbo].[MedicoAusente_T1] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[MedicoAusente_T1]
(
	@IdCliente			int
...
*/		
	...
	where	--convert(date,FechaCreacion) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
			month(r.fechaCreacion) between month(@fechaDesde) and month(@fechaHasta)
			and year(r.fechaCreacion) between year(@fechaDesde) and year(@fechaHasta)
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('ma','Usuario'),('qty','Cantidad')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('ma','User'),('qty','qty')

	select name, title from #columnasConfiguracion


	select u.apellido+ ', ' + u.nombre collate database_default as ma,
	       count(r.idReporte) as qty
	from #tempReporte r
