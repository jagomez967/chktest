IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_EvolPopCadena_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_EvolPopCadena_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_EvolPopCadena_T10]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idCadena, mes, qty)
	select tmp.idCadena, tmp.mes, SUM(isnull(rp.Cantidad,0))
	from Reporte r
