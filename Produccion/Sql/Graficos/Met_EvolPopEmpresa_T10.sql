IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_EvolPopEmpresa_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_EvolPopEmpresa_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_EvolPopEmpresa_T10]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idPOP, mes, qty)
	select rp.idPOP, tmp.mes, SUM(isnull(rp.Cantidad,0))
    from Reporte r
	...
	insert #POPMeses (idPOP, mes)
	select distinct tmp.idPOP, m.mes
	from #datos tmp
	inner join #Meses m on 1=1
	
	insert #Resultados (idPOP, mes, qty)
	select te.idPOP, te.mes, SUM(isnull(d.qty,0))
	from #POPMeses te
	left join #datos d on d.idPOP=te.idPOP and d.mes=te.mes
	group by te.idPOP, te.mes
	...
	select r.idPOP, ltrim(rtrim(c.Nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1))
	from #Resultados r
	inner join POP c on c.idPOP = r.idPOP
	group by r.idPOP, ltrim(rtrim(c.Nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6)
	order by r.idPOP, r.mes
end
go

-- Met_EvolPopEmpresa_T10 235