IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_EvolPopMarcaEpson_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_EvolPopMarcaEpson_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_EvolPopMarcaEpson_T10]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idpop, mes, qty,idReporte,IdMarca)
	select rp.idpop, tmp.mes, SUM(isnull(rp.Cantidad,0)),rp.idReporte,m.IdMarca
	from Reporte r
	...
	group by rp.idpop, tmp.mes,rp.idReporte,m.IdMarca
	...
	insert #MarcasMeses (idpop, mes,IdMarca)
	select distinct tmp.idpop, m.mes,tmp.idMarca
	from #datos tmp
	inner join #Meses m on 1=1
	
	--select top 10 * from #MarcasMeses 
	
	insert #Resultados (idpop, mes, qty,IdMarca)
	select te.idpop, te.mes, SUM(isnull(d.qty,0)),te.IdMarca
	from #MarcasMeses te
	left join #datos d on d.idpop=te.idpop and d.mes=te.mes and d.idMarca = te.IdMarca
	group by te.idpop, te.mes,te.IdMarca
	order by te.idpop, te.mes
	...
	select  cast(p.idpop as varchar(200)) + cast(m.idMarca as varchar(200)), ltrim(rtrim(p.Nombre)) + ' ' + m.nombre ,r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1))
	from #Resultados r
	inner join marca m on m.idMarca = r.idMarca 
	inner join pop p on p.idpop = r.idpop
	group by p.idpop, p.Nombre,m.nombre, r.mes,m.idmarca
	order by p.Nombre, r.mes
...
----insert into @p2 values(N'fltpuntosdeventa',N'177935')
-----insert into @p2 values(N'fltusuarios',N'4643')
----insert into @p2 values(N'fltMarcas',N'614')
--
--exec Met_EvolPopMarcaEpson_T10 @IdCliente=222,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'