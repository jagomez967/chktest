IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PopCadenaMarca_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_PopCadenaMarca_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_PopCadenaMarca_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #datosFinal (idCadena, Cadena, qty,marca)
	select t.idCadena, c.Nombre, sum(isnull(rp.Cantidad,0)),m.nombre 
	from #tempReporte t
	...
	and rp.idpop = 2600
	group by t.idCadena, c.Nombre,m.nombre
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (esclave, mostrar, name, title, width) values (1,0,'idCadena','idCadena',0), (0,1,'Cadena','Cadena',5),(0,1,'marca','Marca',5),
		(0,1,'qty','Cantidad',5)
	...
	--Datos
	if(@NumeroDePagina>0)
		select idCadena, Cadena, marca,qty from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina) order by id
	
	if(@NumeroDePagina=0)
		select idCadena, Cadena,marca, qty from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina) order by id
		
	if(@NumeroDePagina<0)
		select idCadena, Cadena,marca, qty from #datosFinal order by id
end
--go
...
--
--exec Met_PopCadenaMarca_T9 @IdCliente=222,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
