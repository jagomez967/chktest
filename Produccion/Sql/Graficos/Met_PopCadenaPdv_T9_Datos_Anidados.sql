...
*/		
	...
	--Datos
	if(@NumeroDePagina>0)
		select nested1, nested2 from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina) order by id
	
	if(@NumeroDePagina=0)
		select nested1, nested2 from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina) order by id
