IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PopxPdv_T3'))
   exec('CREATE PROCEDURE [dbo].[Met_PopxPdv_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_PopxPdv_T3]
(
	@IdCliente			int
...
*/		
	...
	select ltrim(rtrim(pdv.nombre)), ltrim(rtrim(p.nombre)), isnull(r.qty,0), r.idPop, r.mes
	from #Resultados r
	inner join Pop p on p.IdPop = r.idPop
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = r.idPuntoDeVenta
	order by 1,r.mes, p.IdPop
