IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PrecioComp_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_PrecioComp_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_PrecioComp_T9] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idMarca, idProducto, mes, minimo, maximo, promedio)
	select r.idEmpresa, m.idMarca, rp.IdProducto, left(convert(varchar,r.fechacreacion,112),6),min(isnull(rp.precio,0)), max(isnull(rp.precio,0)), avg(isnull(rp.precio,0))
	from Reporte r
	...
	insert #datosFinal (producto, mes, minimo, maximo, promedio)
	select ltrim(rtrim(p.Nombre)) as Producto, right(CONVERT(VARCHAR(11),convert(datetime,m.mes+'01'),6),6) as Mes, cast(isnull(d.minimo,0) as numeric(18,1)) as Minimo, cast(isnull(d.maximo,0) as numeric(18,1)) as Maximo, cast(isnull(d.promedio,0) as numeric(18,1)) as Promedio
	...
	order by ltrim(rtrim(p.Nombre)), right(CONVERT(VARCHAR(11),convert(datetime,m.mes+'01'),6),6)
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('producto','Producto',5),('mes','Mes',5),('minimo','Min',5),('maximo','Max',5),('promedio','Promedio',10)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('producto','Product',5),('mes','Month',5),('minimo','Min',5),('maximo','Max',5),('promedio','Avg',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select producto, mes, minimo, maximo, promedio from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select producto, mes, minimo, maximo, promedio from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select producto, mes, minimo, maximo, promedio from #datosFinal
end
GO
...
exec met_precioComp_t9 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_precioComp_t9 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/