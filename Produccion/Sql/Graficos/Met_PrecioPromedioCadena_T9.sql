IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PrecioPromedioCadena_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_PrecioPromedioCadena_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[Met_PrecioPromedioCadena_T9]
	(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, r.idCadena, rp.idProducto, avg(isnull(rp.Precio,0)) from #reportesMesPdv r
	inner join ReporteProducto rp on rp.IdReporte = r.idReporte
	inner join Producto p on p.IdProducto = rp.IdProducto
	inner join Marca m on m.IdMarca = p.IdMarca
	where isnull(rp.Precio,0)>0
	group by r.idEmpresa, r.idCadena, rp.IdProducto
	...
	insert #datosRes (idcadena, cadena, idproducto, precio)
	select 'col'+cast(d.idcadena as varchar(8)), ltrim(rtrim(c.Nombre)), d.idProducto, d.precio
	from #datos d
	...
	SELECT [idProducto],'+@PivotColumnHeaders+'
	  FROM (
		Select idProducto, idCadena, precio from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(precio)
		FOR idcadena IN (
	...
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select ltrim(rtrim(p.Nombre)) as producto, ltrim(rtrim(m.Nombre)) as marca, '+@PivotColumnHeaders+'
