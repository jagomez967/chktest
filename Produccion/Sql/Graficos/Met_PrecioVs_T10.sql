IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PrecioVs_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_PrecioVs_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_PrecioVs_T10] 	
(
	@IdCliente			int
...
*/		
	...
	select rp.idProducto, r.mes, avg(isnull(rp.Precio,0))
	...
	create table #productostmp
	(
		idProducto int
	)
	insert #productostmp select distinct idproducto from #datos
	...
	inner join #productostmp prop on prop.idProducto = pc.IdProducto
	inner join Marca m on m.IdMarca = p.IdMarca
	...
	insert #ProductosMeses (idProducto, mes)
	select distinct tmp.idProducto, m.mes
	from #datos tmp
	inner join #Meses m on 1=1
	
	insert #Resultados (idProducto, mes, qty)
	select te.idProducto, te.mes, SUM(isnull(d.qty,0))
	from #ProductosMeses te
	left join #datos d on d.idProducto=te.idProducto and d.mes=te.mes
	group by te.idProducto, te.mes
	order by te.idProducto, te.mes
	...
	select r.idProducto, ltrim(rtrim(p.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1))
	from #Resultados r
	inner join Producto p on p.idProducto = r.idProducto
	group by r.idProducto, ltrim(rtrim(p.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6)
	order by r.idProducto, r.mes
end
GO
...
exec Met_PrecioVs_T10 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec Met_PrecioVs_T10 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/