IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_PrecioVs_T3'))
   exec('CREATE PROCEDURE [dbo].[Met_PrecioVs_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_PrecioVs_T3] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idMarca, idProducto, mes, qty)
	select r.idEmpresa, m.idMarca, rpc.IdProducto, left(convert(varchar,r.fechacreacion,112),6), avg(isnull(rpc.precio,0))
	from Reporte r
	inner join PuntoDeVenta pdv on pdv.IdPuntoDeVenta = r.IdPuntoDeVenta
	...
	inner join #productostmp prop on prop.idProducto = p.IdProducto
			...
			and (isnull(@cCompetenciaPrimaria,0) = 0 
					or exists (select 1 from #CompetenciaPrimaria f 
								...
								where f.idMarca = m.idMarca 
									and mct.idMarcaCompetencia = (select max(isnull(idmarca,-1))from producto where idProducto = rpc.idProducto) 
								and isnull(mct.esCompetenciaPrimaria,0) = 1))
			and (isnull(@cProvincias,0) = 0 or exists(select 1 from #provincias pro where pro.idProvincia in(select idProvincia from localidad loc where loc.idLocalidad = pdv.idLocalidad)))
					and (isnull(@cEquipo,0) = 0 or exists(select 1 from puntodeventa_Vendedor pve2 where pve2.idPuntodeventa = pdv.idPuntodeventa  and pve2.idVendedor
			 in(select idVendedor from vendedor where idEquipo in(select idEquipo from #Equipo))))
	
	group by r.idEmpresa, m.idMarca, rpc.IdProducto, left(convert(varchar,r.fechacreacion,112),6)
...
exec met_precio_t3 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_precio_t3 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/
