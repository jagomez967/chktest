IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Precio_T1'))
   exec('CREATE PROCEDURE [dbo].[Met_Precio_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Precio_T1] 	
(
	@IdCliente			int
...
*/		
	...
	left(convert(varchar, dateadd(day, -(day(r.FechaCreacion) - 1), FechaCreacion),112),6) as mes
	, avg(isnull(rp.precio,0))
	from Reporte r
