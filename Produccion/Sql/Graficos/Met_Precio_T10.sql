IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Precio_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_Precio_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Precio_T10]
(
	@IdCliente			int
...
*/		
	...
	select m.idProducto, ltrim(rtrim(p.Nombre)), m.mes, right(CONVERT(VARCHAR(11),convert(datetime,m.mes+'01'),6),6), isnull(d.qty,0)
...
exec met_precio_t10 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_precio_t10 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/