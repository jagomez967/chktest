IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Precio_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_Precio_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Precio_T9] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('producto','Producto',5),('mes','Mes',5),('minimo','Mínimo',5),('maximo','Máximo',5),('promedio','Promedio',10)
...
exec met_precio_t9 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_precio_t9 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/