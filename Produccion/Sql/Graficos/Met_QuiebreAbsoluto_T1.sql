IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_QuiebreAbsoluto_T1'))
   exec('CREATE PROCEDURE [dbo].[Met_QuiebreAbsoluto_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_QuiebreAbsoluto_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('marca','Marca'),('cantpdvs','Cant Pdvs')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('marca','Brand'),('cantpdvs','Number of POS')

	select name, title from #columnasConfiguracion


	select m.Nombre, count(distinct t.idPuntoDeVenta)
	from #tempReporte t
	...
	where not exists
	(select 1 from 
		(select rp.idreporte, p.idmarca, Stock
		from reportePRoducto rp
		inner join producto p on p.idproducto = rp.idproducto) x
	where x.idreporte = rp.idreporte and p.idmarca = x.idmarca and isnull(Stock,0) = 0)
