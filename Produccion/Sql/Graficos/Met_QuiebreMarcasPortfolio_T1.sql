IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_QuiebreMarcasPortfolio_T1'))
   exec('CREATE PROCEDURE [dbo].[Met_QuiebreMarcasPortfolio_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_QuiebreMarcasPortfolio_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('marca','Marca'),('asignados','Cant Pdvs')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('marca','Brand'),('asignados','Number of POS')

	select name, title from #columnasConfiguracion

	select m.Nombre, count(distinct r.IdPuntoDeVenta) as CantPdvs
	from Reporte r
	inner join ReporteProducto rp on rp.IdReporte=r.IdReporte
	inner join ETL_Andromaco_Portfolio por on por.IdPuntoDeVenta=r.IdPuntoDeVenta and por.idProducto = rp.idProducto
	inner join PuntoDeVenta pdv on pdv.idPuntoDeVenta = por.idPuntoDeVenta
	inner join producto p on p.idproducto = por.idproducto
...
GO
MET_QUIEBREMARCASPORTFOLIO_T1 44
