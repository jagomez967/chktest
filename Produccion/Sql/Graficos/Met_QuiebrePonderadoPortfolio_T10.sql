IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_QuiebrePonderadoPortfolio_T10'))
   exec('CREATE PROCEDURE [dbo].[Met_QuiebrePonderadoPortfolio_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_QuiebrePonderadoPortfolio_T10]
(
	@IdCliente			int
...
*/		
	...
	select mes, count(distinct idPuntoDeVenta) from #reportesMesPdv group by mes


	select 1, 'Quiebre Lineal', r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), round(count(distinct r.IdPuntoDeVenta)*100.00/rel.qty,2,0) as CantPdvs from #reportesMesPdv r
	...
	inner join #Relevados rel on rel.mes = r.mes
	inner join Producto p on p.idProducto = rp.idProducto
	inner join Reporte rep on rep.idReporte = r.idReporte
	where isnull(rp.stock,0)>0
			and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = p.IdProducto))
	group by r.mes, rel.qty
	union all
	select 2, 'Quiebre Ponderado', r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), round(sum(etl2.Peso)/rel.qty,2,0) as CantPdvs from #reportesMesPdv r
	inner join ReporteProducto rp on r.idReporte = rp.idReporte
	inner join ETL_Andromaco_PesoQuiebre etl2 on etl2.idProducto = rp.IdProducto
	inner join Producto p on p.idProducto = rp.idProducto
	inner join #Relevados rel on rel.mes = r.mes
	where isnull(rp.Stock,0)>0
			and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = p.IdProducto))
...
go
--MET_QuiebrePonderadoPortFolio_T10 44