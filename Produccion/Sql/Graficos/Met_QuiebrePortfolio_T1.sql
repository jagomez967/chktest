IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_QuiebrePortfolio_T1'))
   exec('CREATE PROCEDURE [dbo].[Met_QuiebrePortfolio_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_QuiebrePortfolio_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('asignados','Cant Pdvs')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('asignados','Number of POS')

	select name, title from #columnasConfiguracion

	select left(convert(varchar,r.fechacreacion,112),6), count(distinct r.IdPuntoDeVenta) as CantPdvs
	from Reporte r
	inner join ReporteProducto rp on rp.IdReporte=r.IdReporte
	inner join ETL_Andromaco_Portfolio pr on pr.IdPuntoDeVenta=r.IdPuntoDeVenta and pr.idProducto = rp.idProducto
	inner join PuntoDeVenta pdv on pdv.idPuntoDeventa = pr.idPuntoDeVenta
