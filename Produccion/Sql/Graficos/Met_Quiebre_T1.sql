IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Quiebre_T1'))
   exec('CREATE PROCEDURE [dbo].[Met_Quiebre_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Quiebre_T1] 	
(
	@IdCliente			int
...
exec met_quiebre_t1 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_quiebre_t1 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/
