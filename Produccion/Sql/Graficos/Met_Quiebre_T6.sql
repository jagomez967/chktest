IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Quiebre_T6'))
   exec('CREATE PROCEDURE [dbo].[Met_Quiebre_T6] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Quiebre_T6] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idMarca, idProducto, idpuntodeventa, mes)
	select r.idEmpresa, p.idMarca, rp.IdProducto, r.idpuntoDeVenta, left(convert(varchar,r.fechacreacion,112),6)
	from Reporte r
	...
	select te.idEmpresa, te.idMarca, te.mes, count(distinct d.idpuntodeventa)
	from #MarcasMeses te
	...
	select te.idEmpresa, te.idmarca, te.idProducto, te.mes, COUNT(distinct d.idpuntodeventa)
...
exec met_quiebre_T6 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec met_quiebre_t6 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/
