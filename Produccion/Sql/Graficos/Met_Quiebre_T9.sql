IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_Quiebre_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_Quiebre_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_Quiebre_T9] 	
(
	@IdCliente			int
...
*/		
	...
	if(@cProductos + @cMarcas + @cFamilia + @cCompetenciaPrimaria + @cSubCategoria + @cCategoriaProducto> 0)
	BEGIN
		...
		set @cProductos = @cProductos + @cMarcas + @cFamilia+ @cCompetenciaPrimaria + @cSubCategoria + @cCategoriaProducto			
	END
	...
	insert #datosFinal (fecharep, pdv, direccion, cadena, zona, rtm, marca, producto, equipo, vendedor,idReporte)
	select	convert(varchar(10),rd.fechaCreacion ,103)as fechaRep
	...
	inner join reporte rd on rd.idReporte = r.idReporte 
	...
	order by convert(varchar(10), rd.fechaCreacion,103), r.IdPuntoDeVenta, m.IdMarca, p.IdProducto 
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('fecharep','Fecha',5),('pdv','PDV',5),('direccion','Dirección',5),('cadena','Cadena',5),('zona','Zona',5),('rtm','RTM',5),('marca','Marca',5),('producto','Producto',10),('equipo','Equipo',10),('vendedor','Vendedor',10)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('fecharep','Date',5),('pdv','POS',5),('direccion','Address',5),('cadena','Retail',5),('zona','Zone',5),('rtm','RTM',5),('marca','Branch',5),('producto','Product',10),('equipo','Team',10),('vendedor','Salesman',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select fecharep, pdv, direccion, cadena, zona, rtm, marca, producto, equipo, vendedor from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select fecharep, pdv, direccion, cadena, zona, rtm, marca, producto, equipo, vendedor from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select fecharep, pdv, direccion, cadena, zona, rtm, marca, producto, equipo, vendedor from #datosFinal
