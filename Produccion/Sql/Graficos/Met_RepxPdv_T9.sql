IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_RepxPdv_T9'))
   exec('CREATE PROCEDURE [dbo].[Met_RepxPdv_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Met_RepxPdv_T9]
(
	@IdCliente			int
...
*/		
	...
	and (isnull(@cTipoPDV,0) = 0 or exists(select 1 from #TipoPDV where idTipo = p.idTipo))
		and (isnull(@cProvincias,0) = 0 or exists(select 1 from #provincias pro where pro.idProvincia in(select idProvincia from localidad loc where loc.idLocalidad = p.idLocalidad)))

	insert #RelevadosxDia (dia, IdUsuario, qty)
	select mes, idUsuario, count(distinct(IdPuntoDeVenta)) from #tempReporte
	group by mes, idUsuario
	order by mes, idUsuario


	insert #datosFinal (fecha, usuarionombre, qty)
	select	r.dia as Fecha
			,u.Apellido+', '+u.Nombre collate database_default as Usuario
			,r.qty as Relevados
	from #RelevadosxDia r
	inner join Usuario u on u.IdUsuario=r.idUsuario
	order by r.dia desc, u.Apellido
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Fecha',5),('usuarionombre','Usuario',5),('qty','Relevados',10)

	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title, width) values ('fecha','Date',5),('usuarionombre','User',5),('qty','Visited',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select convert(varchar,fecha,103), usuarionombre, qty from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select convert(varchar,fecha,103), usuarionombre, qty from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select convert(varchar,fecha,103), usuarionombre, qty from #datosFinal
