IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Met_TimelineAudit_T17'))
   exec('CREATE PROCEDURE [dbo].[Met_TimelineAudit_T17] AS BEGIN SET NOCOUNT ON; END')
GO  --Met_TimelineAudit_T17 9
alter procedure [dbo].[Met_TimelineAudit_T17] 	
(
	@IdCliente			int
...
select * from
	(
	select u.IdUsuario as IdUsuario, u.Apellido as Apellido, u.Nombre as Nombre, el.FechaHora as Fecha, e.Nombre as AccionTipo, 'Cambio de estado' as Descripcion,'' as Cliente
	from Usuario u
	...
	union all
	select u.IdUsuario as IdUsuario, u.Apellido as Apellido, u.Nombre as Nombre, r.FechaCreacion as Fecha, 'Reporte' as AccionTipo, 'Carga de Reporte Id: '+str(r.idreporte) + ';' + isnull(p.nombre,'') as Descripcion, c.nombre as Cliente
	from Usuario u
	...
	union all
	select u.IdUsuario as IdUsuario, u.Apellido as Apellido, u.Nombre as Nombre, ou.Fecha as Fecha, o.Descripcion as AccionTipo, 'Inicio o Cierre de Sesion' as Descripcion,c.nombre as Cliente
	from Usuario u
	inner join Operacion_Usuario ou on ou.IdUsuario=u.IdUsuario
	inner join Operacion o on o.IdOperacion=ou.IdOperacion
	inner join Usuario_Cliente uc on uc.IdUsuario=u.IdUsuario
	inner join cliente c on c.idCliente = uc.idCliente
	where convert(date,ou.Fecha) between convert(date,@fechaDesde) and convert(date,@fechaHasta)
