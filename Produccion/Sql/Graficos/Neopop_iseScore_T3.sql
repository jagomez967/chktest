IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Neopop_iseScore_T3'))
   exec('CREATE PROCEDURE [dbo].[Neopop_iseScore_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure Neopop_iseScore_T3
(
	@IdCliente			int
...
*/		
	...
	select ltrim(rtrim(M.Nombre)) as Marca, 'Availability', isnull(A.total,0) as [Availability], m.IdMarca, 'Availability'
	from #availability A
	inner join Marca m on m.IdMarca=A.idMarca
	union all
	select ltrim(rtrim(M.Nombre)) as Marca, 'Pricing', isnull(P.total,0) as [Pricing], m.IdMarca, 'Pricing'
	from #pricing P
	inner join Marca m on m.IdMarca=P.idMarca
	union all
	select ltrim(rtrim(M.Nombre)) as Marca, 'Merchandising', isnull(S.total,0)*0.5 + isnull(PT.total,0)*0.25 + isnull(PM.total,0)*0.25 as [Merchandising], m.idmarca, 'Merchandising'
	from #share S
	left join #prodtag PT on PT.idMarca=S.idMarca
	left join #popMaterial PM on PM.idMarca=S.idMarca
	inner join Marca m on m.IdMarca=S.idMarca


	--exec Neopop_iseScore_T3 1, 'M,00010101,00010101', null, null, null, null, null, null, '380'
end