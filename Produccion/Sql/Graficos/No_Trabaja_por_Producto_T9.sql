IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.No_Trabaja_por_Producto_T9'))
   exec('CREATE PROCEDURE [dbo].[No_Trabaja_por_Producto_T9] AS BEGIN SET NOCOUNT ON; END')
GO 
alter PROCEDURE [dbo].[No_Trabaja_por_Producto_T9] 	
(
	@IdCliente			int
...
begin
	...
	*/		
	...
	insert #tempReporte (idPuntoDeVenta,  mes, idReporte, nombre, apellido)
	select distinct r.idpuntodeventa, left(convert(varchar,r.fechacreacion,112),8), max(r.idreporte), ltrim(rtrim(u.nombre)), ltrim(rtrim(u.apellido)) 
	from Reporte r
	...
	group by r.idpuntodeventa, r.IdEmpresa, left(convert(varchar,r.fechacreacion,112),8), ltrim(rtrim(u.nombre)),ltrim(rtrim(u.apellido))
	
	
	
	/* Se inserta el universo  pdv, marca, producto dentro de la tabla DatosFinal*/
	
	insert #datosFinal (pdv, marca, producto,localidad, direccion,nombre, apellido,fecha)
	...
	if(@lenguaje='es')
		...
		('pdv','pdv',5,1),('nombre','nombre',5,2),('apellido','apellido',5,3),
		('direccion','direccion',5,4),('localidad','localidad',5,5),
		('producto','producto',5,6),('marca','marca',5,7),('fecha','fecha',5,8)
	...
	----Datos
	if(@NumeroDePagina>0)
		select pdv, nombre,apellido, direccion,localidad, producto,marca,fecha
