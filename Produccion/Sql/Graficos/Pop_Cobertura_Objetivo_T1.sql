IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pop_Cobertura_Objetivo_T1'))
   exec('CREATE PROCEDURE [dbo].[Pop_Cobertura_Objetivo_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pop_Cobertura_Objetivo_T1] 	
(
	@IdCliente			int
...
*/		
	...
	insert #reportesMesPdv (IdPuntoDeVenta, idUsuario, idReporte, mes)
	...
	where c.IdCliente=@IdCliente
			...
			and (isnull(@cTipoRtm,0) = 0 or exists(select 1 from UsuarioPerfil upe where upe.IdUsuario=r.IdUsuario and upe.IdPerfil in (select IdTipoRTM from #tipoRtm)))
			and (isnull(@cTipoPDV,0) = 0 or exists(select 1 from #TipoPDV where idTipo = p.idTipo))
	group by r.idPuntoDeVenta, r.idUsuario, convert(varchar, dateadd(day, -(day(FechaCreacion) - 1), FechaCreacion),112)
	...
	insert #datosRes (idPuntoDeVenta, idPop, Pop, qty,mes,idmarca)
	select r.idPuntoDeVenta, rp.idPop, p.Nombre, sum(isnull(rp.Cantidad,0)),r.mes, m.idMarca
	...
	inner join Pop_Marca pm on m.idMarca = pm.idMarca
	where isnull(rp.Cantidad,0) > 0
	and (ISNULL(@cMarcas,0) = 0 OR EXISTS(SELECT 1 FROM #marcas WHERE idMarca = M.IdMarca))
	group by r.idPuntoDeVenta, rp.idPop, p.Nombre,r.mes,m.idMarca
	
	--select * from #datosRes
	
	create table #datosRes1
	(qty int,
	mes varchar(8))
	
	insert #datosRes1 (qty,mes)
	select count(distinct idPuntoDeVenta),mes
	from #datosRes
	group by  mes
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('Fecha','Fecha'),('Objetivo','Objetivo'),('Cobertura','Cobertura'),('Pop','Pop')
	...
	select 
	Convert(varchar(10),CONVERT(date,a.mes,106),103) as Fecha,
	b.qty as Objetivo, 
	c.qty as Cobertura, 
	a.qty as Pop
	from #datosRes1 a 
	inner join #Asignados b on a.mes = b.mes
    inner join #Relevados c on a.mes = c.mes
...
--exec Pop_Cobertura_Objetivo_T1 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
