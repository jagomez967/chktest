IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_Cobertura_T1'))
   exec('CREATE PROCEDURE [dbo].[Pper_Cobertura_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_Cobertura_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('zona','Zona'),('qty','Cobertura')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('zona','Zone'),('qty','Coverage')
