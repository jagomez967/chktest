IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_ObjetivoLoc_T1'))
   exec('CREATE PROCEDURE [dbo].[Pper_ObjetivoLoc_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_ObjetivoLoc_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('ciudad','Ciudad'),('objetivo','Universo')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('ciudad','City'),('objetivo','Universe')
