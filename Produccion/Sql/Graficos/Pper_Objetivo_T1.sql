IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_Objetivo_T1'))
   exec('CREATE PROCEDURE [dbo].[Pper_Objetivo_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_Objetivo_T1] 	
(
	@IdCliente			int
...
*/		
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('zona','Zona'),('objetivo','Universo')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('zona','Zone'),('objetivo','Universe')

	select name, title from #columnasConfiguracion

	select	ltrim(rtrim(z.Nombre)) as zona,
			COUNT(distinct a.idPuntoDeVenta) as objetivo
	from #asignados a
