IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_PorcentajeCob_T1'))
   exec('CREATE PROCEDURE [dbo].[Pper_PorcentajeCob_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_PorcentajeCob_T1] 	
(
	@IdCliente			int
...
*/		
	...
	insert #tempReporte (idCliente, idUsuario, IdPuntoDeVenta, idReporte, mes)
	select	c.IdCliente
			,r.IdUsuario
			,r.IdPuntoDeVenta
			,max(r.idReporte)
	...
	insert #Relevados (idZona, asignados, relevados)
	select pdv.idZona, count(distinct a.idPuntoDeVenta), count(distinct t.IdPuntoDeVenta) from #asignados a
	left join #tempReporte t on t.idUsuario = a.idUsuario
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('zona','Zona'),('pc','Cobertura')
	
	if(@lenguaje = 'en')
		insert #columnasConfiguracion (name, title) values ('zona','Zone'),('pc','Coverage')
	...
	from #relevados a
	inner join Zona z on z.idZona = a.idZona
	order by ltrim(rtrim(z.Nombre))
