IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_TotalCobertura_T9'))
   exec('CREATE PROCEDURE [dbo].[Pper_TotalCobertura_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_TotalCobertura_T9] 	
(
	@IdCliente			int
...
*/		
	...
	(
		id int identity(1,1),
		idcliente int, --Cambio de Varchar a int
		qty int
	)
...
--	select ltrim(rtrim(c.Nombre)), count(distinct idPuntoDeVenta) from #reportesMesPdv r
--	inner join Cliente c on c.idEmpresa = r.idEmpresa
--	group by c.Nombre

	--COBERTURA , es universo menos los de usuario 1626
	insert #DatosFinal (idCliente, qty)
	select x.idCliente, count(x.IdPuntoDeVenta) 
	from (select idCliente, idPuntodeventa  from #Asignados where idUsuario <> 1626 group by idCliente, idPuntodeventa)x
--	where idUsuario <> 1626
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total Cobertura',30),('qty','Cantidad',50)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total Coverage',30),('qty','Qty',50)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select c.nombre as cliente, d.qty from #datosFinal d
...
--[Pper_TotalCobertura_T9] 9


--select * From cliente where nombre like '%claro%'