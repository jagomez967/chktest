IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_TotalObjetivo_T9'))
   exec('CREATE PROCEDURE [dbo].[Pper_TotalObjetivo_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_TotalObjetivo_T9] 	
(
	@IdCliente			int
...
*/		
	...
	/*
	insert #datosFinal (cliente, qty)
	select ltrim(rtrim(c.Nombre)), count(distinct idPuntoDeVenta) from #asignados
	inner join Cliente c on c.idCliente = @idCliente --noooooooooooooooo
	group by c.Nombre
	*/

	--Arreglo (con cinta scotch) lo de arriba, DESCONFIAR
	insert #datosFinal (cliente,qty)
	select ltrim(rtrim(c.nombre)), count(distinct a.idPuntodeventa) from #asignados A
	inner join puntodeventa p on p.idPuntodeventa = a.idPuntodeventa 
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total Universo',30),('qty','Qty',50)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total Universe',30),('qty','Qty',50)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select cliente, qty from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select cliente, qty from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select cliente, qty from #datosFinal
end