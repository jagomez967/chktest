IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Pper_TotalPorcentajeCob_T9'))
   exec('CREATE PROCEDURE [dbo].[Pper_TotalPorcentajeCob_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Pper_TotalPorcentajeCob_T9] 	
(
	@IdCliente			int
...
*/		
	...
	select ltrim(rtrim(c.Nombre)), count(distinct t.idPuntoDeVenta)*100.0/(select count(distinct idPuntoDeVenta) from #asignados) from #reportesMesPdv t
	inner join Cliente c on c.idEmpresa = t.idEmpresa
	group by c.Nombre
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total % Cobertura',30),('qty','%',50)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('cliente','Total % Coverage',30),('qty','%',50)
