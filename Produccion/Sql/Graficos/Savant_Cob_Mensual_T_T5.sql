IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Savant_Cob_Mensual_T_T5'))
   exec('CREATE PROCEDURE [dbo].[Savant_Cob_Mensual_T_T5] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Savant_Cob_Mensual_T_T5] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	begin
		...
		(
			select count( IdPuntoDeVenta) from #tempPCU
		) where id=@i
	...
	insert #ReVisitados(idUsuario,qty)
	select idUsuario, count(distinct IdPuntodeventa) from
		(select idUsuario,IdPuntodeventa,count(distinct idReporte)as qtyRepos 
		from #tempReporte
		group by idUsuario,IdPuntoDeVenta
...
exec Savant_Cob_Mensual_T_T5 @IdCliente=49,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
