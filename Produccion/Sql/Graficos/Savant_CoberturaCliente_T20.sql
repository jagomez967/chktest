IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Savant_CoberturaCliente_T20'))
   EXEC('CREATE PROCEDURE [dbo].[Savant_CoberturaCliente_T20] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Savant_CoberturaCliente_T20]
(
	@IdCliente			INT,
...
*/		
	...
	AS
	(
		SELECT CAST(@FechaHasta AS DATETIME) Fecha, NULL Asignados, NULL Relevados, NULL Restantes, NULL Porcentaje
		UNION ALL
		SELECT CONVERT(DATETIME,DATEADD(s,-1,DATEADD(mm, DATEDIFF(m,0,Fecha),0))) , NULL Asignados, NULL Relevados, NULL Restantes, NULL Porcentaje
		FROM DateTable
	...
	UPDATE 
		#Tmp 
	SET 
		Restantes = CASE WHEN(Asignados - ISNULL(Relevados,0)) > 0 THEN (Asignados - ISNULL(Relevados,0)) ELSE 0 END , 
		Porcentaje = (Relevados * 100 / Asignados)
	...
	IF ISNULL(@Lenguaje,'ES') = 'ES' BEGIN
		...
		('Periodo','Periodo',10,0),
		('Asignados','Asignados',10,1),
		('Relevados','Relevados',10,2),
		('Restantes','Restantes',10,3),
		('Porcentaje','Porcentaje',10,4)
	END

	IF ISNULL(@Lenguaje,'ES') = 'EN' BEGIN
		INSERT INTO #ColumnasDef (name, title, width, orden) VALUES 
		('Periodo','Period',10,0),
		('Asignados','Assigned',10,1),
		('Relevados','Done',10,2),
		('Restantes','Left',10,3),
		('Porcentaje','Percent',10,4)
	END
	...
	SELECT
		Id,
		Periodo,
		ISNULL(Asignados,0) Asignados,
		ISNULL(Relevados,0) Relevados,
		ISNULL(Restantes,0) Restantes,
		CONVERT(VARCHAR,ISNULL(Porcentaje,0)) + ' %' Porcentaje
	FROM 
		#DatosFinal
	WHERE 
		Id >= 
	...
	SELECT
		...
		SUM(Relevados), 
		SUM(Restantes), 
		CONVERT(VARCHAR,AVG(Porcentaje)) + ' %' Porcentaje 
	FROM 
		#DatosFinal
	WHERE
		ISNULL(Asignados | Relevados | Restantes | Porcentaje, 0) != 0
	) A
	ORDER BY 
		Id
