IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Savant_FacingMarcaPropia_T25'))
   EXEC('CREATE PROCEDURE [dbo].[Savant_FacingMarcaPropia_T25] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].Savant_FacingMarcaPropia_T25
(
	@IdCliente			INT,
...
BEGIN
	...
	*/		
	...
	SELECT
		Dt.Id,
		LEFT(DATENAME(MONTH,DT.Fecha),3) + '-' + RIGHT(YEAR(DT.Fecha),2) Fecha,
		M.idMarca,
		M.Nombre,
		SUM(ISNULL(Cantidad,0) + ISNULL(Cantidad2,0)) Qty
	INTO 
		#TMP
	FROM 
		RPT 
	...
	IF ISNULL(@cMarcas,0) != 0 BEGIN
		DELETE FROM #TMP WHERE idMarca NOT IN (SELECT idMarca FROM #Marcas)
	END
	...
	SELECT 
		@SelectMonts = COALESCE(@SelectMonts + ',', '') + '['+CONVERT(VARCHAR,Fecha)+']' 
	FROM 
		#SelectMonths
