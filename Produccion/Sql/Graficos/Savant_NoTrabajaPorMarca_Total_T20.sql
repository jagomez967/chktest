IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Savant_NoTrabajaPorMarca_Total_T20'))
   exec('CREATE PROCEDURE [dbo].[Savant_NoTrabajaPorMarca_Total_T20] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Savant_NoTrabajaPorMarca_Total_T20] 	
(
	@IdCliente			INT,
...
*/		
	...
	SET
		Relevados = (
			SELECT 
				COUNT(DISTINCT idPuntoDeVenta) a
			FROM
				(
				SELECT	C.idCliente,
						R.idUsuario,
	...
	SELECT DATEPART(MONTH,Fecha) Id,LEFT(DATENAME(MONTH,Fecha),3) + '-' + RIGHT(DATENAME(YEAR,Fecha),2) Fecha, 'Relevados' Marca, Relevados Qty1 INTO #TMP2 FROM #TMPPDV ORDER BY Id
	...
	INTO
		##TMPRESULT
	FROM 
		(SELECT DISTINCT Marca, Fecha, Qty
		FROM #TMP ) A
	PIVOT 
		(MIN(Qty) 
	FOR 
	...
	SELECT COUNT(DISTINCT Fecha) FROM #TMP
		...
		UNION
		SELECT  0 Id, 'Marca' name,'Marca' title ,10 width FROM #TMP
		UNION
		SELECT 20 Id, 'Totales' name, 'Totales' title, 10 width FROM #TMP 
...
--Savant_NoTrabajaPorMarca_Total_T20 49
