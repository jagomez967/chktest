IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Savant_QuiebrePorProducto_T1'))
   EXEC('CREATE PROCEDURE [dbo].[Savant_QuiebrePorProducto_T1] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[Savant_QuiebrePorProducto_T1]
(
	@IdCliente			INT,
...
*/		
	...
	SELECT 
		@SelectFechas = COALESCE(@SelectFechas + ',', '') + '['+CONVERT(VARCHAR,Fecha)+'] ' + '''' +  Fecha  + ''''
	FROM 
		#SelectFechas
