IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Seagate_Frentes2_T7'))
   exec('CREATE PROCEDURE [dbo].[Seagate_Frentes2_T7] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Seagate_Frentes2_T7] 	
(
	@IdCliente			int
...
*/		
	...
	select	r.fecha, r.Idpdv,m.idmarca,m.nombre,sum(isnull(rp.Cantidad2,0))
	...
	group by r.fecha, r.Idpdv,m.idmarca,m.nombre
	
	select fecha,fecha, nombre_marca, sum(total_Frentes2) as total_Frentes2 from #datos group by fecha, nombre_marca
	...
	from ReporteProducto rp
		...
		and (isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = m.IdMarca))
		and (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = p.IdProducto))
	group by r.fecha, r.Idpdv,m.idmarca,m.nombre,p.idProducto,p.nombre
	
	select fecha, fecha, id_producto,producto,nombre_marca,total_Frentes2  from #datos_final 
