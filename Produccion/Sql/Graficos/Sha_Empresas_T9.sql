IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_Empresas_T9'))
   exec('CREATE PROCEDURE [dbo].[Sha_Empresas_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_Empresas_T9]
(
	@IdCliente			int
...
*/		
	...
	inner join Empresa e on e.IdEmpresa=te.idEmpresa

	insert #datosFinal (empresa, facing)
	select LTRIM(rtrim(e.nombre)) as Empresa, cast(SUM(isnull(d.qty,0))*100/@total as numeric(18,1)) as Facing
	from #EmpresasMeses te
	left join #datos d on d.idEmpresa=te.idEmpresa and d.mes=te.mes
	inner join Empresa e on e.IdEmpresa=te.idEmpresa
	group by ltrim(rtrim(e.nombre))
	order by ltrim(rtrim(e.nombre))
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('empresa','Empresa',5),('facing','Facing',10)

	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('empresa','Company',5),('facing','Facing',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select empresa, cast(facing as varchar(10))+' %' as facing from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select empresa, cast(facing as varchar(10))+' %' as facing from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select empresa, cast(facing as varchar(10))+' %' as facing from #datosFinal
