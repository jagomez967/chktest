IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_EvolMarcaComp_T10'))
   exec('CREATE PROCEDURE [dbo].[Sha_EvolMarcaComp_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_EvolMarcaComp_T10]
(
	@IdCliente			int
...
*/		
	...
	select @total = sum(qty) from #datos

	insert #MarcasMeses (idEmpresa, idMarca, mes)
	select distinct tmp.idempresa, tmp.idMarca, tmp.mes
	from #datos tmp
	inner join #Meses m on 1=1
	
	insert #Resultados (idEmpresa, idMarca, mes, qty)
	select te.idEmpresa, te.idMarca, te.mes, SUM(isnull(d.qty,0))*100.0/@total
	from #MarcasMeses te
	...
	create table #totalMeses
	(
		qty numeric(18,5),
		mes varchar(6)
	)
	insert #totalMeses (qty, mes)
	select sum(isnull(qty,0)), mes from #Resultados group by mes

	select r.idMarca, ltrim(rtrim(m.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0))*100.0/isnull(tm.qty,0) as numeric(18,1)),1 Visibletooltip
	from #Resultados r
	inner join Marca m on m.IdMarca=r.idMarca
	inner join #totalMeses tm on tm.mes = r.mes
	group by r.idMarca, ltrim(rtrim(m.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), tm.qty
