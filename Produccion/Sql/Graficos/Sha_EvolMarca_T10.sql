IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_EvolMarca_T10'))
   exec('CREATE PROCEDURE [dbo].[Sha_EvolMarca_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_EvolMarca_T10]
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, p.IdMarca, r.mes, SUM(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0))
	...
	group by r.idEmpresa, p.IdMarca, r.mes
	...
	create table #totalMes
	(
		mes varchar(6),
		qty numeric(18,5)
	)
	insert #totalMes (mes, qty)
	select mes, sum(isnull(qty,0)) from #Resultados
	group by mes

	
	select r.idMarca, ltrim(rtrim(m.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0))*100.0/tm.qty as numeric(18,1)),1 Visibletooltip
	from #Resultados r
	inner join Marca m on m.IdMarca = r.idMarca
	inner join #totalMes tm on tm.mes = r.mes
