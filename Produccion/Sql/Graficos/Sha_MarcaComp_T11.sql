IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_MarcaComp_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_MarcaComp_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_MarcaComp_T11] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idmarca, LTRIM(rtrim(m.nombre)) as Marca, cast(isnull(r.qty,0)*100*1.0/@total as numeric(18,1)) as Share, m.sColor
	from #Resultados r
	inner join Marca m on m.idmarca=r.idMarca
	order by m.sColor, m.nombre
end
GO
