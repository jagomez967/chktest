IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_MarcaPropiaComp_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_MarcaPropiaComp_T11] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER procedure [dbo].[Sha_MarcaPropiaComp_T11]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idMarca, qty)
	select r.idEmpresa, m.idMarca, SUM(isnull(rp.Cantidad,0)+isnull(rp.Cantidad2,0))
	...
	group by r.idEmpresa, m.idMarca
	...
	where	(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = mc.IdMarca))
			...
			and (isnull(@cFamilia,0) = 0 or exists(select 1 from #Familia where idFamilia = (select idFamilia from producto where idProducto = pc.idProducto)))
	...
	insert #EmpresasMeses (idEmpresa)
	select distinct tmp.idempresa from #datos tmp
	...
	select te.idEmpresa, d.idMarca, SUM(isnull(d.qty,0))
	from #EmpresasMeses te
	left join #datos d on d.idEmpresa=te.idEmpresa
	...
	select m.IdMarca, ltrim(rtrim(m.Nombre)), cast(isnull(r.qty,0)*100*1.0/@total as numeric(18,1)) as Share
	from #Resultados r
