IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_MarcaPropiaYComp_Frentes1_T10'))
   exec('CREATE PROCEDURE [dbo].[Sha_MarcaPropiaYComp_Frentes1_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_MarcaPropiaYComp_Frentes1_T10]
(
	@IdCliente			int
...
*/		
	...
	select m.idMarca, r.mes, SUM(isnull(rp.Cantidad,0))
	...
	if(@idCliente != 200)
	BEGIN
		select r.idMarca, ltrim(rtrim(m.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1))
		from #Resultados r
	...
	ELSE
	BEGIN
		select r.idMarca, ltrim(rtrim(m.nombre)), r.mes, right(CONVERT(VARCHAR(11),convert(datetime, r.mes+'01'),6),6), cast(SUM(isnull(r.qty,0)) as numeric(18,1)), isnull(m.scolor,'grey') as sColor
		from #Resultados r
		inner join Marca m on m.idMarca = r.idMarca
...
exec Sha_MarcaPropiaYComp_Frentes1_T10 @IdCliente=235,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
...
exec Sha_MarcaPropiaYComp_Frentes1_T10 @IdCliente=235,@Filtros=@p3,@NumeroDePagina=-1,@Lenguaje='es'
*/