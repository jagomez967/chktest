...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_MarcaPropiaYComp_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_MarcaPropiaYComp_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_MarcaPropiaYComp_T11] 	
(
	@IdCliente			int
...
*/		
	...
	select	r.idEmpresa
			,r.IdUsuario
			,r.IdPuntoDeVenta
			,convert(varchar, (r.YCreacion * 10000) + (r.MCreacion * 100)  + 1) as mes
			,max(r.idReporte)
	from reporte r--  WITH(INDEX([IX_Partition_Date]))
	...
	group by r.idEmpresa, r.IdUsuario, r.IdPuntoDeVenta, convert(varchar, (r.YCreacion * 10000) + (r.MCreacion * 100)  + 1)
