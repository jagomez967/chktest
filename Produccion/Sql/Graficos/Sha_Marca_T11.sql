IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_Marca_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_Marca_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_Marca_T11]
(
	@IdCliente			int
...
*/		
	...
	select m.IdMarca, ltrim(rtrim(m.Nombre)), isnull(r.qty,0)
	from #Resultados r
