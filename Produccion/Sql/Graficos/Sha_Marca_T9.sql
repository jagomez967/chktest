IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_Marca_T9'))
   exec('CREATE PROCEDURE [dbo].[Sha_Marca_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_Marca_T9]
(
	@IdCliente			int
...
*/		
	...
	select ltrim(rtrim(m.Nombre)) as Marca, cast(isnull(r.qty,0)*100/@Total as numeric(18,1)) as Share
	from #Resultados r
