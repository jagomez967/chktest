IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_ProdComp_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_ProdComp_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_ProdComp_T11] 	
(
	@IdCliente			int
...
*/		
	...
	select p.idproducto, ltrim(rtrim(p.Nombre)), isnull(r.qty,0),m.sColor
	from #Resultados r
	inner join producto p on p.IdProducto=r.idProducto
	left join marca m on m.idMarca = p.idMarca
	order by m.sColor, p.idproducto, p.Nombre
end
GO
