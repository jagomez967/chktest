IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_ProdComp_T9'))
   exec('CREATE PROCEDURE [dbo].[Sha_ProdComp_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_ProdComp_T9] 	
(
	@IdCliente			int
...
*/		
	...
	select @total = sum(isnull(r.qty,0))
	from #Resultados r

	update #Resultados set qty = round(isnull(qty,0)*100/@total,1)
	...
	insert #datosfinal(producto, share)
	select ltrim(rtrim(p.Nombre)) as Producto, cast(isnull(r.qty,0) as numeric(18,1)) as Share
	from #Resultados r
	...
	if(@lenguaje='es')
		insert #columnasConfiguracion (name, title, width) values ('producto','Producto',5),('share','Share',10)
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title, width) values ('producto','Product',5),('share','Share',10)

	select name, title, width from #columnasConfiguracion

	--Datos
	if(@NumeroDePagina>0)
		select producto, share from #datosFinal where id between ((@NumeroDePagina - 1) * @TamañoPagina + 1) and (@NumeroDePagina * @TamañoPagina)
	
	if(@NumeroDePagina=0)
		select producto, share from #datosFinal where id between ((@maxpag - 1) * @TamañoPagina + 1) and (@maxpag * @TamañoPagina)
		
	if(@NumeroDePagina<0)
		select producto, share from #datosFinal
