IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Sha_Prod_T11'))
   exec('CREATE PROCEDURE [dbo].[Sha_Prod_T11] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Sha_Prod_T11] 	
(
	@IdCliente			int
...
*/		
	...
	delete from #Resultados where idProducto in
	(select idProducto
	from #Resultados
	group by idProducto
	having sum(qty)=0)

	select p.idproducto, ltrim(rtrim(p.Nombre)), isnull(r.qty,0)
	from #Resultados r
	inner join producto p on p.IdProducto=r.idProducto
	order by p.idproducto, p.Nombre
end
GO
