IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_EvolShareMarcas_T10'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_EvolShareMarcas_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_EvolShareMarcas_T10]
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, p.idMarca, r.mes, SUM(isnull(rp.Precio,0))
	from #tempReporte r
	...
	insert #MesesProductos (idEmpresa, idMarca, mes)
	select distinct d.idEmpresa, d.idMarca, m.mes
	from #datos d
	...
	select m.idMarca, ltrim(rtrim(ma.Nombre)), m.mes, right(CONVERT(VARCHAR(11),convert(datetime,m.mes),6),6), cast(isnull(d.qty,0)*100.0/@total as numeric(18,2))
	from #MesesProductos m
	left join #datos d on d.idEmpresa = m.idEmpresa and d.idMarca = m.idMarca and d.mes = m.mes
