IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_EvolShareProd_T10'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_EvolShareProd_T10] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_EvolShareProd_T10]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idProducto, mes, qty)
	select r.idEmpresa, rp.idProducto, r.mes, SUM(isnull(rp.Precio,0))
	from #tempReporte r
	...
	group by r.idEmpresa, rp.idProducto, r.mes
	...
	insert #MesesProductos (idEmpresa, idProducto, mes)
	select distinct d.idEmpresa, d.idProducto, m.mes
	from #datos d
	inner join #Meses m on 1=1

	declare @total numeric(18,5)
	select @total = SUM(isnull(qty,0)) from #datos


	select max(m.idProducto), left(p.Nombre, charindex('-',p.Nombre)), m.mes, right(CONVERT(VARCHAR(11),convert(datetime,m.mes+'01'),6),6), cast(sum(isnull(d.qty,0))*100.0/@total as numeric(18,2))
	from #MesesProductos m
	left join #datos d on d.idEmpresa = m.idEmpresa and d.idProducto = m.idProducto and d.mes = m.mes
	inner join Producto p on p.IdProducto = m.idProducto
	group by left(p.Nombre, charindex('-',p.Nombre)), m.mes, right(CONVERT(VARCHAR(11),convert(datetime,m.mes+'01'),6),6)
