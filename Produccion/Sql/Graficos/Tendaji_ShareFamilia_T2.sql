IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_ShareFamilia_T2'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_ShareFamilia_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_ShareFamilia_T2] 	
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idEmpresa, idFamilia, qty)
	select r.idEmpresa, p.idFamilia, SUM(isnull(rp.Precio,0))
	from #tempReporte r
	...
	group by r.idEmpresa, p.idFamilia
	...
	insert #Resultados (idEmpresa, idFamilia, qty)
	select te.idEmpresa, te.idFamilia, SUM(isnull(te.qty,0))
	from #datos te
	group by te.idEmpresa, te.idFamilia

	delete from #Resultados where idEmpresa in (select idEmpresa from #Resultados group by idEmpresa having sum(qty)=0)
	delete from #Resultados where idFamilia in (select idFamilia from #Resultados group by idFamilia having sum(qty)=0)

	declare @total numeric(18,5)
	select @total = SUM(isnull(qty,0)) from #Resultados

	select replace(left(f.Nombre+' ', charindex(' ',f.Nombre+' ')),' ','') as Categoria, sum(isnull(r.qty,0)) as Share
	from #Resultados r
	inner join Familia f on f.idFamilia = r.idFamilia
	group by replace(left(f.Nombre+' ', charindex(' ',f.Nombre+' ')),' ','')
	order by replace(left(f.Nombre+' ', charindex(' ',f.Nombre+' ')),' ','')
end