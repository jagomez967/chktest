IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_ShareMarca_T2'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_ShareMarca_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_ShareMarca_T2] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, p.idMarca, SUM(isnull(rp.Precio,0))
	from #tempReporte r
	...
	select te.idEmpresa, te.idMarca, isnull(te.qty,0)
	from #datos te
	...
	select ltrim(rtrim(m.Nombre)) as Marca, isnull(r.qty,0) as Share
	from #Resultados r
