IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_ShareProd_T2'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_ShareProd_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_ShareProd_T2] 	
(
	@IdCliente			int
...
*/		
	...
	select r.idEmpresa, rp.idProducto, SUM(isnull(rp.Precio,0))
	from #tempReporte r
	...
	select left(p.Nombre, charindex('-',p.Nombre)) as Producto, sum(isnull(r.qty,0)) as Share
	from #Resultados r
	inner join Producto p on p.idProducto = r.idProducto
	group by left(p.Nombre, charindex('-',p.Nombre))
	order by left(p.Nombre, charindex('-',p.Nombre))
end