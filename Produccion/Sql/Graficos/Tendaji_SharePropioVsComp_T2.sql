IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Tendaji_SharePropioVsComp_T2'))
   exec('CREATE PROCEDURE [dbo].[Tendaji_SharePropioVsComp_T2] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Tendaji_SharePropioVsComp_T2] 	
(
	@IdCliente			int
...
*/		
	...
	where m.idMarca in (1580,1590,1591)
	...
	select LTRIM(rtrim(p.Nombre)) as Producto, cast(isnull(r.qty,0)*100.0/@total as numeric(18,1)) as Share, r.idProducto
	from #Resultados r
