IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_CobMensual_T1'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_CobMensual_T1] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[WhirlPool_CobMensual_T1]
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Temps) ----------------------------------------------------------------
	...
	--SELECT DISTINCT T.mes,SUM(DISTINCT TPW.TotalPdv) Qty INTO #tmpTotCadena FROM #tempReporte T INNER JOIN PuntoDeVenta PDV ON T.idPuntoDeVenta = PDV.idPuntoDeVenta INNER JOIN TotalPdvCadenaWhirlPool TPW ON PDV.idCadena = TPW.idCadena GROUP BY T.mes
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('Totales','Totales'),('asignados','Asignados'),('relevados','Relevados')
	
	if(@lenguaje='en')
		insert #columnasConfiguracion (name, title) values ('mes','mes'),('Totales','Total'),('asignados','Assigned'),('relevados','Visited')

	select name, title from #columnasConfiguracion

	
	SELECT @Total = SUM(TotalPdv) FROM TotalPdvCadenaWhirlPool TW 
	INNER JOIN Cadena CA ON TW.idCadena = CA.idCadena
	INNER JOIN Empresa E ON CA.idNegocio = E.idNegocio
	INNER JOIN Cliente C ON E.idEmpresa = C.idEmpresa
	WHERE 
	C.idCliente = @idCliente
	and (isnull(@cCadenas,0) = 0 or exists(select 1 from #cadenas where idCadena = TW.IdCadena))
	and (isnull(@cCategoria,0) = 0 or exists(select 1 from #Categoria where idCategoria IN (SELECT DISTINCT idCategoria FROM PuntoDeVenta PDV WHERE PDV.idCadena = TW.idCadena)))
	
	select right(CONVERT(VARCHAR(11),convert(datetime, A.mes),6),6) as mes, @Total as Totales, A.qty as asignados, isnull(V.qty,0) as relevados from #Asignados A left join #Relevados V on V.mes=A.mes
