IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_DistribucionFisicaNumericaCadena_T9'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaCadena_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaCadena_T9]
(
	@IdCliente			INT
...
*/		
	...
	SELECT 
		...
		CONVERT(VARCHAR,(SELECT Cant FROM #TmpCant WHERE idCadena = T.idCadena)) TotalPdv,
		CONVERT(NUMERIC(18,2),CONVERT(NUMERIC(18,2),SUM(TieneFrente) * 100) / (SELECT Cant FROM #TmpCant WHERE idCadena = T.idCadena)) Porcentaje
	INTO
		#TmpDistriProd
	FROM 
		#TmpDistrPdv T
