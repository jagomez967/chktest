IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_DistribucionFisicaNumericaCocinas_PY_T7'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaCocinas_PY_T7] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaCocinas_PY_T7]
(
	@IdCliente			INT
...
*/		
	...
	(
		...
		A.idPuntoDeVenta,
		RIGHT('00' + CONVERT(VARCHAR,DATEPART(MONTH,CONVERT(DATE,ISNULL(CONVERT(VARCHAR,R.FechaCreacion),'')))),2) + '-' + DATENAME(YEAR,ISNULL(CONVERT(VARCHAR,R.FechaCreacion),''))  + ' Cocinas' Mes,
		RP.idProducto,
