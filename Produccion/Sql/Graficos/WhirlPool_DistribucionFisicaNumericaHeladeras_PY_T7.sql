IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_DistribucionFisicaNumericaHeladeras_PY_T7'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaHeladeras_PY_T7] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaHeladeras_PY_T7]
(
	@IdCliente			INT
...
*/		
	...
	(
		...
		RP.idProducto,
		CASE WHEN SUM(ISNULL(RP.Cantidad2,0)) != 0 THEN 1 ELSE 0 END TieneFrente
	INTO 
		#TmpDistrPdv
	FROM 
		A 
