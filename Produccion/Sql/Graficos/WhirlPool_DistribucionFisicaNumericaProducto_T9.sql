IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_DistribucionFisicaNumericaProducto_T9'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaProducto_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_DistribucionFisicaNumericaProducto_T9]
(
	@IdCliente			INT
...
*/		
	...
	SELECT distinct
		...
		CONVERT(NUMERIC(18,2),CONVERT(NUMERIC(18,2),SUM(TieneFrente) * 100) / (SELECT Cant FROM #TmpCant WHERE fecha = T.fecha))Porcentaje																				
	INTO
		#TmpDistriProd
	FROM 
		#TmpDistrPdv T
	...
	SELECT 
		...
		{SelectFecha}
	FROM 
		(SELECT DISTINCT  fecha,Producto, Qty FROM #TmpDistriPivot) A
	PIVOT 
		(MIN(Qty) 
	FOR 
		fecha 
	...
	SELECT @SelectFechas = COALESCE(@SelectFechas + ',', '') + '['+CONVERT(VARCHAR,Fecha)+'] ' 
	FROM #SelectFecha	
		
	SET @Query = REPLACE(@Query,N'{Fechas}',@SelectFechas)
	...
	(
	SELECT 0 Id, 'Producto' name,'Producto' title, 5 width
	UNION
	SELECT id, fecha,fecha,5 
	FROM #TmpDistriPivot 
	) A
	---ORDER BY A.Id
...
exec WhirlPool_DistribucionFisicaNumericaProducto_T9 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
