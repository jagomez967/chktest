IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_DistribucionFisicaPorFamilia_T9'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_DistribucionFisicaPorFamilia_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_DistribucionFisicaPorFamilia_T9]
(
	@IdCliente			INT
...
*/      
 ...
 (    
  ...
  A.idPuntoDeVenta,    
  RIGHT('00' + CONVERT(VARCHAR,DATEPART(MONTH,CONVERT(DATE,ISNULL(CONVERT(VARCHAR,R.FechaCreacion),'')))),2) + '-' + DATENAME(YEAR,ISNULL(CONVERT(VARCHAR,R.FechaCreacion),''))  + ' Heladeras' Mes,    
  RP.idProducto,    
 ...
 SELECT     
    ...
    CONVERT(VARCHAR,(SELECT Cant FROM #TmpCant WHERE Mes = T.Mes AND idUsuario = T.idUsuario)) + LEFT(T.Mes,2) TotalPdv,    
    CONVERT(NUMERIC(18,2),CONVERT(NUMERIC(18,2),SUM(TieneFrente) * 100) / (SELECT sum(Cant) FROM #TmpCant WHERE Mes = T.Mes AND idUsuario = T.idUsuario)) Porcentaje,  
 ...
 SELECT      
  ...
  {SelectProductos}    
 FROM     
  (SELECT DISTINCT Usuario, Familia, Qty FROM #TmpDistriProy1) A    
 PIVOT     
  (MIN(Qty)     
 FOR     
  Familia    
 ...
 SELECT     
  @ProductosColumns = ISNULL(@ProductosColumns,N'') + N'[' +  CONVERT(NVARCHAR,Familia) + N'],'    
 FROM     
  #SelectFamilia    
 SET @ProductosColumns = LEFT(@ProductosColumns,LEN(@ProductosColumns) - 1)    
 SET @Query = REPLACE(@Query,N'{Familias}',@ProductosColumns)    
 ...
 (    
 SELECT 1 Id, 'Usuario' name,'Usuario' title, 5 width    
 UNION    
 SELECT 2, Familia,Familia,5 FROM #SelectFamilia       
 ) A    
        
 --PRINT @Query    
 EXEC sp_executesql @Query    
        
 DROP TABLE #TmpDistriProy1    
...
--insert into @p2 values(N'fltFechaReporte',N'M,20181101,20181106')
...
--exec WhirlPool_DistribucionFisicaPorFamilia_T9 @IdCliente=187,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'