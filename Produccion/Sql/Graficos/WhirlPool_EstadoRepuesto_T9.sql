IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EstadoRepuesto_T9'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EstadoRepuesto_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_EstadoRepuesto_T9]
(
	@IdCliente			INT
	...
	*/		
	...
	SELECT 
		...
		CASE WHEN @NumeroDePagina = -1 THEN 
			E.Nombre
		ELSE 
			'<b style="color:'+ CASE WHEN E.idExhibidor = 411 THEN '#00FF00' ELSE '#FF0000' END  +'">' + E.Nombre + '</b>'  
		END Estado
	FROM 
		Reporte R
