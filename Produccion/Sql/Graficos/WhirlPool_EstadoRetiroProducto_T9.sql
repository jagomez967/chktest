IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EstadoRetiroProducto_T9'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EstadoRetiroProducto_T9] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirlPool_EstadoRetiroProducto_T9]
(
	@IdCliente			INT
...
BEGIN
	...
	*/		
	...
	SELECT 
		...
		(SELECT ISNULL(Valor4,'') FROM MD_ReporteModuloItem WHERE idItem = 11424 AND idReporte = R.idReporte)+ ' ' + 
		(SELECT ISNULL(Valor4,'') FROM MD_ReporteModuloItem WHERE idItem = 11425 AND idReporte = R.idReporte)+ ' ' + 
		(SELECT ISNULL(Valor4,'') FROM MD_ReporteModuloItem WHERE idItem = 11426 AND idReporte = R.idReporte) Valor4,
		CASE WHEN @NumeroDePagina = -1 THEN 
			E.Nombre
		ELSE 
			'<b style="color:'+ CASE WHEN E.idExhibidor = 383 THEN '#FF0000' WHEN E.idExhibidor = 410 THEN '#00FF00' END  +'">' + E.Nombre + '</b>'  
		END Estado
	FROM 
		Reporte R
	...
	WHERE 
		...
		AND RP.idExhibidor IN (383,410) --RETIRO DE PRODUCTO, FINALIZADO RETIRO DE PRODUCTO
