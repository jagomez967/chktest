IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EvolucionPorPlataforma_PY_T25'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EvolucionPorPlataforma_PY_T25] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].WhirlPool_EvolucionPorPlataforma_PY_T25
(
	@IdCliente			INT
...
--insert into @p2 values(N'fltFechaReporte',N'M,20180821,20190122')
----insert into @p2 values(N'fltpuntosdeventa',N'99647')
----insert into @p2 values(N'fltusuarios',N'4957')
----insert into @p2 values(N'fltMarcas',N'614')
--
---exec WhirlPool_EvolucionPorPlataforma_PY_T25 @IdCliente=190,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'