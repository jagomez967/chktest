IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EvolucionPrecio1y2_T22'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EvolucionPrecio1y2_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[WhirlPool_EvolucionPrecio1y2_T22]
(
	@IdCliente			int
...
*/		
	...
	LEFT join subMarca_Producto smp on smp.idProducto = p.idProducto
	LEFT join SubMarca m on m.idSubMarca = smp.idSubMarca
		and m.idMarca = p.idMarca
	LEFT JOIN Marca M2 ON P.idMarca = M2.idMarca
	...
	and p.Reporte = 1
	union
	select p.idProducto+10,p.nombre + ' Accionado',/*CONTADOR*/1,/*m.MarcaColor*/'',
	year(r.FechaCreacion) * 10000
	+
	datepart(month,r.FechaCreacion) * 100
	+
	...
	update p SET cantidad = (select count(1) from #preciosPorProducto where idproducto = p.idproducto and fecha = p.fecha and precio = p.precio) 
	from #preciosPorProducto p
	update p SET cantidad = (select count(1) from #preciosPorProducto where idproducto = p.idproducto-10 and fecha = p.fecha and precio = p.precio) 
	from #preciosPorProducto p
	


	select d.idPRoducto,d.producto,convert(varchar,(SELECT Fecha 
											FROM #Semanas 
											WHERE NroSemana = DATEPART(WEEK,CONVERT(VARCHAR,d.fecha,112))
											and anio = DATEPART(YEAR,CONVERT(VARCHAR,d.fecha,112))
											),112) fecha,
	...
	order by d.idProducto,p.precio asc
	...
	inner join #preciosPorProducto p		
	on p.idProducto = d.idProducto and p.fecha = d.fecha	
	inner join producto prod on prod.idProducto = p.idProducto -10
	where p.cantidad = (select max(cantidad) from #preciosPorProducto WHERE idproducto = p.idproducto and fecha = d.fecha)
	group by d.idProducto,d.producto,d.fecha,d.valorFecha,d.color
	,d.dashStyle,d.linkedTo,d.Marca,prod.orden,p.precio,p.cantidad
	order by d.idProducto,p.precio asc
	
	SELECT DISTINCT idProducto,Producto,fecha,valorfecha,valor,color,dashStyle,linkedTo,Marca,orden,Cat 
	INTO #finaldata FROM #prepdata F WHERE Qty = (SELECT MAX(QTY) FROM #prepdata WHERE idProducto = F.idProducto AND Cat = F.Cat)
	SELECT DISTINCT idProducto,Producto,fecha,CONVERT(VARCHAR,fecha,103) valorfecha,valor,color,dashStyle,linkedTo,Marca,orden,Cat FROM #finaldata  F WHERE valor = (SELECT MIN(valor) FROM #finaldata WHERE idProducto = F.idProducto AND Cat = F.Cat)


	SELECT	'DONT ASK'				WHERE 1 = 2
	SELECT			'JUST'			WHERE 1 = 2
	SELECT				'BELIEVE'	WHERE 1 = 2

	---------------------------------------------------------------------------------------------------------------------------------
end
GO
[WhirlPool_EvolucionPrecio1y2_T22] 199
