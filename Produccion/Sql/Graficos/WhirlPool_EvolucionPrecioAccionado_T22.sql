IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EvolucionPrecioAccionado_T22'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EvolucionPrecioAccionado_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[WhirlPool_EvolucionPrecioAccionado_T22]
(
	@IdCliente			int
...
WHERE 
	...
	*/
	...
	INTO #finaldata 
	FROM #prepdata F WHERE Qty = (SELECT MAX(QTY) FROM #prepdata WHERE idProducto = F.idProducto AND Cat = F.Cat)
	SELECT DISTINCT idProducto,Producto,fecha,CONVERT(VARCHAR,fecha,103) valorfecha,valor,color,dashStyle,linkedTo,Marca,orden,Cat ,2 as TipoPrecio
	FROM #finaldata  F WHERE valor = (SELECT MIN(valor) FROM #finaldata WHERE idProducto = F.idProducto AND Cat = F.Cat)
