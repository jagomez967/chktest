IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_EvolucionPrecios_T22'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_EvolucionPrecios_T22] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[WhirlPool_EvolucionPrecios_T22]
(
	@IdCliente			int
...
*/		
	...
	update p SET cantidad = (select count(1) from #preciosPorProducto where idproducto = p.idproducto and fecha = p.fecha and precio = p.precio) from #preciosPorProducto p
	...
	SELECT DISTINCT idProducto,Producto,fecha,valorfecha,valor,color,dashStyle,linkedTo,Marca,orden,Cat INTO #finaldata FROM #prepdata F WHERE Qty = (SELECT MAX(QTY) FROM #prepdata WHERE idProducto = F.idProducto AND Cat = F.Cat)
