IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirlPool_FloorShare_T3'))
   exec('CREATE PROCEDURE [dbo].[WhirlPool_FloorShare_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[WhirlPool_FloorShare_T3]
(
	@IdCliente			int
...
*/    
 ...
 -------------------------------------------------------------------- END (Filtros) ----------------------------------------------------------------  
	...
	RG.mesStr,   
	A.GrupoPop,  
	(SELECT top 1 Color FROM GrupoPop WHERE Nombre = A.GrupoPop),  
	0  
	FROM   
	#RawGroup RG,   
	...
	mesStr,  
	GrupoPop,  
	CONVERT(NUMERIC(18,2),CONVERT(NUMERIC(18,2),100 * SUM(qty)) / (SELECT SUM(qty) FROM #RawGroup WHERE mesStr = R.mesStr)),  
	Color,  
	CASE WHEN CONVERT(DATE,'01-'+mesStr) < '20180701'THEN '' ELSE CONVERT(VARCHAR,CONVERT(INT,SUM(qty))) END + ' (' + CONVERT(VARCHAR,CONVERT(NUMERIC(18,2),CONVERT(NUMERIC(18,2),100 * SUM(qty)) / (SELECT SUM(qty) FROM #RawGroup WHERE mesStr = R.mesStr))) + 
...
'	 %)' Texto  
 ...
 declare @filtros as [dbo].[FiltrosReporting]  
   
 insert @filtros(IdFiltro, Valores) values ('fltFechaReporte','M,20180101,20181031')  
 insert @filtros(IdFiltro, Valores) values ('fltMarcas','2460')  
 insert @filtros(IdFiltro, Valores) values ('fltFamilia','927')  
 insert @filtros(IdFiltro, Valores) values ('fltPop','1367,1380,1368,1369,1370,1366,1371,1372,1381,1373,1374,1375,1376,1377,1378,1379,1484,1483')  
   
 exec WhirlPool_FloorShare_T3 187, @filtros, 0, 'es', 24, 8  
  */
