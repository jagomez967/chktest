IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.WhirpoolPY_MD_Capacitacion_T30'))
   exec('CREATE PROCEDURE [dbo].[WhirpoolPY_MD_Capacitacion_T30] AS BEGIN SET NOCOUNT ON; END')
GO
ALTER PROCEDURE [dbo].[WhirpoolPY_MD_Capacitacion_T30]
(
	@IdCliente			INT,
...
BEGIN
	...
	rc.contacto as 'Contacto/Email',
	rc.telefono as Telefono,
	rc.fecha as 'Fecha Cumpleaños'	
	from reporte r 
	...
	group by Convert(varchar(10),CONVERT(date,r.FechaCreacion,106),103),pdv.nombre,u.apellido,u.nombre,pdv.direccion,z.nombre,rc.contacto,rc.Telefono,rc.Fecha
	order by Convert(varchar(10),CONVERT(date,r.FechaCreacion,106),103) desc
...
insert into @p2 values(N'fltFechaReporte',N'M,20180601,20190122')
...
exec WhirpoolPY_MD_Capacitacion_T30 @IdCliente=190,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
