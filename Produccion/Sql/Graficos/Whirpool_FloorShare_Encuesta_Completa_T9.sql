IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Whirpool_FloorShare_Encuesta_Completa_T9'))
   exec('CREATE PROCEDURE [dbo].[Whirpool_FloorShare_Encuesta_Completa_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Whirpool_FloorShare_Encuesta_Completa_T9
(
	@IdCliente			int
...
*/		
	...
	--insert #clientes (idCliente) values(@idCliente)
	--if(@IdCliente = 204) 
	--BEGIN 
	--	insert #clientes (idCliente) values(187)
	--END
	
	
	insert #reportesMesPdv (IdPuntoDeVenta, Usuario, idReporte, Fecha,idCliente, nombre_pdv, direccion)
	select r.IdPuntoDeVenta, r.idUsuario, r.IdReporte,r.fechacreacion,c.idCliente,p.nombre,p.direccion
	from Reporte r
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha,idCliente,nombre_pdv,direccion)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo1+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, 
	mi.Orden as preg,r.fecha,r.idCliente,r.nombre_pdv, r.direccion
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha,idCliente,nombre_pdv,direccion)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo2+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha,r.idCliente,r.nombre_pdv,r.direccion from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha,idCliente,nombre_pdv,direccion)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mi.LabelCampo3+' - '+mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha,r.idCliente,r.nombre_pdv,r.direccion
	from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha,idCliente,nombre_pdv,direccion)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, mdr.Valor4 as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha,r.idCliente,r.nombre_pdv,r.direccion
	from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datos (idreporte, pdv, usuario, valor, iditem, idmarca, preg, orden,fecha,idCliente,nombre_pdv,direccion)
	select r.idReporte, r.IdPuntoDeVenta, r.usuario, NULL as valor, mi.IdItem, mdr.idMarca, mi.Nombre, mi.Orden as preg,r.fecha,r.idCliente,r.nombre_pdv,r.direccion
	from MD_ReporteModuloItem mdr
	inner join MD_Item mi on mi.IdItem = mdr.IdItem
	inner join MD_Modulo mm on mm.IdModulo = mi.IdModulo
	inner join #reportesMesPdv r on r.IdReporte = mdr.IdReporte
	...
	insert #datosRes (idreporte, iditem, idmarca, preg, pdv, usuario, valor,fecha)
	select idreporte, idItem, idmarca, preg, pdv, usuario, valor,fecha from #datos order by orden
	...
	set @ColDef='idreporte varchar(max), pdv varchar(max), usuario varchar(max), idmarca varchar(max),fecha varchar(max)'
	SELECT @ColDef = 
	  COALESCE(
	...
	insert #DatosPivot([idreporte],[pdv],[usuario],[idmarca],[fecha],'+@PivotColumnHeaders+')
	SELECT [idreporte],[pdv],[usuario],[idmarca],[fecha],'+@PivotColumnHeaders+'
	  FROM (
		Select iditem, idreporte, pdv, usuario, idmarca,fecha, valor from #DatosRes
	  ) AS PivotData
	  PIVOT (
		max(valor)
		FOR iditem IN (
	...
	insert #DatosPivotFinal ([idreporte],[pdv],[usuario],[idmarca],[fecha],'+@PivotColumnHeaders+')
	select [idreporte],[pdv],[usuario],[idmarca],[fecha],'+@PivotColumnHeaders+' from #DatosPivot where '+@PivotWhereCondition +'
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'idReporte'+char(39)+','+char(39)+'idReporte'+char(39)+', 80, -3)
	...
	insert #columnasDef (name, title, width, orden) values ('+char(39)+'UsuarioFS'+char(39)+','+char(39)+'UsuarioFS'+char(39)+', 80, -1)
	...
	insert #columnasDef(name,title,width,orden) values('+char(39)+'fecha'+char(39)+','+char(39)+'Fecha'+char(39)+', 80, 99999999)
	
	select name, title, width from #columnasDef order by orden, name
	
    
	if('+cast(@NumeroDePagina as varchar)+'>0)
			select distinct pdv.idCLiente, d.idReporte as idReporte, pdv.Nombre, u.Apellido+'', ''+u.Nombre collate database_default as Usuario,r.Apellido+'', ''+r.Nombre collate database_default as UsuarioFS, m.Nombre as Marca,'+@PivotColumnHeaders+',datename(MONTH,d.fecha) as fecha
			, r.idUsuario
			...
			left join puntodeventa pdv2 on pdv2.nombre = pdv.nombre and pdv2.direccion = pdv.direccion 
			left join usuario_puntodeventa up2 on up2.idPuntodeventa = pdv2.idPuntodeventa
			left join usuario r on r.idUsuario =  up2.idUsuario
			...
			and up2.activo = 1
							and pdv2.idCliente = 187 and r.idusuario is not null
...
insert into @p2 values(N'fltFechaReporte',N'M,20181001,20181020')
...
exec Whirpool_FloorShare_Encuesta_Completa_T9 @IdCliente=204,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
