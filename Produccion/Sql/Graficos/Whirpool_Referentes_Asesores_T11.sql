IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Whirpool_Referentes_Asesores_T11'))
   exec('CREATE PROCEDURE [dbo].[Whirpool_Referentes_Asesores_T11] AS BEGIN SET NOCOUNT ON; END')
Go
alter procedure [dbo].[Whirpool_Referentes_Asesores_T11]
(
	@IdCliente			int
...
*/		
	...
	insert #usuario (id_usuario,nombre)
	values 
        (4140,'Referente'),
		(4141,'Referente'),
		(3976,'Asesor'),
	...
	insert #capacitados (categoria, cantidad)
	select u.nombre,SUM(case when rmi.Valor4 not like '%[^0-9]%' then convert(int,rmi.Valor4) else 0 end)--,r.fecha
	...
	and u.nombre = 'Referente'
	group by mi.nombre,u.nombre
	union
	select u.nombre,SUM(case when rmi.Valor4 not like '%[^0-9]%' then convert(int,rmi.Valor4) else 0 end)--,r.fecha
	from MD_ReporteModuloItem rmi
	inner join #tempReporte r on r.IdReporte = rmi.IdReporte
	...
	select categoria, categoria, cantidad, 1 showVal from #capacitados
	...
	insert #datosRes ( id_usuario,usuario,cantidad,categoria)
    
	select  r.usuario,r.nombre,
	SUM(case when rmi.Valor4 not like '%[^0-9]%' then convert(int,rmi.Valor4) else 0 end),u.nombre
	from MD_ReporteModuloItem rmi
	inner join #tempReporte r on r.IdReporte = rmi.IdReporte
	inner join MD_Item mi on mi.idItem = rmi.idItem
	inner join #usuario u on r.usuario = u.id_usuario
	...
	and mi.idmodulo = 2442
	and u.nombre = 'Referente'
	group by r.nombre,r.usuario,r.nombre,u.nombre
	union
	select r.usuario,r.nombre,
    SUM(case when rmi.Valor4 not like '%[^0-9]%' then convert(int,rmi.Valor4) else 0 end),u.nombre
	from MD_ReporteModuloItem rmi
	...
	select categoria,categoria,id_usuario,usuario,cantidad, 1 showVal from #datosRes
