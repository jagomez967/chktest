IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Whirpool_Referente_Asesores_T1'))
   exec('CREATE PROCEDURE [dbo].[Whirpool_Referente_Asesores_T1] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Whirpool_Referente_Asesores_T1] 	
(
	@IdCliente			int
...
-------------------------------------------------------------------- END (Filtros) ----------------------------------------------------------------
	...
	--meses int
	--)
	--insert into #datos(usuario,cantidad,meses)
	--select usuario,isnull(cantidad,0),
	--RIGHT('00' + CONVERT(VARCHAR,DATEPART(MONTH,CONVERT(DATE,ISNULL(CONVERT(VARCHAR,meses),'')))),2) 
	--from #datos_fecha 
  ...
  --select * from #datos_fecha_mes_final
	...
	insert into #capacitados (id_usuario,usuario,capacitados,fecha)
	select   r.usuario,r.nombre as usuario,
	SUM(case when dbo.CleanAndTrimString(rmi.Valor4) not like '%[^0-9]%' then convert(int,dbo.CleanAndTrimString(rmi.Valor4)) else 0 end) as Capacitados,
	---SUM(case when (rmi.Valor4) not like '%[^0-9]%' then convert(int,(rmi.Valor4)) else 0 end) as Capacitados,
	...
	group by r.nombre,convert(varchar, dateadd(day, -(day(r.fecha) - 1), r.fecha),112),r.usuario
	...
	select  id_usuario,usuario,sum(capacitados) 
	from #capacitados group by id_usuario,usuario
	...
	insert #datos_alcance (alcance, total_alcance,meses,id_usuario)
		select  'Alcance: ' + r.nombre,
		isnull(cantidad,0), d.fecha, r.usuario
		from #datos_fecha_mes_final d
		inner join #tempReporte r on r.Usuario = d.Usuario ---and d.fecha = convert(varchar, dateadd(day, -(day(r.fecha) - 1), r.fecha),112)
		group by d.fecha,r.nombre,cantidad,r.usuario
		
		---select * from #datos_alcance
	
	create table #datos_alcance_total
	(
	...
	insert into #datos_alcance_total(alcance,total_alcance,id_usuario)
	select alcance, sum(total_alcance),id_usuario 
	from #datos_alcance group by alcance,id_usuario
	...
	if(@lenguaje = 'es')
		insert #columnasConfiguracion (name, title) values ('usuario','usuario'),('objetivo','objetivo'),('Capacitados','Capacitados')
	...
	b.total_alcance as objetivo,
	a.capacitados as Capacitados
	from #datos_alcance_total b 
	inner join #capacitados_total a on a.id_usuario = b.id_Usuario ---and  b.meses = a.fecha	
	group by a.usuario,	b.total_alcance,a.capacitados
...
--
--exec Whirpool_Referente_Asesores_T1 @IdCliente=201,@Filtros=@p2,@NumeroDePagina=-1,@Lenguaje='es'
