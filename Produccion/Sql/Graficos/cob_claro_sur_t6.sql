...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.cob_claro_sur_T6'))
   exec('CREATE PROCEDURE [dbo].[cob_claro_sur_T6] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[cob_claro_sur_T6] 	
(
	@IdCliente			int
...
--	set @cZonas = @@ROWCOUNT
	insert #zonas (idZona)
	select idZona From zona where idCliente in(select idCliente from familiaclientes)
	and nombre = 'SUR'
	set @cZonas = @@ROWCOUNT
...
-----------------------------------------------------------------
	...
	insert #resultados (idCliente,nombre,universo,cobertura,alcance)
	select 1000,'Total', sum(universo),sum(cobertura),sum(alcance)
	from #resultados


	select idCliente,isnull(nombre,''),isnull('Universo',''),isnull(universo,0),1 as tipo
	from #resultados 
	union
	select idCliente,isnull(nombre,''),isnull('Cobertura',''),isnull(Cobertura,0),2 as tipo
	from #resultados
	union
	select idCliente,isnull(nombre,''),isnull('Alcance',''),isnull(Alcance,0),3 as tipo
	from #resultados
	order by idCliente

	
	select idCliente,isnull(nombre,''),isnull(idCliente,0),isnull(nombre,''),isnull('Universo',''),isnull(universo,0),1 as tipo
	from #resultados 
	union
	select idCliente,isnull(nombre,''),isnull(idCliente,0),isnull(nombre,''),isnull('Cobertura',''),isnull(Cobertura,0),2 as tipo
	from #resultados
	union
	select idCliente,isnull(nombre,''),isnull(idCliente,0),isnull(nombre,''),isnull('Alcance',''),isnull(Alcance,0),3 as tipo
	from #resultados
...
--Tercer columna universo/cobertura/alcance
--CuartaColumna Valor
--idUniverso/idcobertura/idalcance?

---------------------------------
--Primera IdCliente
--Segunda NombreCliente
--Tercera
--Cuarta
--QUinta Universo/Cobertura/alcance
--Sexta Valor
--Septima IdTipo
...
insert into @p2 values(N'fltFechaReporte',N'D,,')

exec cob_claro_t6 @IdCliente=126,@Filtros=@p2,@NumeroDePagina=0,@Lenguaje='es',@IdUsuarioConsulta=827,@TamañoPagina=8

fac_promedio_t6 2
Fac_PromedioMarcaProd_T6 2
*/