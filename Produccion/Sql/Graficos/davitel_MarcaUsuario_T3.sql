IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.davitel_MarcaUsuario_T3'))
   exec('CREATE PROCEDURE [dbo].[davitel_MarcaUsuario_T3] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[davitel_MarcaUsuario_T3]
(
	@IdCliente			int
...
*/		
	...
	insert #datos (idUsuario, idMarca, qty)
	select r.idUsuario,m.idMarca,count(r.idReporte)--count(r.idReporte)
	from #TempReporte r
	...
	group by r.idUsuario,m.idMarca

	insert #datos(idusuario,idMarca,qty)
	select distinct u.idUsuario,m.idMarca,0 from
	(select idMarca from #datos)m
	cross apply
	(select idUsuario from #datos)u
	where not exists
	(select 1 from #datos 
		where idUsuario = u.idUsuario
		and   idMarca = m.idMarca)

	select u.usuario,m.nombre,d.qty,d.idUsuario,convert(varchar(10),d.idMarca) as idMarca
	--select m.nombre,u.usuario,d.qty,d.idMarca,d.idUsuario
	from #datos d
	inner join marca m 
	on m.idMarca = d.idMarca
	inner join usuario u
	on u.idUsuario = d.idUsuario 
	order by d.idMarca,d.idUsuario
