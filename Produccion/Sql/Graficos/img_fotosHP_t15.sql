IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Img_FotosHp_T15'))
   exec('CREATE PROCEDURE [dbo].[Img_FotosHP_T15] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Img_FotosHP_T15] 	
(
	@IdCliente			int
...
*/		
	...
	where	c.IdCliente in(select idCliente from #clientes)
			...
			and exists(select 1 from imagenesTags where idImagen =pdvf.idpuntodeventafoto
				 and idTag in(select idTag from tags where leyenda like '%HP%'))
		...
		where idPdvFoto not in (select idPdvFoto from #datosRes dr 
							inner join imagenesTags it on it.idImagen = dr.IdPdvFoto
								where idTag in(select idTag from tags where leyenda like '%HP%')
							)
	...
	--Datos
		...
		select	'https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/HP_New_Logo_2D.svg/2000px-HP_New_Logo_2D.svg.png' as Imagen
end
go
