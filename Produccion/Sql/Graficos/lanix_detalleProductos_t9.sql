IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_DetalleProductos_T9'))
   exec('CREATE PROCEDURE [dbo].[Lanix_DetalleProductos_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].Lanix_DetalleProductos_T9
(
	@IdCliente			int
...
*/		
	...
	insert #reportes (idPuntodeventa,idReporte,fecha)
	select r.idPuntodeventa,r.idReporte,max(convert(date,r.fechaCreacion))
	from Reporte r
	...
	group by r.idpuntodeventa,r.idreporte
	if(@@rowcount = 0)
	BEGIN
		insert #reportes (idPuntodeventa,idReporte,fecha)
		select r.idPuntodeventa,r.idReporte,max(convert(date,r.fechaCreacion))
		...
		inner join PuntoDeVenta p on p.IdPuntoDeVenta=r.IdPuntoDeVenta
		inner join Cliente c on c.IdEmpresa=r.IdEmpresa
		where month(r.FechaCreacion) = month(@fechaHasta)
				and year(r.FechaCreacion) = year(@fechaHasta)
				and exists (select 1 from #clientes where idCliente = c.idCliente)
				and (isnull(@cZonas,0) = 0 or exists(select 1 from #zonas where idZona = p.IdZona))
				and (isnull(@cCadenas,0) = 0 or exists(select 1 from #cadenas where idCadena = p.IdCadena))
				and (isnull(@cLocalidades,0) = 0 or exists(select 1 from #localidades where idLocalidad = p.IdLocalidad))
	...
	insert #datosFinal(pdv,sku,cantidad,fecha)
	select pdv.nombre,p.nombre,isnull(rp.cantidad2,0),rr.fecha
	from #reportes rr
	inner join reporteproducto rp on rp.idreporte = rr.idReporte
	inner join producto p on rp.idProducto = p.idProducto
	inner join puntodeventa pdv on pdv.idPuntodeventa = rr.idPuntodeventa
	where	--(isnull(@cMarcas,0) = 0 or exists(select 1 from #marcas where idMarca = p.IdMarca))
			 (isnull(@cProductos,0) = 0 or exists(select 1 from #productos where idProducto = p.IdProducto))
	...
	values ('pdv','Punto de Venta',10,0),
	('sku','SKU',10,1),
	('cantidad','Cantidad',10,2),
	('fecha','Fecha Ult Reporte',10,3)

	select name, title, width from #columnasDef order by orden, name
	

	if(@NumeroDePagina>0)
	select pdv,sku,cantidad,convert(varchar,fecha,103) as fecha from #datosfinal
