IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Lanix_ReportePorUsuario_T9'))
   exec('CREATE PROCEDURE [dbo].[Lanix_ReportePorUsuario_T9] AS BEGIN SET NOCOUNT ON; END')
GO
alter procedure [dbo].[Lanix_ReportePorUsuario_T9]
(
	@IdCliente			int
...
*/		
	...
	insert #reportes (idPuntodeventa,idUsuario,fecha)
	select r.idPuntodeventa,r.idUsuario,max(convert(date,r.fechaCreacion))
	from Reporte r
...
/*	if(@@rowcount = 0)
	...
	END
*/
	insert #datosFinal(idpdv,PDV,Usuario,Fecha)
	select distinct pdv.idPuntodeventa,pdv.nombre collate database_default,u.usuario collate database_default,rr.fecha
	from #reportes rr
	...
	values 
	('idpdv','Id PuntoDeVenta',1,0),
	('pdv','Punto de Venta',20,1),
	('usuario','Usuario',19,2),
	...
	if(@NumeroDePagina>0)
	select idpdv,pdv,usuario,convert(varchar,fecha,103) as fecha from #datosfinal
