...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[EnviarMailPriceRequestAdminApprove]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[EnviarMailPriceRequestAdminApprove] AS' 
END
GO
ALTER procedure [dbo].[EnviarMailPriceRequestAdminApprove]
(
...
declare @Producto varchar(max)
	...
	EXEC PriceRequestMailHeader @Titulo = 'La solicitud de baja de precio fue aprobada por EAI', @Usuario = @usuario, @MailHeader = @MailHeader OUTPUT
