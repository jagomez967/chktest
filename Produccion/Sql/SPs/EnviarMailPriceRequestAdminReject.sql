...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[EnviarMailPriceRequestAdminReject]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[EnviarMailPriceRequestAdminReject] AS' 
END
GO
ALTER procedure [dbo].[EnviarMailPriceRequestAdminReject]
(
...
declare @Producto varchar(max)
	...
	EXEC PriceRequestMailHeader @Titulo = 'La solicitud fue rechazada por EAI', @Usuario = @usuario, @MailHeader = @MailHeader OUTPUT
