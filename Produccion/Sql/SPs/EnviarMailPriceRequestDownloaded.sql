...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[EnviarMailPriceRequestDownloaded]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[EnviarMailPriceRequestDownloaded] AS' 
END
GO
ALTER procedure [dbo].[EnviarMailPriceRequestDownloaded]
(
...
declare @Producto varchar(max)
	...
	EXEC PriceRequestMailHeader @Titulo = 'La Solicitud de baja de precio fue descargada', @Usuario = @usuario, @MailHeader = @MailHeader OUTPUT
