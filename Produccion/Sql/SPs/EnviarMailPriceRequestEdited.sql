...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[EnviarMailPriceRequestEdited]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[EnviarMailPriceRequestEdited] AS' 
END
GO
ALTER procedure [dbo].[EnviarMailPriceRequestEdited]
(
...
declare @Producto varchar(max)
	...
	EXEC PriceRequestMailHeader @Titulo = 'La solicitud de baja de precio fue editada', @Usuario = @usuario, @MailHeader = @MailHeader OUTPUT
