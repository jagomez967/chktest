...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[EnviarMailPriceRequestRejected]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[EnviarMailPriceRequestRejected] AS' 
END
GO
ALTER procedure [dbo].[EnviarMailPriceRequestRejected]
(
...
declare @Producto varchar(max)
	...
	EXEC PriceRequestMailHeader @Titulo = 'La solicitud de baja de precio fue rechazada por SEC', @Usuario = @usuario, @MailHeader = @MailHeader OUTPUT
	...
	set @MailBody = @MailBody + '<hr/>'
	set @MailBody = @MailBody + '<p><strong>Mesaje: </strong></p>'
	set @MailBody = @MailBody + '<p>'+@Mensaje+'</p>'	
