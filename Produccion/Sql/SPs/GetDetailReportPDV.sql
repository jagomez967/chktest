IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.GetDetailReportPDV'))
    exec('CREATE PROCEDURE [dbo].[GetDetailReportPDV] AS BEGIN SET NOCOUNT ON; END')
 Go
 alter procedure [dbo].[GetDetailReportPDV]
 (
	@idPuntoDeVenta	int 
...
*/		
	...
	select r.FechaCreacion as Fecha,
		   r.idreporte as IdReporte,
		   u.imagen as FotoUsuario,
		   u.apellido + ', ' + u.nombre collate database_default as NombreUsuario
		from reporte r
		inner join usuario u 
		on u.idUsuario = r.idUsuario
		inner join puntodeventa p
		on p.idPuntodeventa = r.idPuntodeventa
	where convert(date,r.fechaCreacion) between convert(date,dateadd(day,-day(getdate()) + 1, getdate())) and convert(date,getdate())
