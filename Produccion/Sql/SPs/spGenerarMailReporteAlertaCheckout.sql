...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spGenerarMailReporteAlertaCheckout]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[spGenerarMailReporteAlertaCheckout] AS' 
END
GO
ALTER procedure [dbo].[spGenerarMailReporteAlertaCheckout]
(
...
begin
	...
	BEGIN
		set @MailBody = @MailBody + '<p><strong>Cantidad de usuarios con Checkout: </strong>'+cast(@CantidadUsuarios as varchar)+'</p>'
	END
	ELSE
	BEGIN
