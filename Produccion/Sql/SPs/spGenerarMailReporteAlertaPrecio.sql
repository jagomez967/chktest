...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spGenerarMailReporteAlertaPrecio]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[spGenerarMailReporteAlertaPrecio] AS' 
END
GO
ALTER procedure [dbo].[spGenerarMailReporteAlertaPrecio]
(
	@idReporte int,
	@idAlerta int
)
...
begin
	...
	declare @precio1 decimal(18,3)
	declare @precio2 decimal(18,3)
	declare @precio3 decimal(18,3)



	if not exists(select 1 from ReporteProducto where idReporte = @idReporte and (isnull(precio,-1)>=0 or isnull(precio2,-1)>=0 or isnull(precio3,-1)>=0))
		return
	...
	declare @Precio1_text varchar(max)
	declare @Precio2_text varchar(max)
	declare @Precio3_text varchar(max)

	SELECT	@Precio1_text = isnull(m.Leyenda,'')
		...
		and m.Activo = 1
		and m.visibilidad = 1
		and m.idModuloItem =7
				
	SELECT	@Precio2_text = isnull(m.Leyenda,'')
		from m_moduloClienteItem m
		inner join m_moduloCliente mc
			on mc.idModuloCliente = m.idmoduloCliente
		where mc.idCliente = @idCliente
		...
		and m.Activo = 1
		and m.visibilidad = 1
		and m.idModuloItem = 50

	SELECT	@Precio3_text = isnull(m.Leyenda,'')
		from m_moduloClienteItem m
		inner join m_moduloCliente mc
			on mc.idModuloCliente = m.idmoduloCliente
		where mc.idCliente = @idCliente
	...
	for	
		...
		and 
		(
		isnull(RP.Precio,0)> 0
		 or isnull(rp.Precio2,0) > 0
			or isnull(rp.Precio3,0) > 0
		 )
		 and exists (select 1 
	...
	open cur_productos
	fetch next from cur_productos into @idmarca, @idproducto, @NombreMarca, @NombreProducto,@Precio1,@Precio2,@Precio3
	...
	begin
		...
		set @MailBody = @MailBody + '<th>Producto</th><th>'+@Precio1_Text+'</th><th>'+@Precio2_text+'</th><th>'+@Precio3_text+'</th>'
		set @MailBody = @MailBody + '</tr>'
		...
		begin
			...
			set @MailBody = @MailBody + '<td>'+ltrim(rtrim(isnull(str(@Precio1),'')))+'</td>'
			set @MailBody = @MailBody + '<td>'+ltrim(rtrim(isnull(str(@Precio2),'')))+'</td>'
			set @MailBody = @MailBody + '<td>'+ltrim(rtrim(isnull(str(@Precio3),'')))+'</td>'
			set @MailBody = @MailBody + '</tr>'
